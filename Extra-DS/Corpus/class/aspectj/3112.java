...
public class RuntimeVisibleAnnotationAttributeTest extends BcelTestCase {
    ...
    public void testSeeAnnotationsAsAttribute() throws ClassNotFoundException {
        ...
        assertTrue("Expected a RuntimeVisibleAnnotations attribute but found none", rvaAttr.length == 1);
    }
    ...
    public void testAnnotationsAttributeContainsRightData() throws ClassNotFoundException {
        ...
        assertTrue("Should be one annotation but found " + anns.size(), anns.size() == 1);
        Annotation ann = (Annotation) anns.get(0);
        assertTrue("Should be called 'SimpleAnnotation' but was called " + ann.getTypeName(), ann.getTypeName().equals("SimpleAnnotation"));
        List l = ann.getValues();
        assertTrue("Should be one value for annotation 'SimpleAnnotation' but found " + l.size(), l.size() == 1);
        ElementNameValuePair envp = (ElementNameValuePair) l.get(0);
        assertTrue("Name of element in SimpleAnnotation should be 'id' but it is " + envp.getNameString(), envp.getNameString().equals("id"));
        ...
        assertTrue("'id' should have value 4 but it is " + evalue.getValueInt(), evalue.getValueInt() == 4);
    }
    ...
    public void testAccessingAnnotationsOnClazz() throws ClassNotFoundException {
        ...
        assertTrue("Expected one annotation on SimpleAnnotatedClass class but got " + anns.length, anns.length == 1);
    }
    ...
    public void testReadingWritingAnnotations() throws ClassNotFoundException, IOException {
        ...
        assertTrue("Expected one annotation on SimpleAnnotatedClass class but got " + anns2.length, anns2.length == 1);
    ...
    private void verifyAnnotationStringElement(JavaClass clazz) {
        ...
        assertTrue("should be called 'AnnotationStringElement' but was called " + ann.getTypeName(), ann.getTypeName().equals("AnnotationStringElement"));
        List l = ann.getValues();
        assertTrue("Should be one value but there were " + l.size(), l.size() == 1);
        ElementNameValuePair nvp = (ElementNameValuePair) l.get(0);
        assertTrue("Name of element should be 'stringval' but was " + nvp.getNameString(), nvp.getNameString().equals("stringval"));
        SimpleElementValue ev = (SimpleElementValue) nvp.getValue();
        assertTrue("String value should be 'hello' but was '" + ev.getValueString() + "'", ev.getValueString().equals("hello"));
    }
    ...
    private void verifyComplexAnnotation(JavaClass clazz) {
        ...
        assertTrue("Should be one annotation but found " + anns.length, anns.length == 1);
        Annotation ann = anns[0];
        assertTrue("Should be called 'ComplexAnnotation' but was called " + ann.getTypeName(), ann.getTypeName().equals("ComplexAnnotation"));
        List l = ann.getValues();
        assertTrue("Should be eight values for annotation 'ComplexAnnotation' but found " + l.size(), l.size() == 8);
    ...
    private void verifyClassAnnotation(JavaClass clazz) {
        ...
        assertTrue("should be one annotation but found " + anns.length, anns.length == 1);
        Annotation ann = anns[0];
        assertTrue("should be called 'AnnotationClassElement' but was called " + ann.getTypeName(), ann.getTypeName().equals("AnnotationClassElement"));
        List l = ann.getValues();
        assertTrue("Should be one value but there were " + l.size(), l.size() == 1);
        ElementNameValuePair nvp = (ElementNameValuePair) l.get(0);
        assertTrue("Name of element should be 'clz' but was " + nvp.getNameString(), nvp.getNameString().equals("clz"));
        ClassElementValue ev = (ClassElementValue) nvp.getValue();
        assertTrue("String value should be 'Ljava/lang/Integer;' but was '" + ev.getClassString() + "'", ev.getClassString().equals("Ljava/lang/Integer;"));
    }
    ...
    public void verifyAnnotationEnumElement(JavaClass clazz) {
        ...
        assertTrue("should be one annotation but found " + anns.length, anns.length == 1);
        Annotation ann = anns[0];
        assertTrue("should be called 'AnnotationEnumElement' but was called " + ann.getTypeName(), ann.getTypeName().equals("AnnotationEnumElement"));
        List l = ann.getValues();
        assertTrue("Should be one value but there were " + l.size(), l.size() == 1);
        ElementNameValuePair nvp = (ElementNameValuePair) l.get(0);
        assertTrue("Name of element should be 'enumval' but was " + nvp.getNameString(), nvp.getNameString().equals("enumval"));
        ...
        assertTrue("String value should be 'Red' but was '" + eev.getEnumValueString() + "'", eev.getEnumValueString().equals("Red"));
    }
    ...
    private void checkCombinedAnnotation(Annotation ann) {
        assertTrue("should be called 'CombinedAnnotation' but was called " + ann.getTypeName(), ann.getTypeName().equals("CombinedAnnotation"));
        List l = ann.getValues();
        assertTrue("Should be one value but there were " + l.size(), l.size() == 1);
        ElementNameValuePair nvp = (ElementNameValuePair) l.get(0);
        assertTrue("Name of element should be 'value' but was " + nvp.getNameString(), nvp.getNameString().equals("value"));
        ElementValue ev = nvp.getValue();
        ...
        assertTrue("Array element value should be of size 1 but is " + aev.getElementValuesArraySize(), aev.getElementValuesArraySize() == 1);
        ElementValue[] evs = aev.getElementValuesArray();
        assertTrue("Entry in the array should be AnnotationElementValue but is " + evs[0], evs[0] instanceof AnnotationElementValue);
        ...
        assertTrue("Value of 'id' should be 4 but it is " + envp.getValue().stringifyValue(), envp.getValue().stringifyValue().equals("4"));
    }
