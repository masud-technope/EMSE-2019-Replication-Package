...
public class SampleGatherer {
    ...
    private static void gatherFromFile(final File source, final Samples sink) throws IOException {
        Reader reader = null;
        try {
            String author = null;
            ...
            while (null != (line = lineReader.readLine())) {
                ...
                if (-1 != loc) {
                    ...
                    endLine = lineNumber;
                    Sample sample = new Sample(anchorName, anchorTitle, author, sampleCode.toString(), source, startLine, endLine, (String[]) flags.toArray(new String[flags.size()]));
...
class SamplesRenderer {
    ...
    public static final String COPYRIGHT = "<p><small>Copyright 2003 Contributors. All Rights Reserved. " + "This sample code is made available under the Common Public " + "License version 1.0 available at " + "<a href=\"http://www.eclipse.org/legal/cpl-v10.html\">" + "http://www.eclipse.org/legal/cpl-v10.html</a>." + "Contributors are listed in this document as authors. " + "Permission to republish portions of this sample code " + "is hereby granted if the publication acknowledges " + "the author by name and " + "the source by reference to the AspectJ project home page " + " at http://eclipse.org/aspectj.</small></p>" + EOL;
    ...
    protected void render(Sample sample, StringBuffer sink) {
        if (null != currentAnchor) {
            if (!currentAnchor.equals(sample.anchorName)) {
                String m = "expected " + currentAnchor + " got " + sample.anchorName;
                throw new Error(m);
            }
            currentAnchor = null;
        }
...
class SampleUtil {
    ...
    public static void simpleRender(Samples result, StringBuffer sink) {
        ...
        int i = 0;
        for (ListIterator iter = sortedSamples.listIterator(); iter.hasNext(); ) {
            Sample sample = (Sample) iter.next();
