...
public class WildTypePattern extends TypePattern {
    ...
    protected boolean matchesExactly(ResolvedType type, ResolvedType annotatedType) {
        ...
        return matchesExactlyByName(targetTypeName, type.isAnonymous(), type.isNested()) && matchesParameters(type, STATIC) && matchesBounds(type, STATIC) && annotationPattern.matches(annotatedType).alwaysTrue();
    }
    ...
    private boolean matchesBounds(ResolvedType aType, MatchKind staticOrDynamic) {
        ...
        if (lowerBound == null && aType.getLowerBound() != null)
            return false;
        if (upperBound != null) {
            // match ? extends
            if (aType.isGenericWildcard() && aType.isSuper())
                return false;
            if (aType.getUpperBound() == null)
                return false;
    ...
    public TypePattern parameterizeWith(Map typeVariableMap) {
        ...
        WildTypePattern ret = new WildTypePattern(newNamePatterns, includeSubtypes, dim, isVarArgs, typeParameters.parameterizeWith(typeVariableMap));
    ...
    private TypePattern maybeResolveToAnyPattern(IScope scope, Bindings bindings, boolean allowBinding, boolean requireExactType) {
        ...
        if (annotationPattern == AnnotationTypePattern.ANY) {
            if (// pr72531
            dim == 0 && !isVarArgs && upperBound == null && lowerBound == null && (additionalInterfaceBounds == null || additionalInterfaceBounds.length == 0)) {
    ...
    private TypePattern resolveGenericWildcard(IScope scope, UnresolvedType aType) {
        if (!aType.getSignature().equals(GENERIC_WILDCARD_CHARACTER))
    ...
    private TypePattern resolveParameterizedType(IScope scope, UnresolvedType aType, boolean requireExactType) {
        ...
        if (!verifyTypeParameters(aType.resolve(scope.getWorld()), scope, requireExactType))
            ...
            if (typeParameters.areAllExactWithNoSubtypesAllowed()) {
                for (int i = 0; i < tvs.length; i++) {
                    ...
                    if (continueCheck && !tvs[i].canBeBoundTo(ut.resolve(scope.getWorld()))) {
                        ...
                        String msg = WeaverMessages.format(WeaverMessages.VIOLATES_TYPE_VARIABLE_BOUNDS, parameterName, new Integer(i + 1), tvs[i].getDisplayName(), genericType.getName());
                        if (requireExactType)
