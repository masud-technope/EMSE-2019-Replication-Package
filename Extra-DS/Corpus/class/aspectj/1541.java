...
public class TypeVisitor implements ClassVisitor {
    ...
    public void visit(int version, int mods, String name, String signature, String superclassname, String[] interfacenames) {
        ...
        relatedDelegate.declaredSignature = signature;
    ...
    public MethodVisitor visitMethod(int modifiers, String name, String desc, String signature, String[] exceptions) {
        if (// wtf? (see testIterator in WorldTestCase)
        Modifier.isInterface(relatedDelegate.getModifiers()) && Modifier.isAbstract(modifiers))
    ...
    public void visitEnd() {
            ...
            for (Iterator iter = attributes.iterator(); iter.hasNext(); ) {
                ...
                if (element instanceof AjASMAttribute) {
                    ...
                    if (element.type.equals(Aspect.AttributeName)) {
                        ...
                        relatedDelegate.perClause = // param not used
                        aspectAttribute.reify(// param not used
                        null);
        ...
        if (annotations.size() > 0)
            relatedDelegate.annotations = new ArrayList();
        ...
        if (relatedDelegate.isAnnotation() && !retentionPolicySet) {
            relatedDelegate.retentionPolicy = "CLASS";
            relatedDelegate.isRuntimeRetention = false;
        }
    }

    private void setDelegateFieldsForAnnotationTarget(List /*AnnotationNameValuePair*/
    nvpairs) {
