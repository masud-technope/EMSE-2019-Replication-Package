 ...
 */
    public  Browser(Composite parent, int style) {
        ...
        try {
            //$NON-NLS-1$
            site = new WebSite(frame, SWT.NONE, "Shell.Explorer");
                ...
                switch(event.type) {
								...
								*/
									...
									*/
                                            if (OS.CreateStreamOnHGlobal(hGlobal, true, ppstm) == OS.S_OK) {
                                                int[] rgdispid = //$NON-NLS-1$
                                                auto.getIDsOfNames(//$NON-NLS-1$
                                                new String[] { "Document" });
					...
					*/
                    ...
                    case OnVisible:
                        {
							...
							*/
                                    int[] rgdispid = auto.getIDsOfNames(new //$NON-NLS-1$
                                    String[] //$NON-NLS-1$
                                    { "AddressBar" });
                                    Variant pVarResult = auto.getProperty(rgdispid[0]);
                    ...
                    case ProgressChange:
                        {
                            Variant arg1 = event.arguments[0];
                            int nProgress = // may be -1
                            arg1.getType() != OLE.VT_I4 ? // may be -1
                            0 : // may be -1
                            arg1.getInt();
