...
public class AsmDelegateTests extends AbstractWorldTestCase {
    ...
    public void testWeDontGoBang() {
        ...
        assertTrue("Should be an ASM delegate but is " + delegate.getClass(), delegate.getClass().toString().equals("class org.aspectj.weaver.asm.AsmDelegate"));
    }
    ...
    public void testCompareDelegates() {
        ...
        assertTrue("Should be a bcel delegate? " + bcelList.getDelegate().getClass(), bcelList.getDelegate().getClass().toString().equals("class org.aspectj.weaver.bcel.BcelObjectType"));
        assertTrue("Should be an asm delegate? " + asmList.getDelegate().getClass(), asmList.getDelegate().getClass().toString().equals("class org.aspectj.weaver.asm.AsmDelegate"));
    ...
    public void testCompareGenericMethods() {
        ...
        for (int i = 0; i < bcelMethods.length; i++) {
            // forget them, asm delegates dont currently know them
            bcelMethods[i].setParameterNames(null);
            ...
            if (bcelMethods[i].canBeParameterized()) {
                assertTrue("ASM method '" + asmMethods[i].toDebugString() + "' can't be parameterized whereas its' BCEL variant could", asmMethods[i].canBeParameterized());
            }
        }
        ...
        for (int i = 0; i < bcelMethods.length; i++) {
            if (bcelMethods[i].getName().equals("asSubclass")) {
                bcelSubclassMethod = bcelMethods[i];
                break;
            }
        }
        ResolvedMember asmSubclassMethod = null;
        for (int i = 0; i < asmMethods.length; i++) {
            if (asmMethods[i].getName().equals("asSubclass")) {
                asmSubclassMethod = asmMethods[i];
                break;
            }
        }
    ...
    public void testCompareDelegatesMonster() {
        BcelWorld slowWorld = new BcelWorld("../lib/aspectj/lib/aspectjtools.jar");
        slowWorld.setFastDelegateSupport(false);
    ...
    private void checkEquivalent(String prefix, AbstractReferenceTypeDelegate asmType, AbstractReferenceTypeDelegate bcelType) {
        assertTrue("Should be a bcel delegate? " + bcelType.getClass(), bcelType.getClass().toString().equals("class org.aspectj.weaver.bcel.BcelObjectType"));
        assertTrue("Should be an asm delegate? " + asmType.getClass(), asmType.getClass().toString().equals("class org.aspectj.weaver.asm.AsmDelegate"));
    ...
    public void testLoadingAttributesForMethods() {
        ...
        for (int i = 0; i < bcelMethods.length; i++) {
            ...
            assertTrue("Seem to be in a muddle. a=" + amember.getName() + " b=" + bmember.getName(), amember.getName().equals(bmember.getName()));
            if (debug)
                System.err.println("Looking at " + bmember);
            ...
            if (bcelEsa == null) {
                ...
                if (asmEsa == null)
                    fail("ASM effective signature is null, when BCEL effective signature is " + bcelEsa.toString());
            ...
            if (bcelSM == null) {
                ...
                if (asmSM == null)
                    fail("ASM effective signature is null, when BCEL effective signature is " + bcelSM.toString());
    ...
    public void testLoadingAnnotationsForFields() {
        ...
        for (int i = 0; i < bcelFields.length; i++) {
            ...
            String exp = null;
            if (i == 0)
                exp = "ANNOTATION [LSimpleAnnotation3;] [runtimeVisible] [id=1]";
            else if (i == 1)
                exp = "ANNOTATION [LSimpleAnnotation3;] [runtimeVisible] [id=2]";
    ...
    public void testLoadingAnnotationsForMethods() {
        ...
        for (int i = 0; i < bcelMethods.length; i++) {
            BcelMethod bmember = (BcelMethod) bcelMethods[i];
            AsmMethod amember = (AsmMethod) asmMethods[i];
            if (!bmember.getName().startsWith("method"))
                continue;
            assertTrue("Seem to be in a muddle. a=" + amember.getName() + " b=" + bmember.getName(), amember.getName().equals(bmember.getName()));
            if (debug)
                System.err.println("Looking at " + bmember);
