...
public class BcelTypeMunger extends ConcreteTypeMunger {
    ...
    public boolean munge(BcelClassWeaver weaver) {
        ...
        if (changed && worthReporting && munger != null && !weaver.getWorld().getMessageHandler().isIgnoring(IMessage.WEAVEINFO)) {
            ...
            if (munger.getKind().equals(ResolvedTypeMunger.Parent)) {
                ...
                if (declaredSig == null)
                    declaredSig = munger.getSignature();
    ...
    private boolean enforceDecpRule1_abstractMethodsImplemented(BcelClassWeaver weaver, ISourceLocation mungerLoc, LazyClassGen newParentTarget, ResolvedType newParent) {
        ...
        if (!(newParentTarget.isAbstract() || newParentTarget.isInterface())) {
            ...
            for (Iterator i = methods.iterator(); i.hasNext(); ) {
                ...
                if (o.isAbstract() && !o.getName().startsWith("ajc$interField")) {
                    ...
                    for (Iterator ii = newParentTargetMethods.iterator(); ii.hasNext() && discoveredImpl == null; ) {
                        ...
                        if (gen2.getName().equals(o.getName()) && gen2.getParameterSignature().equals(o.getParameterSignature()) && !gen2.isAbstract()) {
                    ...
                    if (discoveredImpl == null) {
                        ...
                        for (Iterator ii = newParentTarget.getType().getInterTypeMungersIncludingSupers().iterator(); ii.hasNext(); ) {
                            ...
                            if (m.getMunger().getKind() == ResolvedTypeMunger.Method) {
                                ...
                                if (!Modifier.isAbstract(sig.getModifiers())) {
                                    ...
                                    if (ResolvedType.matches(AjcMemberMaker.interMethod(sig, m.getAspectType(), sig.getDeclaringType().resolve(weaver.getWorld()).isInterface()), o)) {
                                        satisfiedByITD = true;
                                    }
                                }
                        ...
                        if (!satisfiedByITD) {
                            error(weaver, "The type " + newParentTarget.getName() + " must implement the inherited abstract method " + o.getDeclaringType() + "." + o.getName() + o.getParameterSignature(), newParentTarget.getType().getSourceLocation(), new ISourceLocation[] { o.getSourceLocation(), mungerLoc });
     ...
     */
    private boolean enforceDecpRule2_cantExtendFinalClass(BcelClassWeaver weaver, ISourceLocation mungerLoc, LazyClassGen newParentTarget, ResolvedType newParent) {
        if (newParent.isFinal()) {
            error(weaver, "Cannot make type " + newParentTarget.getName() + " extend final class " + newParent.getName(), newParentTarget.getType().getSourceLocation(), new ISourceLocation[] { mungerLoc });
            return false;
        }
        return true;
    }
    ...
    private void addFieldGetter(LazyClassGen gen, ResolvedMember field, ResolvedMember accessMethod) {
        ...
        if (field.isStatic()) {
            il.append(fact.createFieldAccess(gen.getClassName(), field.getName(), BcelWorld.makeBcelType(field.getType()), Constants.GETSTATIC));
        } else {
            il.append(InstructionConstants.ALOAD_0);
            il.append(fact.createFieldAccess(gen.getClassName(), field.getName(), BcelWorld.makeBcelType(field.getType()), Constants.GETFIELD));
        }
    ...
    protected LazyMethodGen makeMethodGen(LazyClassGen gen, ResolvedMember member) {
        LazyMethodGen ret = new LazyMethodGen(member.getModifiers(), BcelWorld.makeBcelType(member.getReturnType()), member.getName(), BcelWorld.makeBcelTypes(member.getParameterTypes()), UnresolvedType.getNames(member.getExceptions()), gen);
        // ret.makeSynthetic();    
        return ret;
    }
    ...
    private boolean mungePerObjectInterface(BcelClassWeaver weaver, PerObjectInterfaceTypeMunger munger) {
        ...
        if (couldMatch(gen.getBcelObjectType(), munger.getTestPointcut())) {
            ...
            LazyMethodGen mg = new LazyMethodGen(Modifier.PUBLIC, fieldType, NameMangler.perObjectInterfaceGet(aspectType), new Type[0], new String[0], gen);
            InstructionList il = new InstructionList();
            InstructionFactory fact = gen.getFactory();
            il.append(InstructionConstants.ALOAD_0);
            ...
            mg.getBody().insert(il);
            gen.addMethodGen(mg);
            LazyMethodGen mg1 = new LazyMethodGen(Modifier.PUBLIC, Type.VOID, NameMangler.perObjectInterfaceSet(aspectType), new Type[] { fieldType }, new String[0], gen);
    ...
    private boolean mungePerTypeWithinTransformer(BcelClassWeaver weaver) {
        ...
        Type fieldType = BcelWorld.makeBcelType(aspectType);
        LazyMethodGen mg = new LazyMethodGen(Modifier.PUBLIC | Modifier.STATIC, fieldType, NameMangler.perTypeWithinLocalAspectOf(aspectType), new Type[0], new String[0], gen);
        InstructionList il = new InstructionList();
    ...
    private boolean mungeNewMethod(BcelClassWeaver weaver, NewMethodTypeMunger munger) {
        ...
        if (onType.equals(gen.getType())) {
            ...
            if (weaver.getWorld().isInJava5Mode()) {
                ...
                if (realMember == null)
                    throw new BCException("Couldn't find ITD holder member '" + memberHoldingAnyAnnotations + "' on aspect " + aspectType);
            ...
            if (!onInterface && !Modifier.isAbstract(mangledInterMethod.getModifiers())) {
                ...
                if (// Don't need bridge methods if not in 1.5 mode.
                weaver.getWorld().isInJava5Mode()) {
            ...
            if (!gen.getType().isTopmostImplementor(onType)) {
                ...
                if (// Check if the munger being processed is a parameterized form of some original munger.
                munger.getDeclaredSignature() != null) {
    ...
    private boolean mungeMethodDelegate(BcelClassWeaver weaver, MethodDelegateTypeMunger munger) {
        ...
        if (shouldApply) {
            LazyMethodGen mg = new LazyMethodGen(introduced.getModifiers() - Modifier.ABSTRACT, BcelWorld.makeBcelType(introduced.getReturnType()), introduced.getName(), BcelWorld.makeBcelTypes(introduced.getParameterTypes()), BcelWorld.makeBcelTypesAsClassNames(introduced.getExceptions()), gen);
            ...
            //args
            int pos = 0;
            if (// skip 'this' (?? can this really happen)
            !introduced.isStatic()) {
                //body.append(InstructionFactory.createThis());
                pos++;
            }
    ...
    private boolean mungeFieldHost(BcelClassWeaver weaver, MethodDelegateTypeMunger.FieldHostTypeMunger munger) {
        ...
        boolean shouldApply = munger.matches(weaver.getLazyClassGen().getType(), aspectType);
        ResolvedMember host = AjcMemberMaker.itdAtDeclareParentsField(weaver.getLazyClassGen().getType(), munger.getSignature().getType(), aspectType);
        weaver.getLazyClassGen().addField(makeFieldGen(weaver.getLazyClassGen(), host).getField(), null);
        return true;
    }
    ...
    private boolean mungeNewField(BcelClassWeaver weaver, NewFieldTypeMunger munger) {
        ...
        if (onType.equals(gen.getType())) {
            ...
            ResolvedMember itdfieldGetter = AjcMemberMaker.interFieldInterfaceGetter(field, gen.getType(), /*onType*/
            aspectType);
            LazyMethodGen mg = makeMethodGen(gen, itdfieldGetter);
            InstructionList il = new InstructionList();
            InstructionFactory fact = gen.getFactory();
            ...
            if (// is this munger a parameterized form of some original munger?
            munger.getDeclaredSignature() != null) {
                ResolvedMember toBridgeTo = munger.getDeclaredSignature().parameterizedWith(null, munger.getSignature().getDeclaringType().resolve(getWorld()), false, munger.getTypeVariableAliases());
                boolean needsbridging = false;
                if (!toBridgeTo.getReturnType().getErasureSignature().equals(munger.getSignature().getReturnType().getErasureSignature()))
	 ...
	 * the strings "A" and "B".
	 */
    public List getTypeVariableAliases() /*String*/
    {
    ...
    public boolean equals(Object other) {
        ...
        return ((o.getMunger() == null) ? (getMunger() == null) : o.getMunger().equals(getMunger())) && ((o.getAspectType() == null) ? (getAspectType() == null) : o.getAspectType().equals(getAspectType()));
    }
