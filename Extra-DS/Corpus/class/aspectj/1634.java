     ...
     */
        ...
        switch(opcode) {
            case TABLESWITCH:
                ...
                for (int i = 0; i < jump_table.length; i++) buf.append("<TD><A HREF=\"#code" + method_number + "@" + jump_table[i] + "\">" + jump_table[i] + "</A></TD>");
       ...
       */
            case LOOKUPSWITCH:
                ...
                buf.append("<TH>default</TH></TR>\n<TR>");
                // Print target and default indices in second row
                for (int i = 0; i < npairs; i++) buf.append("<TD><A HREF=\"#code" + method_number + "@" + jump_table[i] + "\">" + jump_table[i] + "</A></TD>");
                buf.append("<TD><A HREF=\"#code" + method_number + "@" + default_offset + "\">" + default_offset + "</A></TD></TR>\n</TABLE>\n");
                break;
       ...
       */
            ...
            case INVOKEINTERFACE:
                ...
                ConstantNameAndType c2 = (ConstantNameAndType) constant_pool.getConstant(index, CONSTANT_NameAndType);
    ...
    private void writeMethod(Method method, int method_number) throws IOException {
        ...
        if (attributes.length > 0) {
            ...
            for (int i = 0; i < attributes.length; i++) {
                ...
                if (tag != ATTR_UNKNOWN)
                    file.print("<LI><A HREF=\"" + class_name + "_attributes.html#method" + method_number + "@" + i + "\" TARGET=Attributes>" + ATTRIBUTE_NAMES[tag] + "</A></LI>\n");
                else
                ...
                if (tag == ATTR_CODE) {
                    ...
                    for (int j = 0; j < attributes2.length; j++) {
                        ...
                        file.print("<LI><A HREF=\"" + class_name + "_attributes.html#" + "method" + method_number + "@" + i + "@" + j + "\" TARGET=Attributes>" + ATTRIBUTE_NAMES[tag] + "</A></LI>\n");
                    }
                    file.print("</UL>");
                }
            }
