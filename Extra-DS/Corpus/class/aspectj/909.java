...
public class ControlFlowGraph {
    ...
    private class InstructionContextImpl implements InstructionContext {
        ...
        public boolean execute(Frame inFrame, ArrayList execPreds, InstConstraintVisitor icv, ExecutionVisitor ev) {
            ...
            if (// no incoming frame was set, so set it.
            inF == null) {
                inFrames.put(lastExecutionJSR(), inFrame);
                inF = inFrame;
            } else // if there was an "old" inFrame
            {
                if (//shortcut: no need to merge equal frames.
                inF.equals(inFrame)) {
                    return false;
                }
        ...
        private InstructionHandle[] _getSuccessors() {
            ...
            if (inst instanceof RET) {
                ...
                //return empty; // RET in dead code. "empty" would be the correct answer, but we know something about the surrounding project...
                if (s == null) {
