    ...
    private static String[] matchVCardPrefixedField(String prefix, String rawText, boolean trim) {
        ...
        while (i < max) {
            ...
            while ((i = rawText.indexOf((int) '\n', i)) >= 0) {
                ...
                i < rawText.length() - 1 && (// this is only a continuation
                rawText.charAt(i + 1) == ' ' || rawText.charAt(i + 1) == '\t')) {
                    // Skip \n and continutation whitespace
                    i += 2;
                } else if (// If preceded by = in quoted printable
                quotedPrintable && (// this is a continuation
                rawText.charAt(i - 1) == '=' || rawText.charAt(i - 2) == '=')) {
                    // Skip \n
                    i++;
                } else {
                    break;
