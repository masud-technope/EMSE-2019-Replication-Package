...
public class FieldAnnotationsTest extends BcelTestCase {
    ...
    public void testFieldAnnotationsModification() throws ClassNotFoundException, IOException {
        ...
        if (dbg)
            System.err.println("Annotations on field are: " + dumpAnnotations(f.getAnnotations()));
        ...
        if (dbg)
            System.err.println("Adding annotation to the field");
        fg.addAnnotation(fruitBasedAnnotation);
        if (dbg)
            System.err.println("FieldGen (mutable field) is " + fg);
        if (dbg)
            System.err.println("with annotations: " + dumpAnnotations(fg.getAnnotations()));
        if (dbg)
            System.err.println("Replacing original field with new field that has extra annotation");
        clg.removeField(f);
        ...
        if (dbg)
            System.err.println("With annotations: " + dumpAnnotations(f.getAnnotations()));
        assertTrue("Should be 2 annotations on this field, but there are " + f.getAnnotations().length, f.getAnnotations().length == 2);
    }

    // helper methods
    public void checkAnnotatedField(JavaClass clazz, String fieldname, String annotationName, String annotationElementName, String annotationElementValue) {
    ...
    private void checkAnnotation(Annotation a, String name, String elementname, String elementvalue) {
        assertTrue("Expected annotation to have name " + name + " but it had name " + a.getTypeName(), a.getTypeName().equals(name));
