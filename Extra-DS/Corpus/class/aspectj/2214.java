...
public class Subroutines {
    ...
    public  Subroutines(MethodGen mg) {
        ...
        while (iter.hasNext()) {
            ...
            if (// If we don't deal with the top-level 'subroutine'
            actual != all[0]) {
        ...
        for (int i = 0; i < handlers.length; i++) {
            ...
            while (// Note the inclusive/inclusive notation of "generic API" exception handlers!
            _protected != handlers[i].getEndPC().getNext()) {
                Enumeration subs = subroutines.elements();
                while (subs.hasMoreElements()) {
                    Subroutine sub = (Subroutine) subs.nextElement();
                    if (// We don't want to forbid top-level exception handlers.
                    sub != subroutines.get(all[0])) {
