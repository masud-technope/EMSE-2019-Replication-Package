...
public class WebInstall {
    ...
    public static final String SYNTAX = "java WebInstall url {args}" + EOL + "  url  - to installer" + EOL + "  args - normally -text install.properties" + EOL + "         (if n/a, use install.properties)" + EOL;

    /** default arguments assume file <code>install.properties</code> 
     * is in current directory */
    private static final String[] ARGS = new String[] { "-text", "install.properties" };

    /** @param args the String[] <code>{ "<url>" {, "-text", "<propsPath>" }</code> */
    public static void main(String[] args) throws Exception {
        if ((null != args) && (args.length > 0) && ("-baseurl".equals(args[0]))) {
            driver(args);
        } else {
            try {
    ...
    protected static String[] packages = new String[] { "aspectj-tools-", "", "aspectj-docs-", "", "ajde-forteModule-", "", "ajde-jbuilderOpenTool-", "" };
    ...
    protected void install(String[] args) throws Exception {
        if ((null == args) || (args.length < 1) || (null == args[0]) || (1 > args[0].length())) {
            if (printError("expecting installer URL"))
                return;
        }
        ...
        Class c = cl.loadClass("$installer$.org.aspectj.Main");
