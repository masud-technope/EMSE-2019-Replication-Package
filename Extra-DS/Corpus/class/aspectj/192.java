...
public abstract class Tester extends DebuggerAdapter implements DebuggerListener {
    ...
    //true;    
    public static boolean verboseSuccess = false;
    ...
    protected static final String errFile = "err.txt";
    ...
    static class Case {
        ...
        public String toString() {
            return "msg=" + msg + " line=" + line + " frames=" + frames + " locals=" + locals + " names=" + names + " sizes=" + sizes + " time=" + time;
        }
    }
            ...
            public void breakpointEvent(BreakpointEvent e) {
                try {
                    if (cases.size() > time) {
                        ...
                        if (lineNumber > 0) {
                            check(lineNumber == line, "Lines don't match " + lineNumber + ":" + line);
                        } else {
                            check(msg.endsWith(methodName), "Method '" + msg + "' does not match '" + methodName + "'.");
                        }
                        msg(msg + ": " + d.format(e));
                        threads(names, sizes);
                        where("", frames);
    ...
    protected boolean locals(List locals) throws DebuggerException {
        ...
        for (int i = 0; i < locals.size(); i++) {
            ...
            allGood &= check(there, "The local variable '" + locals.get(i) + "' was not found in\n" + d.locals(vars));
        }
        return allGood;
    }
    ...
    protected void threads(List names, List sizes) throws DebuggerException {
        for (int i = 0; i < names.size(); i++) {
            ...
            check(threads.size() == ((Integer) sizes.get(i)).intValue(), "need " + sizes.get(i) + " thread(s) in '" + names.get(i) + "':\n" + d.threads(threads));
        }
    }
    ...
    protected void where(String name, int frames) throws DebuggerException {
        try {
            ...
            check(stack.size() == frames, "need " + frames + " frame(s) in '" + name + "':\n" + d.frames(stack));
