...
public class PseudoTokens extends ASTNode {
    ...
    //XXX this is redundant with the field above
    PseudoToken[] tokens;
    ...
    private void checkEof(Parser parser) {
        ...
        if (tokenSource.next() != IToken.EOF) {
            parser.problemReporter().parseError(last.getStart(), last.getEnd(), TerminalTokens.TokenNameIdentifier, last.getString().toCharArray(), last.getString(), new String[] { endToken });
        }
    }
    ...
    private void reportError(Parser parser, ParserException pe) {
        ...
        parser.problemReporter().parseError(start, end, TerminalTokens.TokenNameIdentifier, found.toCharArray(), found, new String[] { pe.getMessage() });
    }
