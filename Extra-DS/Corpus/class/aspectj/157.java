...
public abstract class Advice extends ShadowMunger {
    ...
    protected List /*Lint.Kind*/
    suppressedLintKinds = null;
    ...
    public static Advice makeCflowEntry(World world, Pointcut entry, boolean isBelow, Member stackField, int nFreeVars, List innerCflowEntries, ResolvedType inAspect) {
        Advice ret = world.createAdviceMunger(isBelow ? AdviceKind.CflowBelowEntry : AdviceKind.CflowEntry, entry, stackField, 0, entry);
        //0);
    ...
    public static Advice makePerCflowEntry(World world, Pointcut entry, boolean isBelow, Member stackField, ResolvedType inAspect, List innerCflowEntries) {
        Advice ret = world.createAdviceMunger(isBelow ? AdviceKind.PerCflowBelowEntry : AdviceKind.PerCflowEntry, entry, stackField, 0, entry);
        ret.innerCflowEntries = innerCflowEntries;
        ret.concreteAspect = inAspect;
        return ret;
    }
    ...
    public boolean match(Shadow shadow, World world) {
        if (super.match(shadow, world)) {
            ...
            hasExtraParameter() && kind == AdviceKind.AfterThrowing) {
                ...
                if (shadow.getKind() == Shadow.PreInitialization) {
                    ...
                    if (getSignature().getReturnType() == ResolvedType.VOID) {
                        ...
                        if (// Set<Integer> and Set
                        shadowReturnType.isParameterizedType() && adviceReturnType.isRawType()) {
                            ResolvedType shadowReturnGenericType = // Set
                            shadowReturnType.getGenericType();
                            ResolvedType adviceReturnGenericType = // Set
                            adviceReturnType.getGenericType();
    ...
    public UnresolvedType getExtraParameterType() {
        ...
        if (signature instanceof ResolvedMember) {
            if (getConcreteAspect().isAnnotationStyleAspect()) {
                ...
                while ((baseParmCnt + 1 < genericParameterTypes.length) && (genericParameterTypes[baseParmCnt].equals(AjcMemberMaker.TYPEX_JOINPOINT) || genericParameterTypes[baseParmCnt].equals(AjcMemberMaker.TYPEX_STATICJOINPOINT) || genericParameterTypes[baseParmCnt].equals(AjcMemberMaker.TYPEX_ENCLOSINGSTATICJOINPOINT))) {
                    baseParmCnt++;
                }
    ...
    public boolean equals(Object other) {
        ...
        return o.kind.equals(kind) && ((o.pointcut == null) ? (pointcut == null) : o.pointcut.equals(pointcut)) && ((o.signature == null) ? (signature == null) : o.signature.equals(signature));
    }
