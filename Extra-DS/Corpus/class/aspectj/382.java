...
public class AdviceDeclaration extends AjMethodDeclaration {
    ...
    private AjAttribute makeAttribute() {
        if (kind == AdviceKind.Around) {
            return new AjAttribute.AdviceAttribute(kind, pointcutDesignator.getPointcut(), extraArgumentFlags, sourceStart, sourceEnd, null, proceedInInners, proceedCallSignatures, formalsUnchangedToProceed, declaredExceptions);
        } else {
            return new AjAttribute.AdviceAttribute(kind, pointcutDesignator.getPointcut(), extraArgumentFlags, sourceStart, sourceEnd, null);
        }
    }
    ...
    public void resolveStatements() {
        ...
        if (kind == AdviceKind.Around) {
            ...
            proceedMethodBinding = new MethodBinding(Modifier.STATIC, "proceed".toCharArray(), binding.returnType, resize(baseArgumentCount + 1, binding.parameters), exceptions, binding.declaringClass);
    ...
    private void generateProceedMethod(ClassScope classScope, ClassFile classFile) {
        ...
        codeStream.newArray(new ArrayBinding(classScope.getType(TypeBinding.JAVA_LANG_OBJECT, TypeBinding.JAVA_LANG_OBJECT.length), 1, classScope.environment()));
        int index = 0;
        for (int i = 0; i < nargs - 1; i++) {
    ...
    public void postParse(TypeDeclaration typeDec) {
        ...
        this.selector = NameMangler.adviceName(EclipseFactory.getName(typeDec.binding).replace('.', '_'), kind, adviceSequenceNumberInType, stringifiedPointcut.toString().hashCode()).toCharArray();
        if (arguments != null) {
