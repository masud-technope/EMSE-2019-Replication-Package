...
public class CompilerRunSpecTest extends TestCase {
    ...
    static {
        if (!SETUP_JAVA14) {
            System.err.println("warning: set -D" + Globals.J2SE14_RTJAR_NAME + "=... in order to run all tests");
        }
        if (!SETUP_JAVA13) {
            System.err.println("warning: set -D" + Globals.J2SE13_RTJAR_NAME + "=... in order to run all tests");
        }
    }
    ...
    private static String[][] duplicate(String[][] input, String prefix) {
        ...
        for (int i = 0; i < output.length; i++) {
            ...
            if ((length > 0) && (prefixLength > 0)) {
                ...
                output[i][0] = prefix + output[i][0].substring(prefixLength);
            }
        }
        return output;
    }
    ...
    public void testCompliance() {
        ...
        MessageHandler handler = runTest(specOptions, globalOptions, expectAdopted, resultContains, messagesContain);
        checkMessages(handler, null);
        // 1.3 should work with collision?
        globalOptions = new String[] { "-1.3" };
        resultContains = "1.3";
        handler = runTest(specOptions, globalOptions, expectAdopted, resultContains, messagesContain);
        checkMessages(handler, null);
        // 1.4 should work
    ...
    public void testSource() {
        ...
        checkSourceTargetVersionConversion("source", 2, FAIL, "not permitted");
        checkSourceTargetVersionConversion("source", 6, FAIL, "not permitted");
    }
    ...
    public void testCompilerOptions() {
        ...
        for (Iterator iter = options.iterator(); iter.hasNext(); ) {
            ...
            if (!(crsOptions.compilerIsLoadable(compilerOption))) {
                ...
                // XXX snoop
                String argValue = compilerOption.toString();
                String arg = Option.ON.render(argValue);
                checkCompilerOption(arg, className);
            }
        }
        if (0 < notLoaded.length()) {
            System.err.println(getClass().getName() + ".testCompilerOptions()" + " warning: compiler options not tested because not loadable: " + notLoaded);
        }
    }
    ...
    public void testSpecOptions() {
        ...
        // help, 
        String[][] input = new String[][] { new String[] { "-verbose" }, new String[] { "-incremental" }, new String[] { "-emacssym" }, new String[] { "-Xlint" }, new String[] { "-Xlint:error" }, new String[] { "-1.3" }, new String[] { "-1.4" }, new String[] { "-source", "1.3" }, new String[] { "-source", "1.4" }, new String[] { "-target", "1.1" }, new String[] { "-target", "1.2" }, new String[] { "-preserveAllLocals" }, new String[] { "-referenceInfo" }, new String[] { "-deprecation" }, new String[] { "-noImportError" }, new String[] { "-proceedOnError" } };
        String[][] literalInput = new String[][] { new String[] { "-nowarn" }, new String[] { "-warn:deprecated" }, new String[] { "-warn:deprecated,unverified" }, new String[] { "-warn:deprecated", "-warn:unusedLocals" }, new String[] { "-g" }, new String[] { "-g", "-g:none" }, new String[] { "-g:vars,source" }, new String[] { "-verbose", "-g:vars,source" } };
        // normal
    ...
    void checkSourceTargetVersionConversion(String name, int i, boolean expectPass, String expectedErr) {
        final String specOptions = "-" + name + ", 1." + i;
        String[] globalOptions = new String[0];
        boolean expectAdopted = expectPass;
        String resultContains = !expectPass ? null : "[-" + name + ", 1." + i;
        String messagesContain = expectedErr;
        /*MessageHandler handler =*/
        runTest(specOptions, globalOptions, expectAdopted, resultContains, messagesContain);
    }
     ...
     * @return
     */
    MessageHandler runTest(String specOptions, String[] globalOptions, boolean expectAdopted, String resultContains, String messagesContain) {
        MessageHandler handler = new MessageHandler();
        try {
            ...
            boolean adopted = spec.adoptParentValues(parentRuntime, handler);
            if (adopted != expectAdopted) {
                String s = (expectAdopted ? "not " : "") + "adopted spec=" + specOptions + " globals=" + (LangUtil.isEmpty(globalOptions) ? "[]" : Arrays.asList(globalOptions).toString()) + " -- " + handler;
                assertTrue(s, false);
            }
            if (null != resultContains) {
                String result = "" + spec.testSetup.commandOptions;
                if (-1 == result.indexOf(resultContains)) {
                    assertTrue("expected " + resultContains + " got " + result, false);
                }
            }
            if (null != messagesContain) {
                boolean haveMessages = (0 != handler.numMessages(null, true));
                if (!haveMessages) {
                    assertTrue("expected " + messagesContain, false);
                } else {
                    String messages = handler.toString();
                    if (-1 == messages.indexOf(messagesContain)) {
                        assertTrue("expected " + messagesContain + " got " + messages, false);
                    }
                }
            }
            return handler;
    ...
    void checkSetupArgs(String arg, final boolean isTestArg) {
        ...
        try {
            ...
            if (!expResult.equals(result)) {
                assertTrue("expected " + expResult + " got " + result, false);
            }
