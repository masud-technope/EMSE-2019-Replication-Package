...
public abstract class AntTaskTester implements BuildListener {
    ...
    protected void checkAfterTask() {
        ...
        for (Iterator i = wants.iterator(); i.hasNext(); ) {
            Object want = i.next();
            check(haves.contains(want), "didn't see " + want + " in " + haves);
        }
        for (Iterator i = haves.iterator(); i.hasNext(); ) {
            Object have = i.next();
            check(wants.contains(have), "shouldn't have seen " + have + " in " + wants);
        }
        for (Iterator i = donts.iterator(); i.hasNext(); ) {
            Object dont = i.next();
            check(!haves.contains(dont), "should't have seen " + dont + " in " + haves);
        }
    }
    ...
    public void runProject(String[] args, String buildFile) {
        ...
        if (args == null || args.length == 0 || args[0].equals("${args}")) {
            ...
            for (int i = 0; i < args.length; i++) {
                String name = args[i];
                if (// todo wes: was (i)?
                !Character.isJavaIdentifierStart(name.charAt(0)) || name.charAt(0) == '$') {
                    continue;
                }
                try {
    ...
    private final void taskMethod(String prefix) {
        String name = prefix + Character.toUpperCase(taskname.charAt(0)) + taskname.substring(1);
        try {
    ...
    public int[] verbosities() {
        return new int[] { /*Project.MSG_VERBOSE,*/
