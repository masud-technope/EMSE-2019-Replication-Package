...
public class InstructionList implements Serializable {
     ...
     */
        for (int i = 0; i < count; i++) {
            if (ihs[i] instanceof BranchHandle) {
                ...
                if (// Either LOOKUPSWITCH or TABLESWITCH
                bi instanceof Select) {
                    Select s = (Select) bi;
   ...
   */
    public InstructionHandle append(Instruction i, InstructionList il) {
        InstructionHandle ih;
        if (// Also applies for empty list
        (ih = findInstruction2(i)) == null)
   ...
   */
    public void move(InstructionHandle start, InstructionHandle end, InstructionHandle target) {
        ...
        for (InstructionHandle ih = start; ih != end.next; ih = ih.next) {
            if (// At end of list, end not found yet
            ih == null)
                throw new ClassGenException("Invalid range: From " + start + " to " + end);
            else if (// target may be null
            ih == target)
                throw new ClassGenException("Invalid range: From " + start + " to " + end + " contains target " + target);
        }
   ...
   */
    private void remove(InstructionHandle prev, InstructionHandle next) throws TargetLostException {
        ...
        for (InstructionHandle ih = first; ih != null; ih = ih.next) // e.g. BranchInstructions release their targets
   ...
   */
    private InstructionHandle findInstruction1(Instruction i) {
        for (InstructionHandle ih = start; ih != null; ih = ih.next) if (ih.instruction == i)
            return ih;
        return null;
    }
   ...
   */
    private InstructionHandle findInstruction2(Instruction i) {
        for (InstructionHandle ih = end; ih != null; ih = ih.prev) if (ih.instruction == i)
            return ih;
        return null;
    }
    ...
    public boolean contains(InstructionHandle i) {
        if (i == null)
            return false;
        for (InstructionHandle ih = start; ih != null; ih = ih.next) if (ih == i)
            return true;
        return false;
    }
     ...
     */
        if (check) {
            for (InstructionHandle ih = start; ih != null; ih = ih.next) {
                ...
                if (// target instruction within list?
                i instanceof BranchInstruction) {
        ...
        while (ih != null) {
            ...
            if (i instanceof BranchInstruction) {
                ...
                bi instanceof Select) {
                    ...
                    for (// Update all targets
                    int j = 0; // Update all targets
                    j < itargets.length; // Update all targets
                    j++) {
   ...
   */
    public void dispose() {
        ...
        for (InstructionHandle ih = end; ih != null; ih = ih.prev) /* Causes BranchInstructions to release target and targeters, because it
        ...
        for (InstructionHandle ih = start; ih != null; ih = ih.next) {
            ...
            if (i instanceof BranchInstruction) {
                ...
                b instanceof Select) {
                    ...
                    for (// Update targets
                    int j = 0; // Update targets
                    j < targets.length; // Update targets
                    j++) if (targets[j] == old_target)
   ...
   */
    public void update() {
        if (observers != null)
            for (Iterator e = observers.iterator(); e.hasNext(); ) ((InstructionListObserver) e.next()).notify(this);
    }
}
