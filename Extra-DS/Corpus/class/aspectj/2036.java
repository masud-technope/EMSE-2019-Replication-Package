...
public class AssertInOnePackage {
    ...
    static void runTests() {
        check(true, "static initializer should throw", new Runnable() {

            void run() {
                StaticInitializerOnHelper.x = 3;
            }
        });
        check(false, "static initializer should not throw", new Runnable() {

            void run() {
                StaticInitializerOffHelper.x = 3;
            }
        });
        check(true, "constructor should throw", new Runnable() {

            void pre() {
                ConstructorOnHelper.x = 3;
            }

            void run() {
                new ConstructorOnHelper();
            }
        });
        check(false, "static initializer should not throw", new Runnable() {

            void pre() {
                ConstructorOffHelper.x = 3;
            }

            void run() {
                new ConstructorOffHelper();
            }
        });
        check(true, "inner static initializer should throw", new Runnable() {

            void run() {
                InnerStaticInitializerOnHelper.Inner.x = 3;
            }
        });
        check(false, "inner static initializer should not throw", new Runnable() {

            void run() {
                InnerStaticInitializerOffHelper.Inner.x = 3;
            }
        });
        check(true, "inner static initializer of interface should throw", new Runnable() {

            void run() {
                InnerStaticInitializerOnHelperI.Inner.x = 3;
            }
        });
        check(false, "inner static initializer of interface should not throw", new Runnable() {

            void run() {
                InnerStaticInitializerOffHelperI.Inner.x = 3;
            }
        });
        check(true, "static initializer in cyclic should throw", new Runnable() {

            void run() {
                CycleSubOn.x = 3;
            }
        });
        check(true, "static initializer in cyclic should always throw", new Runnable() {
