...
public class AbstractIndexWriter extends com.sun.tools.doclets.standard.AbstractIndexWriter {
    ...
    protected  AbstractIndexWriter(String path, String filename, String relativePath, IndexBuilder indexbuilder) throws IOException {
    ...
    protected void printMemberDesc(MemberDoc member) {
        String classdesc = Statics.type(member.containingClass()) + " " + getPreQualifiedClassLink(member.containingClass());
        ...
        if (member instanceof org.aspectj.ajdoc.MemberDoc) {
            IntroducedDoc intro = ((org.aspectj.ajdoc.MemberDoc) member).introduced();
            if (intro != null) {
                print(' ');
                printText("doclet.introduced_by", Statics.type(intro.containingClass()) + " " + getPreQualifiedClassLink(intro.containingClass()));
            }
        }
    }
}
