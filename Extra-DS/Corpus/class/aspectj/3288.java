...
public class ITDTest extends AjdocTestCase {
    ...
    public void testITDDeclaredOn() throws Exception {
        ...
        File[] files = { new File(getAbsoluteProjectDir() + "/src/pack/C.java"), new File(getAbsoluteProjectDir() + "/src/pack/A.aj") };
        ...
        boolean b = AjdocOutputChecker.detailSectionContainsRel(htmlA, "DECLARE DETAIL SUMMARY", "C.y", HtmlDecorator.HtmlRelationshipKind.DECLARED_ON, "HREF=\"../pack/C.html\"");
        assertTrue("Should have 'C.y declared on HREF=\"../pack/C.html\"" + "' in the Declare Detail section", b);
        b = AjdocOutputChecker.summarySectionContainsRel(htmlA, "DECLARE SUMMARY", "C.y", HtmlDecorator.HtmlRelationshipKind.DECLARED_ON, "HREF=\"../pack/C.html\"");
        assertTrue("Should have 'C.y declared on HREF=\"../pack/C.html\"" + "' in the Declare Summary section", b);
        // check the modifiers are correct in the declare detail summary
        String[] stringsA = { "private&nbsp;int", "public&nbsp;java.lang.String", "<H3>C.y</H3>", "public&nbsp;</TT><B>C.C", "package&nbsp;void" };
        ...
        assertEquals("the 'package' and 'void' modifiers shouldn't appear in the 'Declare Detail' section of the ajdoc", "package&nbsp;void", missing.get(0));
    ...
    public void testITDMatchesDeclare() throws Exception {
        ...
        if (htmlC == null || !htmlC.exists()) {
            fail("couldn't find " + getAbsolutePathOutdir() + "/pack/C.html - were there compilation errors?");
        }
        // check that the required sections exist
        assertTrue(htmlC.getAbsolutePath() + " should contain an " + "'INTER-TYPE METHOD SUMMARY' section", AjdocOutputChecker.containsString(htmlC, "INTER-TYPE METHOD SUMMARY"));
        assertTrue(htmlC.getAbsolutePath() + " should contain an " + "'INTER-TYPE FIELD SUMMARY' section", AjdocOutputChecker.containsString(htmlC, "INTER-TYPE FIELD SUMMARY"));
        assertTrue(htmlC.getAbsolutePath() + " should contain an " + "'INTER-TYPE CONSTRUCTOR SUMMARY' section", AjdocOutputChecker.containsString(htmlC, "INTER-TYPE CONSTRUCTOR SUMMARY"));
        ...
        assertEquals("There should be one missing string", 1, missing.size());
        assertEquals("public itd methods should not have the 'public' modifier in the ajdoc", "public", missing.get(0));
    ...
    public void testNoAspectDeclarations() throws Exception {
        ...
        boolean b = AjdocOutputChecker.classDataSectionContainsRel(htmlC, HtmlDecorator.HtmlRelationshipKind.ASPECT_DECLARATIONS, "pack.A.C.y");
        assertFalse("The class data section should not have 'aspect declarations" + " pack.A.C.y' since this is an ITD", b);
    }
}
