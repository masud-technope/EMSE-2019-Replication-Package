...
final class Camera {
    ...
    // simulator seems to need >= 500
    private static final int INITIALIZATION_TIME_MS = 500;

    // simulator seems to need >= 100
    private static final int KEY_PAUSE_TIME_MS = 100;
   ...
   */
    void exit() {
        if (setResolution) {
            // for now, we dont know the original resolution setting. Assume it was max res.
            setMaxResolution();
            sleep(KEY_PAUSE_TIME_MS);
        }
        // this sleep is needed for the esc to be processed(3000 originally)
        sleep(3000);
        ...
        if (app != null) {
            ...
            if (app.isForeground()) {
                ...
                // need two (no timeout in between esc key presses seems to work best)
   ...
   */
    private static void setMaxResolution() {
        ...
        keyUpAndDown(Characters.ESCAPE);
        sleep(KEY_PAUSE_TIME_MS);
        // yes to changes, even if there werent really any!
        trackBallClick();
   ...
   */
    ...
    private static void accessResolutionMenuAfterSave() {
        keyUpAndDown(Characters.CONTROL_MENU);
        // seems to be down 6 items on bb and 4 on simulator
        keyUpAndDown(Characters.CONTROL_DOWN, 6, 0);
        trackBallClick();
        keyUpAndDown(Characters.CONTROL_DOWN);
        keyUpAndDown(Characters.CONTROL_DOWN);
        trackBallClick();
   ...
   */
    ...
    private static void trackBallClick() {
        EventInjector.invokeEvent(new EventInjector.NavigationEvent(EventInjector.NavigationEvent.NAVIGATION_CLICK, 0, 0, 1));
        EventInjector.invokeEvent(new EventInjector.NavigationEvent(EventInjector.NavigationEvent.NAVIGATION_UNCLICK, 0, 0, 1));
    }
