...
public class TypeVariable {
    ...
    public boolean canBeBoundTo(ResolvedType aCandidateType) {
        ...
        if (// AMC - need a more robust test!
        aCandidateType.isGenericWildcard()) {
            return true;
        }
    ...
    private boolean matchingBounds(TypeVariableReferenceType tvrt) {
        if (tvrt.getUpperBound() != getUpperBound())
            return false;
        if (tvrt.hasLowerBound() != (getLowerBound() != null))
            return false;
        if (tvrt.hasLowerBound() && tvrt.getLowerBound() != getLowerBound())
            return false;
        // either we both have bounds, or neither of us have bounds
        if ((tvrt.additionalInterfaceBounds != null) != (additionalInterfaceBounds != null))
            return false;
        if (additionalInterfaceBounds != null) {
            // we both have bounds, compare
            if (tvrt.additionalInterfaceBounds.length != additionalInterfaceBounds.length)
                return false;
            ...
            if (!(aAndNotB.isEmpty() && bAndNotA.isEmpty()))
                return false;
        }
        return true;
    }
