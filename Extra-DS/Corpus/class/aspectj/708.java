...
public class Ajdoc extends AjdocCompiler {
    ...
    public int execute(String[] args) {
        try {
            ...
            return err.errors;
        } catch (InternalCompilerError // cf ajc.Main
        error) {
    ...
    public void handleInternalError(Throwable uncaughtThrowable) {
        System.err.println("An internal error occured in Ajdoc invocation of AspectJ-" + getCompiler().getVersion());
    ...
    public boolean doc(String[] args) {
        ...
        boolean good = true;
        try {
            rootDoc = makeRootDoc(sourcepath, classpath, bootclasspath, extdirs, verbosity, encoding, locale, source, filenamesAndPackages, options, err, programName, getFilter());
    ...
    private String set(String[] args, int i) {
        ...
        if (check(args, i)) {
            ...
            err.internalError("internal_error", new ArrayIndexOutOfBoundsException(i));
        }
        return arg;
    }
