...
public class RegressionComparePackageMode {
    ...
    static final String[] FILES_TO_COMPARE = { "/bPack/cPack/Class3.html", "/coordination/Coordinator.html", "/spacewar/Ship.html", "/spacewar/Debug.html" };

    static final String[] AJDOC_ARGS = { "-d", RUN_RESULTS_DIR, "-sourcepath", "input;input/pkgExample", "spacewar", "coordination", "bPack.cPack" };
    ...
    public static void test() {
        ...
        for (int i = 0; i < FILES_TO_COMPARE.length; i++) {
            Vector diffs = null;
            try {
                diffs = outputComparator.compareFilesByLine(CORRECT_RESULTS_DIR + FILES_TO_COMPARE[i], RUN_RESULTS_DIR + FILES_TO_COMPARE[i]);
            } catch (IOException ioe) {
                System.out.println("Couldn't compare files: " + ioe.getMessage());
            }
            String result = "";
            if (diffs != null)
                result = diffs.toString();
    ...
    public static void runCommand(String command) {
        try {
            ...
            throw new RuntimeException("could not execute: " + command + ", " + ioe.getMessage());
        }
    }
}
