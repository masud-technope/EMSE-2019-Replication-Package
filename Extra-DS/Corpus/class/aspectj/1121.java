 ...
 * @see Builder
 */
public class BuildModule extends // quickie hack...
Task {
    ...
    private void build(BuildSpec buildSpec) throws BuildException {
        ...
        try {
            ...
            if ((null == buildSpec.productDir) && (null != buildSpec.moduleDir)) {
                // XXXFileLiteral
                File buildScript = new File(buildSpec.moduleDir, "build.xml");
                if (buildScript.canRead()) {
                    built = buildByScript(buildSpec, buildScript);
                    if (!built) {
                        log("unable to build " + buildSpec + " using script: " + buildScript.getAbsolutePath());
                    }
                }
            }
            if (!built) {
                builder = AntBuilder.getBuilder(buildSpec.buildConfig, getProject(), buildSpec.tempDir);
