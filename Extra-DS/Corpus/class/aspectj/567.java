...
public class CompilerRun implements IAjcRun {
    ...
    public boolean setupAjcRun(Sandbox sandbox, Validator validator) {
        if (!validator.nullcheck(spec.getOptionsArray(), "localOptions") || !validator.nullcheck(sandbox, "sandbox") || !validator.nullcheck(spec.compiler, "compilerName") || !validator.canRead(Globals.F_aspectjrt_jar, "aspectjrt.jar") || !validator.canRead(Globals.F_testingclient_jar, "testing-client.jar")) {
            return false;
        }
        ...
        if (!spec.badInput) {
            if (!validator.canRead(testBaseSrcDir, srcPaths, "sources") || // see validation of inpathPaths below due to ambiguous base dir
            !validator.canRead(testBaseSrcDir, spec.argfiles, "argfiles") || !validator.canRead(testBaseSrcDir, spec.classpath, "classpath") || !validator.canRead(testBaseSrcDir, spec.aspectpath, "aspectpath") || !validator.canRead(testBaseSrcDir, spec.sourceroots, "sourceroots") || !validator.canRead(testBaseSrcDir, spec.extdirs, "extdirs")) {
                return false;
            }
        }
        ...
        if (!validator.canRead(injarFiles, "injars") || !validator.canRead(injarFiles, "injars")) {
            return false;
        }
        // hmm - duplicates validation above, verifying getBaseDirFiles?
        if (!spec.badInput) {
            if (!validator.canRead(argFiles, "argFiles") || !validator.canRead(injarFiles, "injarFiles") || !validator.canRead(inpathFiles, "inpathFiles") || !validator.canRead(aspectFiles, "aspectFiles") || !validator.canRead(classFiles, "classFiles") || !validator.canRead(xlintFiles, "xlintFiles")) {
                return false;
            }
        }
        final File[] srcFiles;
    ...
    public boolean run(IRunStatus status) {
        ...
        try {
            ...
            ICommand compiler = spec.reuseCompiler ? // throws IllegalStateException if null
            sandbox.getCommand(this) : ReflectionFactory.makeCommand(setupResult.compilerName, status);
            ...
            if (!handlerResult) {
                ...
                if (!result) {
                    String m = commandResult ? "compile did not fail as expected" : "compile failed unexpectedly";
    ...
    public static class Spec extends AbstractRunSpec {
        ...
        public static final String DEFAULT_COMPILER = ReflectionFactory.ECLIPSE;
        ...
        private static String updateBootclasspathForSourceVersion(String sourceVersion, String compilerName, ArrayList toAdd) {
            ...
            if (3 != sourceVersion.length()) {
                throw new IllegalArgumentException("bad version: " + sourceVersion);
            }
            if (null == toAdd) {
        ...
        private String getShortCompilerName() {
            ...
            if (null != compilerClassName) {
                ...
                if (-1 != loc) {
                    compilerClassName = compilerClassName.substring(loc + 1);
                }
            }
        ...
        protected TestSetup setupArgs(IMessageHandler handler) {
            ...
            Option.Family getFamily = CRSOPTIONS.crsIncrementalOption.getFamily();
            final boolean haveIncrementalFlag = (null != values.firstInFamily(getFamily));
            if (spec.isStaging()) {
            ...
            if (null != compiler) {
                result.compilerName = CRSOPTIONS.compilerClassName(compiler.option);
                if (null == result.compilerName) {
                    result.failureReason = "unable to get class name for " + compiler;
                    return checkResult(result);
                }
            }
            String compilerName = (null == result.compilerName ? spec.compiler : result.compilerName);
            ...
            if (null != source) {
                ...
                /*String err =*/
                updateBootclasspathForSourceVersion(sourceVersion, spec.compiler, toAdd);
                args.addAll(toAdd);
            }
        ...
        TestSetup checkResult(TestSetup result) {
            ...
            if (null == result) {
                ...
                err = result.result ? "expected no failure: " + result.failureReason : "fail for no reason";
            } else if (result.result != (null != result.commandOptions)) {
                err = result.result ? "expected command options" : "unexpected command options";
            }
            if (null != err) {
                throw new Error(err);
            }
        ...
        boolean hasInvalidOptions(Values values, TestSetup result) {
            ...
            for (Iterator iter = CRSOPTIONS.invalidOptions.iterator(); iter.hasNext(); ) {
                Option option = (Option) iter.next();
                if (null != values.firstOption(option)) {
                    result.failureReason = "invalid option in harness: " + option;
                    return true;
                }
            }
            return false;
        ...
        boolean hasCompilerSpecErrors(String compilerName, Values values, TestSetup result) {
            /*
             ...
             */
            if (!permitAnyCompiler && (!(ReflectionFactory.ECLIPSE.equals(compilerName) || ReflectionFactory.OLD_AJC.equals(compilerName) || CRSOptions.AJDE_COMPILER.equals(compilerName) || CRSOptions.AJCTASK_COMPILER.equals(compilerName) || permitAnyCompiler))) {
                //|| BUILDER_COMPILER.equals(compilerName))
                result.failureReason = "unrecognized compiler: " + compilerName;
                return true;
            }
            ...
            // not supporting 1.0 options any more
            for (Iterator iter = CRSOPTIONS.ajc10Options.iterator(); iter.hasNext(); ) {
                Option option = (Option) iter.next();
                if (null != values.firstOption(option)) {
        ...
        protected Values gatherValues(TestSetup result) {
            ...
            final Values globalValues = spec.runtime.extractOptions(options, true, errs);
            if (errs.length() > 0) {
                result.failureReason = errs.toString();
                return null;
            }
            final Values combined = Values.wrapValues(new Values[] { localValues, globalValues });
        ...
        Values handleTestArgs(Values values, final TestSetup result) {
            final Option.Family compilerFamily = CRSOPTIONS.ajccompilerOption.getFamily();
                ...
                protected boolean accept(Option.Value value) {
                    ...
                    if (compilerFamily.sameFamily(option.getFamily())) {
                        if (value.prefix.isSet()) {
                            String compilerClass = CRSOPTIONS.compilerClassName(option);
                            if (null == compilerClass) {
                                result.failureReason = "unrecognized compiler: " + value;
                                throw Values.Selector.STOP;
                            }
                            if (!CRSOPTIONS.compilerIsLoadable(option)) {
                                result.failureReason = "unable to load compiler: " + option;
                                throw Values.Selector.STOP;
                            }
                            result.compilerName = compilerClass;
                        }
                        return true;
                    } else if (CRSOPTIONS.crsIgnoreWarnings.sameOptionIdentifier(option)) {
        ...
        public void writeXml(XMLWriter out) {
            ...
            if (!LangUtil.isEmpty(aspectpath)) {
                out.printAttribute("aspectpath", XMLWriter.flattenFiles(aspectpath));
            }
            if (!LangUtil.isEmpty(sourceroots)) {
                out.printAttribute("sourceroots", XMLWriter.flattenFiles(sourceroots));
            }
            if (!LangUtil.isEmpty(extdirs)) {
                out.printAttribute("extdirs", XMLWriter.flattenFiles(extdirs));
            }
            out.endAttributes();
        ...
        class TestSetup {
            ...
            public String toString() {
                return "TestSetup(" + (null == compilerName ? "" : compilerName + " ") + (!ignoreWarningsSet ? "" : (ignoreWarnings ? "" : "do not ") + "ignore warnings ") + (result ? "" : "setup failed") + ")";
            }
        }
        ...
        static class CRSOptions {
            ...
            static final String AJCTASK_COMPILER = AjcTaskCompileCommand.class.getName();
            ...
            */
            ...
            private  CRSOptions() {
                ...
                eclipseOption = factory.create("eclipse", "compiler", Option.FORCE_PREFIXES, false);
                compilerFamily = eclipseOption.getFamily();
                buildercompilerOption = factory.create("builderCompiler", "compiler", Option.FORCE_PREFIXES, false);
                ajctaskcompilerOption = factory.create("ajctaskCompiler", "compiler", Option.FORCE_PREFIXES, false);
                ajdecompilerOption = factory.create("ajdeCompiler", "compiler", Option.FORCE_PREFIXES, false);
                ajccompilerOption = factory.create("ajc", "compiler", Option.FORCE_PREFIXES, false);
                javacOption = factory.create("javac", "compiler", Option.FORCE_PREFIXES, false);
                Map map = new TreeMap();
                map.put(eclipseOption, ReflectionFactory.ECLIPSE);
                //map.put(BUILDERCOMPILER_OPTION, BUILDER_COMPILER);
                map.put(ajctaskcompilerOption, AJCTASK_COMPILER);
                ...
                compilerOptionToClassname = Collections.unmodifiableMap(map);
                compilerOptions = Collections.unmodifiableSet(compilerOptionToClassname.keySet());
                ...
                crsSourceOption = factory.create("source", "source", Option.FORCE_PREFIXES, false, new String[][] { new String[] { "1.3", "1.4", "1.5" } });
                // ajc 1.0 options
                ...
                list.add(factory.create("lenient", "lenient", Option.FORCE_PREFIXES, false));
                list.add(factory.create("strict", "lenient", Option.FORCE_PREFIXES, false));
            ...
            String compilerClassName(Option option) {
                if ((null == option) || (!compilerFamily.sameFamily(option.getFamily()))) {
                    return null;
                }
            ...
            boolean compilerIsLoadable(Option compilerOption) {
                ...
                synchronized (compilerOptionToLoadable) {
                    Boolean result = (Boolean) compilerOptionToLoadable.get(compilerOption);
                    if (null == result) {
                        MessageHandler sink = new MessageHandler();
                        String compilerClassname = (String) compilerOptionToClassname.get(compilerOption);
                        if (null == compilerClassname) {
                            result = Boolean.FALSE;
                        } else {
                            ICommand c = ReflectionFactory.makeCommand(compilerClassname, sink);
                            if ((null == c) || sink.hasAnyMessage(IMessage.ERROR, true)) {
                                result = Boolean.FALSE;
                            } else {
                                result = Boolean.TRUE;
                            }
                        }
                        compilerOptionToLoadable.put(compilerOption, result);
                    }
