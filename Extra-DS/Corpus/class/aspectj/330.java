...
public class CompileCommand implements ICommand {
    ...
    private void waitForCompletion(long startTime) {
        ...
        while ((startTime > endTime) && (maxTime > System.currentTimeMillis())) {
            try {
                Thread.sleep(500);
    ...
    private void setup(String[] args) {
        ...
        EditorAdapter editorAdapter = (EditorAdapter) makeProxy(EditorAdapter.class);
        TaskListManager taskListManager = myHandler;
        BuildProgressMonitor buildProgressMonitor = new DefaultBuildProgressMonitor(new Frame()) {
        ...
        ProjectPropertiesAdapter projectPropertiesAdapter = new ProjectProperties(classesDir);
        // neither of these are in the true classpath
        //            new NullIdeProperties("");  // in testsrc
        //            = new BrowserProperties();  // in ajbrowser
        BuildOptionsAdapter buildOptionsAdapter = new AjcBuildOptions(new UserPreferencesStore(false));
        IdeUIAdapter ideUIAdapter = (IdeUIAdapter) makeProxy(IdeUIAdapter.class);
        ErrorHandler errorHandler = (ErrorHandler) makeProxy(ErrorHandler.class);
        ...
        StructureViewNodeFactory structureViewNodeFactory = new StructureViewNodeFactory(iconRegistry) {
            ...
            protected IStructureViewNode createRelationship(IRelationship node, AbstractIcon icon) {
        ...
        Ajde.init(editorAdapter, taskListManager, buildProgressMonitor, projectPropertiesAdapter, buildOptionsAdapter, structureViewNodeFactory, ideUIAdapter, errorHandler);
...
class MyTaskListManager extends MessageHandler implements TaskListManager {
...
class ProjectProperties implements ProjectPropertiesAdapter {
    ...
    private static final String PREFIX = ProjectProperties.class.getName() + ": ";
    ...
    public void setInJars(Set input) {
        inJars = input;
    }
    ...
    public void setSourceRoots(Set input) {
        sourceRoots = input;
    }
    ...
    public List getBuildConfigFiles() {
        return logs("buildConfigFiles");
    }

    public String getLastActiveBuildConfigFile() {
        return log("lastActiveBuildConfigFile");
    }
    ...
    public String getRootProjectDir() {
        return log("rootProjectDir");
    }

    public List getProjectSourceFiles() {
        return logs("projectSourceFiles");
    }

    public String getProjectSourcePath() {
        return log("projectSourcePath");
    }

    public String getAjcWorkingDir() {
        return log("ajcWorkingDir");
    }

    public String getClassToExecute() {
        return log("classToExecute");
    }

    public String getExecutionArgs() {
        return log("executionArgs");
    }
