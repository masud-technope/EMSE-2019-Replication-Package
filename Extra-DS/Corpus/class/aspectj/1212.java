...
public class Util {
    ...
    public static //,String[] expectedMethods) {
    void dumpMethods(//,String[] expectedMethods) {
    String clazzname) {
        ...
        try {
            ...
            if (m != null) {
                for (int i = 0; i < m.length; i++) {
                    String methodString = m[i].getReturnType().getName() + " " + m[i].getDeclaringClass().getName() + "." + m[i].getName() + "(" + stringify(m[i].getParameterTypes()) + ")" + (m[i].isBridge() ? " [BridgeMethod]" : "");
