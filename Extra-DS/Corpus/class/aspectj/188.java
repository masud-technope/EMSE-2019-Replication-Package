...
public class Util {
    ...
    public static final Tag[] inlineTags(Doc doc, String str, Locale loc, ErrPrinter err) {
        ...
        for (int j = i; i < N; j = i) {
            ...
            // match it
            i = ileft;
            if (i + 7 < N && str.substring(i + 2, i + 7).toLowerCase().equals("link ")) {
                i += 7;
                for (; str.charAt(i) != '}'; i++) {
                    if (i == N - 1) {
    ...
    public static final List findTags(Doc doc, String str, Locale loc, ErrPrinter err) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            //                lastTag = -1
                ...
                if (lastTag != -1) {
                    result.add(parse(doc, str.substring(lastTag, i), loc, err));
                }
                lastTag = i;
            } else {
                newline = false;
        ...
        if (lastTag != -1) {
            result.add(parse(doc, str.substring(lastTag), loc, err));
        }
        return result;
    }
    ...
    public static final String commentText(String rawCommentText) {
        ...
        outer: for (StringTokenizer t = new StringTokenizer(rawCommentText, "\n", true); t.hasMoreTokens(); ) {
            String line = t.nextToken();
            if (line == null || line.length() == 0)
                continue;
            int i;
    ...
    public static final void dump(Object o, String prefix) {
        ...
        for (Iterator i = list.iterator(); i.hasNext(); ) {
            ...
            if (m.getParameterTypes().length == 0 && m.getName().startsWith(prefix)) {
                try {
    ...
    public static String documentation(File html, ErrPrinter err) {
        ...
        for (int j = 1; j >= 0; j--) {
            nextTag: for (; i != -1; i = str.indexOf('<', i + 1)) {
                nextLt: for (int s = i + 1, k = j; s < str.length(); s++, k++) {
                    char c = str.charAt(s);
                    if (k == chars.length) {
                        is[j] += s + 2;
                        break nextTag;
    ...
    public static final ExecutableMemberDoc executableMemberDoc(ExecutableMemberDoc[] emds, String name, Parameter[] params) {
        ...
        next: for (int i = 0; i < emds.length; i++) {
    ...
    public static final PointcutDec pointcutDec(NameType nameType, String id, Formals formals) {
        ...
        next: for (Iterator i = nameType.getPointcuts().iterator(); i.hasNext(); ) {
            PointcutDec md = ((PointcutSO) i.next()).getPointcutDec();
            if (md.getFormals().size() == formals.size() && id.equals(md.getId())) {
                for (int j = 0; j < formals.size(); j++) {
                    if (!md.getFormals().get(j).getType().getString().equals(formals.get(j).getType().getString())) {
                        continue next;
                    }
                }
                result = md;
    ...
    public static final MethodDec methodDec(NameType nameType, String id, Formals formals) {
        ...
        next: for (Iterator i = nameType.getMethods().iterator(); i.hasNext(); ) {
