...
public class SignaturePattern extends PatternNode {
    ...
    public SignaturePattern resolveBindings(IScope scope, Bindings bindings) {
        ...
        if (throwsPattern != null) {
            ...
            if (throwsPattern.getForbidden().getTypePatterns().length > 0 || throwsPattern.getRequired().getTypePatterns().length > 0) {
    ...
    private class TypePatternVisitor extends AbstractPatternNodeVisitor {
        ...
        private Map incorrectTargetKinds = /* PatternNode -> AnnotationTargetKind[] */
        new HashMap();
    ...
    public SignaturePattern parameterizeWith(Map typeVariableMap) {
        SignaturePattern ret = new SignaturePattern(kind, modifiers, returnType.parameterizeWith(typeVariableMap), declaringType.parameterizeWith(typeVariableMap), name, parameterTypes.parameterizeWith(typeVariableMap), throwsPattern.parameterizeWith(typeVariableMap), annotationPattern.parameterizeWith(typeVariableMap));
    ...
    private FuzzyBoolean matchesAnnotations(ResolvedMember member, World world) {
        ...
        if (member.isAnnotatedElsewhere() && member.getKind() == Member.FIELD) {
            ...
            ResolvedMember[] mems = member.getDeclaringType().resolve(world).getDeclaredFields();
    ...
    private boolean matchedArrayAgainstVarArgs(TypePatternList params, int modifiers) {
        if (params.size() > 0 && (modifiers & Constants.ACC_VARARGS) != 0) {
            ...
            if (lastPattern.isArray() && !lastPattern.isVarArgs)
                return true;
        }
        return false;
    }
