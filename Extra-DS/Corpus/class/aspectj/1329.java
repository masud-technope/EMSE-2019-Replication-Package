...
public class Values {
    ...
    static int[] invert(int[] missed, int length) {
        ...
        for (int counter = 0; counter < MAX; counter++) {
            ...
            while (((missedIndex >= missed.length) || (missed[missedIndex] > counter)) && (counter < MAX)) {
    ...
    private static String resolve(Option.Value[] input) {
        ...
        for (Iterator iter = familyToMatches.entrySet().iterator(); (null == err) && iter.hasNext(); ) {
    ...
    private static String resolve(Option.Value[] input, int[] matches) {
            ...
            if (null != value) {
                ...
                if (VERIFYING) {
                    if (null == option) {
                        ...
                        String s = value.option + " has different family from " + option;
        ...
        for (int i = 0; (null == err) && (i < matches.length); i++) {
            ...
            if (null != value) {
                if (null == first) {
                    first = value;
                    if (first.option.getFamily().permitMultipleFamilyValues()) {
                        break;
                    }
                } else {
     ...
     */
    private static String removeForceOff(Option.Value[] input, Option.Value value, int[] matches) {
        if (!value.prefix.forceOff()) {
            throw new IllegalArgumentException("expecting force-off: " + value);
        }
        for (int i = 0; i < matches.length; i++) {
            Option.Value match = input[matches[i]];
            if ((null != match) && value.sameValueIdentifier(match)) {
                if (match.prefix.forceOn()) {
                    return "force conflict between " + value + " and " + match;
                } else {
                    // unset matches[i]? 
     ...
     */
    private static String convertForceOn(Option.Value[] input, int valueIndex, int[] matches) {
        Option.Value value = input[matches[valueIndex]];
        if (!value.prefix.forceOn()) {
            throw new IllegalArgumentException("expecting force-on: " + value);
        }
        ...
        for (int i = 0; i < matches.length; i++) {
            ...
            if (null != match) {
                ...
                if (match.prefix.forceOff()) {
                    throw new Error("unexpected force-off:" + match + " when processing " + value);
                }
