...
public class AjcTaskCompileCommand implements ICommand {
     ...
     */
    static boolean waitUntilMessagesQuiet(IMessageHolder holder, int seconds, int timeoutSeconds) {
        LangUtil.throwIaxIfNull(holder, "holder");
        if ((0 >= timeoutSeconds) || (0 >= seconds) || (timeoutSeconds <= seconds)) {
            return false;
        }
        ...
        while ((curTime < timeout) && (curTime < (numMessagesTime + targetQuietTime))) {
    ...
    public boolean runCommand(String[] args, IMessageHandler handler) {
        return (makeAjcTask(args, handler) && doCommand(handler, false));
    }
    ...
    protected boolean doCommand(IMessageHandler handler, boolean repeat) {
        ...
        try {
            ...
            // rest is for incremental mode
                ...
                incrementalCompileThread = new Thread(new Runnable() {
    ...
    protected boolean updateIncrementalFile(boolean create, boolean delete) {
        ...
        if (file.exists()) {
            ...
            } catch (IOException // XXX warn in verbose mode?
            e) {
                return false;
            }
        }
