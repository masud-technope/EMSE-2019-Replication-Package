...
public class MethodGen extends FieldGenOrMethodGen {
   ...
   */
    ...
    public  MethodGen(Method m, String class_name, ConstantPoolGen cp, boolean useTags) {
        this(m.getAccessFlags(), Type.getReturnType(m.getSignature()), Type.getArgumentTypes(m.getSignature()), null, /* may be overridden anyway */
        m.getName(), class_name, ((m.getAccessFlags() & (Constants.ACC_ABSTRACT | Constants.ACC_NATIVE)) == 0) ? new InstructionList(m.getCode().getCode()) : null, cp);
            ...
            if (a instanceof Code) {
                ...
                if (ces != null) {
                    for (int j = 0; j < ces.length; j++) {
                        ...
                        if (// May happen, because end_pc is exclusive
                        length == end_pc) {
                            end = il.getEnd();
                        } else {
                        ...
                        addExceptionHandler(il.findHandle(ce.getStartPC(), arrayOfInstructions), end, il.findHandle(ce.getHandlerPC(), arrayOfInstructions), c_type);
                    }
                }
                ...
                for (int j = 0; j < codeAttrs.length; j++) {
                    ...
                    if (a instanceof LineNumberTable) {
                        ...
                        if (useTags) {
                            ...
                            for (int k = 0; k < ln.length; k++) {
                                LineNumber l = ln[k];
                                addLineNumber(il.findHandle(l.getStartPC(), arrayOfInstructions), l.getLineNumber());
                            }
                        }
                        ...
                        if (useTags) {
                            ...
                            for (int k = 0; k < lv.length; k++) {
                                ...
                                if (b != Constants.T_ADDRESS) {
                                    ...
                                    if (l.getIndex() + increment > max_locals)
                                        max_locals = l.getIndex() + increment;
                                }
   ...
   */
    public LocalVariableGen addLocalVariable(String name, Type type, int slot, InstructionHandle start, InstructionHandle end) {
        ...
        if (t != Constants.T_ADDRESS) {
            ...
            int i;
            if (// Overwrite if necessary
            (i = variable_vec.indexOf(l)) >= 0)
   ...
   */
    public LineNumberTable getLineNumberTable(ConstantPoolGen cp) {
        ...
        try {
            for (int i = 0; i < size; i++) ln[i] = ((LineNumberGen) line_number_vec.get(i)).getLineNumber();
        }// Never occurs
   ...
   */
     ...
     */
        ...
        if ((il != null) && !isAbstract()) {
            ...
            code = new Code(cp.addUtf8("Code"), // prologue byte code
            8 + byte_code.length + 2 + // exceptions
            exc_len + 2 + // attributes
            attrs_len, max_stack, max_locals, byte_code, c_exc, code_attrs, cp.getConstantPool());
            addAttribute(code);
        }
                ...
                if ((next != null) && (ih.getInstruction() instanceof NOP)) {
                    try {
                        ...
                        for (int i = 0; i < targets.length; i++) {
                            ...
                            for (int j = 0; j < targeters.length; j++) targeters[j].updateTarget(targets[i], next);
                        }
                    }
                }
            }
        ...
        while (ih != null) {
            ...
            if (instruction instanceof BranchInstruction) {
                ...
                if (instruction instanceof Select) {
                    ...
                    for (int i = 0; i < targets.length; i++) branchTargets.push(targets[i], stackDepth);
   ...
   */
    public void update() {
        if (observers != null)
            for (Iterator e = observers.iterator(); e.hasNext(); ) ((MethodObserver) e.next()).notify(this);
    }
   ...
   */
    public final String toString() {
        ...
        if (throws_vec.size() > 0) {
            for (Iterator e = throws_vec.iterator(); e.hasNext(); ) buf.append("\n\t\tthrows " + e.next());
        }
        return buf.toString();
    }
   ...
   */
    ...
    private List makeMutableVersion(/*AnnotationGen*/
    Annotation[] mutableArray) {
