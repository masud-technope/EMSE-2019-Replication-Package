...
public class SplitIndexWriter extends com.sun.tools.doclets.standard.SplitIndexWriter {
    ...
    public  SplitIndexWriter(String path, String filename, String relpath, IndexBuilder indexbuilder, int prev, int next) throws IOException {
        super(path, filename, relpath, indexbuilder, prev, next);
    }
    ...
    public static void generate(IndexBuilder indexbuilder) throws DocletAbortException {
        ...
        try {
            for (int i = 0; i < indexbuilder.elements().length; i++) {
                ...
                (sw = new SplitIndexWriter(path, filename, relpath, indexbuilder, prev, next)).generateIndexFile((Character) indexbuilder.elements()[i]);
            }
        } catch (IOException e) {
            Standard.configuration().standardmessage.error("doclet.exception_encountered", e + "", filename);
