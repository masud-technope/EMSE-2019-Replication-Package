...
public class IfPointcut2Test extends TestCase {
    ...
    public void testSome() {
        ...
        f.doo(1);
        f.dooMulti();
        assertEquals("test aop test2-doo-doo aop2 doo test3-1-doo-doo-doo aop3 doo-1 testTWO-dooMulti testONE-dooMulti aop doMulti ", s_log.toString());
        s_log = new StringBuffer();
        IfAspect.ISON = false;
        f.doo();
        f.doo(1);
        f.dooMulti();
        assertEquals("test test2-doo-doo doo test3-1-doo-doo-doo doo-1 testTWO-dooMulti doMulti ", s_log.toString());
    }
    ...
    public static class IfAspect {
        ...
        @Pointcut("execution(* ataspectj.IfPointcut2Test.Foo.doo()) && if()")
        public static boolean test2(JoinPoint.StaticPart sjp, JoinPoint.StaticPart sjp2) {
            // will require JP flags
        ...
        public static boolean test3(JoinPoint jp, int val, JoinPoint jpbis, JoinPoint.StaticPart jps) {
            log("test3-" + val + "-" + jp.getSignature().getName() + "-" + jpbis.getSignature().getName() + "-" + jps.getSignature().getName());
            return ISON;
        }
