...
public class JavaRun implements IAjcRun {
    ...
    public boolean run(IRunStatus status) throws IllegalAccessException, InvocationTargetException, ClassNotFoundException, NoSuchMethodException {
        ...
        try {
            ...
            completedNormally = (spec.forkSpec.fork) ? runInOtherVM(status, libs, dirs) : runInSameVM(status, libs, dirs);
    ...
    protected boolean runInSameVM(IRunStatus status, File[] libs, File[] dirs) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        ...
        try {
            ...
            boolean snoopFailure = ((null != errSnoop) && 0 < errSnoop.size()) || ((null != outSnoop) && 0 < outSnoop.size());
    ...
    protected boolean runInOtherVM(IRunStatus status, File[] libs, File[] dirs) {
        ...
        final String classpath;
        {
            ...
            if (cpLength != spLength) {
                throw new Error("unable to find " + Arrays.asList(spec.classpath) + " got " + Arrays.asList(classpathFiles));
            }
        ...
        LangUtil.ProcessController controller = new LangUtil.ProcessController() {
            ...
            protected void doCompleting(Thrown ex, int result) {
                ...
                // handle errors
                String context = spec.className + " command \"" + commandLabel + "\"";
    ...
    protected boolean expectedException(ByteArrayOutputStream bout) {
        return ((null != spec.expectedException) && (-1 != bout.toString().indexOf(spec.expectedException)));
    }
        ...
        try {
            if (!spec.skipTester) {
                ...
                if (!baseDirSet.equals(baseDir)) {
                    String l = "AjcScript.setupTester() setting " + baseDir + " returned " + baseDirSet;
    ...
    public static class Spec extends AbstractRunSpec {
         ...
         */
        private static final XMLNames NAMES = new XMLNames(XMLNames.DEFAULT, "", "", null, null, "", null, "", "", false, false, true);
