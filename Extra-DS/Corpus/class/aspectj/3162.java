...
public class DirChangesTest extends TestCase {
    ...
    public void skip_testWriteExpDir() {
        ...
        String expected = "<dir-changes expDir=\"expected directory\"/>" + LangUtil.EOL;
        checkWrite(spec, expected);
    }
    ...
    public void skip_testWriteAdded() {
        ...
        String expected = "<dir-changes added=\"one,two,three\"/>" + LangUtil.EOL;
        checkWrite(spec, expected);
    }
    ...
    private void checkDirChanges(DirChanges.Spec spec, File baseDir, File srcBaseDir, Runnable dirChanger, IMessage[] expected) {
        ...
        try {
            ...
            if (0 < handler.numMessages(null, true)) {
                ...
                try {
                    ...
                    System.out.println("XXX writing to " + output + " messages " + LangUtil.arrayAsList(toprint));
                    new MessageListXmlReader().writeMessages(output, toprint);
                } catch (IOException e) {
                    System.err.println("Error writing to " + output + " messages " + LangUtil.arrayAsList(toprint));
                    e.printStackTrace(System.err);
                }
            }
        }
	 ...
	 * @param handler
	 */
    private void assertSameMessages(IMessage[] expected, MessageHandler handler) {
        ...
        {
            IMessage[] expNotFound = (IMessage[]) LangUtil.safeCopy(expected, new IMessage[0]);
        ...
        {
            IMessage[] actFound = (IMessage[]) LangUtil.safeCopy(actual, new IMessage[0]);
    ...
    private int find(IMessage message, IMessage[] expected) {
        if ((null != expected) && (0 != expected.length) && (null != message)) {
            final IMessage.Kind kind = message.getKind();
            final String text = message.getMessage();
            for (int i = 0; i < expected.length; i++) {
                IMessage exp = expected[i];
                if (null != exp) {
                    if (kind.equals(exp.getKind()) && text.equals(exp.getMessage())) {
                        return i;
                    }
                }
            }
