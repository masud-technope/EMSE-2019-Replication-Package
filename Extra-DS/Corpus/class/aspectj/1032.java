...
public class Ajc2 extends Javac {
     ...
     */
    public void prepare() {
        if (argfiles != null && !haveIncludes && !haveExcludes && getSrcdir() == null) {
            useDefaultSrcdir();
        }
    }
    ...
    protected void scanDir(File srcDir, File destDir, String files[]) {
        ...
        } else // Otherwise we want to add all .java files to the compileList
        {
            for (int i = 0; i < files.length; i++) {
                File newFile = new File(srcDir, files[i]);
                if (newFile != null && newFile.exists() && newFile.getName().endsWith(".java")) {
        ...
        if (newFileSize > 0) {
            ...
            System.arraycopy(newFiles.toArray(), 0, newCompileList, compileList.length, newFileSize);
    ...
    private void expandArgfile(File argfile, List includes, List arguments) {
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (line.startsWith("-")) {
                    arguments.add(line);
                } else // If there are stars we'll try to resolve the file here
                if (line.indexOf("*") != -1) {
                    log("The argfile line '" + line + "' is invalid", Project.MSG_WARN);
                } else // Another argfile
                if (line.startsWith("@")) {
                ...
                } else // Source file
                {
                    ...
                    if (newfile != null && newfile.exists() && !newfile.isDirectory() && newfile.getName().endsWith(".java")) {
