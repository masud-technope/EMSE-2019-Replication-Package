...
public class PackageWriter extends com.sun.tools.doclets.standard.PackageWriter {
    ...
    public static void generate(PackageDoc pkg, PackageDoc prev, PackageDoc next) throws DocletAbortException {
        ...
        try {
            (pw = new PackageWriter(path, filename, pkg, prev, next)).generatePackageFile();
            pw.close();
    ...
    protected void generateClassListing() {
        generateClassKindListing(packagedoc.interfaces(), getText("doclet.Interface_Summary"));
        generateClassKindListing(Statics.classes(packagedoc.ordinaryClasses()), getText("doclet.Class_Summary"));
        generateClassKindListing(((org.aspectj.ajdoc.PackageDoc) packagedoc).aspects(), getText("doclet.Aspect_Summary"));
        generateClassKindListing(packagedoc.exceptions(), getText("doclet.Exception_Summary"));
        generateClassKindListing(packagedoc.errors(), getText("doclet.Error_Summary"));
    }
}
