...
public class CompilerRunTest extends TestCase {
    ...
    void checkCommandLine(File testBaseDir, CompilerRun.Spec spec, String[] globals, String expectedInCommand) {
        ...
        assertTrue(0 == dummyReports.size());
        if ((null == command) || (-1 == command.indexOf(expectedInCommand))) {
            assertTrue("expected " + expectedInCommand + "got " + command, false);
        }
    }

    /** run with dummy compiler */
    boolean checkCompilerRun(File testBaseDir, CompilerRun.Spec spec, String[] globals, MessageHandler handler) {
        ...
        if (!adopted) {
            String s = "not adopted spec=" + spec + " globals=" + (LangUtil.isEmpty(globals) ? "[]" : Arrays.asList(globals).toString()) + " -- " + handler;
            assertTrue(s, false);
        }
