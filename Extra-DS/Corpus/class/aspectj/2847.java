...
public class NullIdeManager {
    ...
    public void init(String testProjectPath) {
        try {
            ...
            AjdeUIManager.getDefault().init(ajdeEditor, taskListManager, projectProperties, preferencesAdapter, uiAdapter, new IconRegistry(), nullFrame, new NullIdeProgressMonitor(), new NullIdeErrorHandler(), true);
            initialized = true;
        } catch (Throwable t) {
            initialized = false;
            t.printStackTrace();
            Ajde.getDefault().getErrorHandler().handleError("Null IDE failed to initialize.", t);
        }
    }
