...
public class AjcTaskCompileCommandTest extends TestCase {
    ...
    static boolean doWait(IMessageHolder holder, int seconds, int timeout) {
        return AjcTaskCompileCommand.waitUntilMessagesQuiet(holder, seconds, timeout);
    }
    ...
    void runTest(ArrayList args, int expectedErrors) {
        AjcTaskCompileCommand command = new AjcTaskCompileCommand();
    ...
    void checkWait(final int seconds, final int timeout, int toAdd, int addInterval) {
        ...
        if (0 == toAdd) {
            ...
            final MessageAdder adder = new MessageAdder(mhandler, toAdd, addInterval);
            ...
            Thread testThread = new Thread(new Runnable() {
                ...
                public void run() {
                    waitResult.addedThread = new Thread(adder, label + "-child");
                    waitResult.addedThread.start();
                    waitResult.result = AjcTaskCompileCommandTest.doWait(mhandler, seconds, timeout);
                }
            }, label);
            testThread.start();
            try {
...
class MessageAdder implements Runnable {
    ...
    public void run() {
        ...
        int numAdded = 0;
        while (!stop && (timeout > curTime) && (numAdded < numToAdd)) {
            long targetTime = curTime + waitBetweenAdds;
            while (!stop && (curTime < timeout) && (curTime < targetTime)) {
                try {
    ...
    public String toString() {
        return "MessageAdder(" + "numAdded=" + numAdded + ", numToAdd=" + numToAdd + ", interval=" + interval + ", stop=" + stop + ", wait=" + wait + ", numMessages=" + (null == messages ? 0 : messages.numMessages(null, true)) + ")";
    }
}
