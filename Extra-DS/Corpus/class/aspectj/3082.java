...
public class Module {
    ...
    private static ArrayList findJarsBySuffix(String suffix, Kind kind, List libJars, List required) {
        ArrayList result = new ArrayList();
        if (null != suffix) {
            // library jars
    ...
    public String toLongString() {
        return "Module [name=" + name + ", srcDirs=" + srcDirs + ", required=" + requiredModules + ", moduleJar=" + moduleJar + ", libJars=" + libJars + "]";
    }

    public Result getResult(Kind kind) {
        return kind.assemble ? (kind.normal ? releaseAll : testAll) : (kind.normal ? release : test);
    }
    ...
    private boolean init() {
        return initClasspath() && initProperties() && reviewInit() && initResults();
    }
    ...
    private boolean initClasspath() {
        ...
        // XXXFileLiteral
        File file = new File(moduleDir, ".classpath");
        FileReader fin = null;
        try {
    ...
    private boolean initProperties() {
        // XXXFileLiteral
        File file = new File(moduleDir, name + ".properties");
    ...
    protected boolean reviewInit() {
        try {
            for (ListIterator iter = srcDirs.listIterator(); iter.hasNext(); ) {
                ...
                if (!Util.JAVA5_VM && (Util.Constants.JAVA5_SRC.equals(lcname) || Util.Constants.JAVA5_TESTSRC.equals(lcname))) {
