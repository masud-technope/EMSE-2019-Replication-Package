 ...
 */
    ...
    private void dolook() {
        ...
        if (className == null || className.length() < 1) {
            ...
            //TODO: Find class somewhere else
                ...
                if (memberName != null && memberName.indexOf('(') == -1) {
        ...
        if (classDoc != null) {
            ...
            // a package name
            if (classDoc == null && className != null && className.indexOf('.') == -1) {
                packageDoc = PackageDocImpl.getPackageDoc(className);
            } else // referenced package to that class's contained package
            if (classDoc != null) {
                packageDoc = classDoc.containingPackage();
            } else // then set the referenced package to the current doc
            ...
            // package elsewhere
            packageDoc = PackageDocImpl.getPackageDoc(packageName != null ? packageName : "");
        }
    ...
    private void lookForMember(String spec, org.aspectj.ajdoc.ClassDoc container) {
        ...
        if (container instanceof AspectDoc) {
            if ((memberDoc = adviceDoc(name, (AspectDoc) container, paramNames)) != null) {
                return;
            }
        }
    }
    ...
    private String[] paramNames(String restNoParens) {
        ...
        for (StringTokenizer t = new StringTokenizer(restNoParens, ",", false); t.hasMoreTokens(); ) {
    ...
    private PointcutDoc pointcutDoc(String name, org.aspectj.ajdoc.ClassDoc container, String[] paramNames) {
        // XXX warn or error                                           
        if (null == name)
            return null;
        for (org.aspectj.ajdoc.ClassDoc cd = container; cd != null; cd = (org.aspectj.ajdoc.ClassDoc) cd.superclass()) {
            ...
            for (int i = 0, N = docs.length; i < N; i++) {
                ...
                if ((null != md) && (name.equals(md.name())) && ((null == paramNames) || parametersMatch(md.parameters(), paramNames, container))) {
                    return md;
                }
            }
        }
    ...
    private MethodDoc methodDoc(String name, ClassDoc container, String[] paramNames) {
        for (ClassDoc cd = container; cd != null; cd = cd.superclass()) {
            ...
            for (int i = 0, N = docs.length; i < N; i++) {
                ...
                if (paramNames == null) {
                    ...
                    if (parametersMatch(md.parameters(), paramNames, container)) {
                        return md;
                    }
                }
            }
    ...
    private ConstructorDoc constructorDoc(ClassDoc container, String[] paramNames) {
        for (ClassDoc cd = container; cd != null; cd = cd.superclass()) {
    ...
    private boolean parametersMatch(Parameter[] params, String[] paramNames, ClassDoc container) {
        ...
        for (int i = 0, N = params.length; i < N; i++) {
            ...
            com.sun.javadoc.Type type2 = TypeImpl.getInstance(paramNames[i], container);
     ...
     */
    private static int findNextChar(final String s, final int start, final char x) {
        // XXX to Util
        if ((null != s) && (start >= 0)) {
        ...
        int i;
            ...
            switch(state) {
                case 0:
                    ...
                    break;
                case // reading initial [type|memberName] 
                1:
                    if (ident(c)) {
                        next = 1;
                    } else if (c == '#') {
                    ...
                    break;
                case // start reading membername (field only?)
                2:
                    sharp = true;
                    if (ident(c)) {
                        next = 4;
                    ...
                    break;
                case // reading qualified type name
                3:
                    if (ident(c)) {
                        next = 1;
                    } else {
                ...
                6:
                    ...
                    break;
                case // reading qualified parameter type .
                7:
                    if (ident(c)) {
                        next = 6;
                    } else {
                ...
                8:
                    ...
                    break;
                case // maybe completed parameter type 
                9:
                    if (c == '[') {
                        next = 8;
                    } else if (space(c)) {
                ...
                12:
                    ...
                    break;
                case // seeking parms or label
                13:
                    if (space(c)) {
                        next = 13;
                    } else if (// start reading parms
                    c == '(') // start reading parms
                    {
                        next = 5;
                    } else if (ident(c)) {
                        // start reading label
                ...
                16:
                    ...
                    break;
                case // reading label - may have internal spaces
                17:
                    if (ident(c)) {
                        next = 17;
                    } else if (space(c)) {
                        next = 17;
                    } else // XXX known limitation - labels may only be ident + whitespace (no -)
                    {
        ...
        if (sharp) {
            ...
            if (-1 != parmLoc) {
                ...
                if ((-1 != spaceLoc) && (spaceLoc < parmLoc)) {
                    memberName = memberName.substring(0, spaceLoc) + memberName.substring(parmLoc).trim();
                }
            }
        }
        if (!finished) {
