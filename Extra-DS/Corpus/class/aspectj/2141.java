...
public final class LazyMethodGen {
            ...
            if (targeters != null) {
                for (int i = targeters.length - 1; i >= 0; i--) {
                    ...
                    if (targeter instanceof LocalVariableGen) {
                        ...
                        LocalVariableTag lr = new LocalVariableTag(lng.getType().getSignature(), /*BcelWorld.fromBcel(lng.getType())*/
                        lng.getName(), lng.getIndex(), lng.getStart().getPosition());
                        if (lng.getStart() == ih) {
                            locals.add(lr);
                        } else {
                            ends.add(lr);
    ...
    public void addExceptionHandler(InstructionHandle start, InstructionHandle end, InstructionHandle handlerStart, ObjectType catchType, boolean highPriority) {
        ...
        ExceptionRange er = new ExceptionRange(body, BcelWorld.fromBcel(catchType), highPriority);
    ...
    public MethodGen pack() {
        //killNops();
        MethodGen gen = new MethodGen(getAccessFlags(), getReturnType(), getArgumentTypes(), //getArgumentNames(),
        null, getName(), getEnclosingClass().getName(), new InstructionList(), getEnclosingClass().getConstantPoolGen());
    ...
    private void addLocalVariables(MethodGen gen, Map localVariables) {
        ...
        for (Iterator iter = localVariables.keySet().iterator(); iter.hasNext(); ) {
            ...
            gen.addLocalVariable(tag.getName(), t, tag.getSlot(), (InstructionHandle) start, (InstructionHandle) lvpos.end);
        }
    }
    ...
    public void assertGoodBody() {
        ...
        if (true)
            return;
        //definingType.getNameAsIdentifier() + "." + getName()); //toString());
