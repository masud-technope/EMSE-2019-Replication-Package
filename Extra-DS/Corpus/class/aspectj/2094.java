...
public class BridgeUtil {
    ...
    public static IMessage makeMessage(String message, IMessage.Kind kind, Throwable thrown, LineReader reader) {
    ...
    public static String writeMessage(IMessage message) {
        ...
        return message.getKind() + KIND_DELIM + message.getMessage() + MESSAGE_DELIM + // XXX implement
    ...
    public static class Comparators {
        ...
        static int compareNull(Object one, Object two) {
            return (null == one ? (null == two ? 0 : 1) : (null == two ? -1 : 0));
        }
        ...
        static int compareStringsSoftly(String lhs_s, String rhs_s) {
            if (LangUtil.isEmpty(lhs_s) || LangUtil.isEmpty(rhs_s)) {
                return 0;
            }
            if ((-1 != lhs_s.indexOf(rhs_s)) || (-1 != rhs_s.indexOf(lhs_s))) {
                return 0;
            }
        ...
        public static final Comparator WEAK_File = new Comparator() {
            ...
            public int compare(Object o1, Object o2) {
                if ((o1 == o2) || (o1 == ISourceLocation.NO_FILE) || (o2 == ISourceLocation.NO_FILE)) {
                    return 0;
                }
    ...
    public static SourceLocation makeSourceLocation(String input) {
    ...
    public static SourceLocation makeSourceLocation(String input, File defaultFile) {
        ...
        final int colon1 = input.indexOf(":", 2);
        ...
        if (-1 == colon1) {
            ...
            // 1+ colon => file:line // XXX later or line:column
            ...
            //throw new IllegalArgumentException(line);
                ...
                //throw new IllegalArgumentException(col);
                    ...
                    // 3 colon => message
                    // do not trim message
                    input.substring(colon3 + 1);
                }
            }
        }
        if (path.startsWith(fixTag)) {
            int len = fixTag.length();
            path = path.substring(len, 1 + len) + ":" + path.substring(1 + len);
        }
        if ((endLine == 0) && (line != 0)) {
            endLine = line;
        }
     ...
     */
    private static int convert(String s, boolean permitText, boolean first) {
        int result = -1;
        int last = -1;
        int max = s.length();
