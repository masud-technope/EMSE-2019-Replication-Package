...
public class TreeStructureViewBuilder {
    ...
    public void buildView(StructureView view, IHierarchy model) {
        ...
        if (!isFileView(view)) {
            StructureViewProperties.Hierarchy hierarchy = ((GlobalStructureView) view).getGlobalViewProperties().getHierarchy();
        ...
        if (viewRoot == null) {
            //modelRoot;
            viewRoot = createViewNode(modelRoot, view.getViewProperties());
        }
        if (view.getViewProperties().getSorting() == StructureViewProperties.Sorting.ALPHABETICAL || (!isFileView(view) && ((GlobalStructureView) view).getGlobalViewProperties().getHierarchy().equals(StructureViewProperties.Hierarchy.DECLARATION))) {
    ...
    private IStructureViewNode createViewNode(IProgramElement node, StructureViewProperties properties) {
        ...
        IStructureViewNode viewNode = nodeFactory.createNode(node, children);
        return viewNode;
    }
    ...
    private boolean acceptGranularity(IProgramElement.Kind kind, StructureViewProperties.Granularity granularity) {
        if (granularity == StructureViewProperties.Granularity.DECLARED_ELEMENTS) {
            return true;
        } else if (granularity == StructureViewProperties.Granularity.MEMBER && (kind != IProgramElement.Kind.CODE)) {
            return true;
        } else if (granularity == StructureViewProperties.Granularity.TYPE && (kind == IProgramElement.Kind.PROJECT || kind == IProgramElement.Kind.PACKAGE || kind.isSourceFile() || kind.isType())) {
            return true;
        } else if (granularity == StructureViewProperties.Granularity.FILE && (kind == IProgramElement.Kind.PROJECT || kind == IProgramElement.Kind.PACKAGE || kind.isSourceFile())) {
            return true;
        } else if (granularity == StructureViewProperties.Granularity.PACKAGE && (kind == IProgramElement.Kind.PROJECT || kind == IProgramElement.Kind.PACKAGE)) {
            return true;
        } else {
            return false;
        }
    ...
    private IStructureViewNode buildCustomTree(GlobalStructureView view, IHierarchy model) {
        ...
        for (Iterator it = rootNodes.iterator(); it.hasNext(); ) {
            if (view.getGlobalViewProperties().getHierarchy().equals(StructureViewProperties.Hierarchy.CROSSCUTTING)) {
                ...
                treeNode.add(getInheritanceChildren((IProgramElement) it.next(), view.getViewProperties().getRelations()));
            }
        }
        return treeNode;
    }
