...
public class COMObject {
    ...
    private static Callback[][] Callbacks = new Callback[MAX_VTABLE_LENGTH][MAX_ARG_COUNT];
    ...
    public  COMObject(int[] argCounts) {
        ...
        for (int i = 0, length = argCounts.length; i < length; i++) {
            if ((Callbacks[i][argCounts[i]]) == null) {
                //$NON-NLS-1$
                Callbacks[i][argCounts[i]] = new Callback(this.getClass(), "callback" + i, argCounts[i] + 1, true, COM.E_FAIL);
                if (Callbacks[i][argCounts[i]].getAddress() == 0)
