...
public class GenericSignatureParser {
    ...
    public MethodTypeSignature parseAsMethodSignature(String sig) {
        ...
        if (returnType == null)
            returnType = new Signature.BaseTypeSignature(eatIdentifier());
        // throws
    ...
    private FieldTypeSignature parseFieldTypeSignature(boolean isOptional) {
        if (isOptional) {
            ...
            if (!tokenStream[tokenIndex].startsWith("L") && !tokenStream[tokenIndex].startsWith("T") && !tokenStream[tokenIndex].startsWith("[")) {
                return null;
            }
        }
        if (maybeEat("[")) {
            ...
            throw new IllegalStateException("Expecting [,L, or T, but found " + tokenStream[tokenIndex] + " while unpacking " + inputString);
        }
    }
