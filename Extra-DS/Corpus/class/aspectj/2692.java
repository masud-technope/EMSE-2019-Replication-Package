...
public class ConcreteAspectCodeGen {
        ...
        while (methods.hasNext()) {
            ...
            if (method.isAbstract()) {
                if ("()V".equals(method.getSignature())) {
                    ...
                    if (// Allow for the abstract pointcut being from a code style aspect compiled with -1.5 (see test for 128744)
                    n.startsWith("ajc$pointcut")) {
                        n = n.substring(14);
    ...
    public byte[] getBytes() {
        ...
        // }
        // @Aspect public class ...
        LazyClassGen cg = new LazyClassGen(m_concreteAspect.name.replace('.', '/'), (m_parent == null) ? "java/lang/Object" : m_parent.getName().replace('.', '/'), //TODO AV - we could point to the aop.xml that defines it and use JSR-45
        null, Modifier.PUBLIC + Constants.ACC_SUPER, EMPTY_STRINGS, m_world);
        ...
        if (m_concreteAspect.precedence != null) {
            SimpleElementValueGen svg = new SimpleElementValueGen(ElementValueGen.STRING, cg.getConstantPoolGen(), m_concreteAspect.precedence);
        ...
        cbody.append(cg.getFactory().createInvoke((m_parent == null) ? "java/lang/Object" : m_parent.getName().replace('.', '/'), "<init>", Type.VOID, EMPTY_TYPES, Constants.INVOKESPECIAL));
        cbody.append(InstructionConstants.RETURN);
        cg.addMethodGen(init);
        for (Iterator it = m_concreteAspect.pointcuts.iterator(); it.hasNext(); ) {
            Definition.Pointcut abstractPc = (Definition.Pointcut) it.next();
            LazyMethodGen mg = new LazyMethodGen(//TODO AV - respect visibility instead of opening up?
            Modifier.PUBLIC, Type.VOID, abstractPc.name, EMPTY_TYPES, EMPTY_STRINGS, cg);
            SimpleElementValueGen svg = new SimpleElementValueGen(ElementValueGen.STRING, cg.getConstantPoolGen(), abstractPc.expression);
            List elems = new ArrayList();
            elems.add(new ElementNameValuePairGen("value", svg, cg.getConstantPoolGen()));
        ...
        BcelPerClauseAspectAdder perClauseMunger = new BcelPerClauseAspectAdder(ResolvedType.forName(m_concreteAspect.name).resolve(m_world), m_perClause.getKind());
