...
public class ExtensionTests extends AjcTestCase {
    ...
    public void testMessageID() {
        ...
        assertTrue("Expected ID of message to be " + IProblem.UnusedImport + " (UnusedImport) but found an ID of " + m.getID(), m.getID() == IProblem.UnusedImport);
    }
  ...
  InnerClasses.java
    ...
    public void walkit(IProgramElement ipe, int indent) {
        if (ipe != null) {
            ...
            //getName());
            if (debugTests)
                System.out.println(ipe.toLabelString());
            if (ipe.getChildren() != null) {
                List kids = ipe.getChildren();
                for (Iterator iter = kids.iterator(); iter.hasNext(); ) {
                    IProgramElement element = (IProgramElement) iter.next();
