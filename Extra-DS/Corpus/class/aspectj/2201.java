...
public class Helper {
    ...
    public void checkSignature(Declaration dec, String sig, String fsig) {
        ...
        if (dsig != null) {
            dsig = dsig.trim();
            if (dsig.startsWith("/*") && dsig.endsWith("*/"))
                dsig = "";
        }
        if (dfsig != null) {
            dfsig = dfsig.trim();
            if (dfsig.startsWith("/*") && dfsig.endsWith("*/"))
                dfsig = "";
        }
    ...
    public void checkPointedToBy(Declaration dec, Declaration[] expected) {
        ...
        for (i = 0; i < expected.length; i++) {
            ...
            for (j = 0; j < pointed.length; j++) {
                if (pointed[j].equals(expected[i]))
                    break;
            }
            if (j >= pointed.length)
        ...
        for (i = 0; i < pointed.length; i++) {
            ...
            for (j = 0; j < expected.length; j++) {
                if (pointed[i].equals(expected[j]))
                    break;
            }
            if (j >= expected.length)
    ...
    public void checkModifiers(Declaration dec, String expected_modifiers) {
        ...
        for (i = 0; i < expected.length; i++) {
            for (j = 0; j < modifiers.length; j++) {
                if (modifiers[j].equals(expected[i]))
                    break;
            }
        ...
        for (i = 0; i < modifiers.length; i++) {
            for (j = 0; j < expected.length; j++) {
                if (modifiers[i].equals(expected[j]))
                    break;
            }
            if (j >= expected.length)
    ...
    private static String getFilePos(Declaration dec) {
        ...
        if (longFileName == null)
            return "?.java:" + dec.getBeginLine();
        int pos = longFileName.lastIndexOf('/');
        if (pos < 0)
            pos = longFileName.lastIndexOf('\\');
        if (pos < 0)
            return longFileName + ":" + dec.getBeginLine();
