...
public class AsmDelegate extends AbstractReferenceTypeDelegate {
    ...
    // filled in when signature is parsed
    ...
    Collection /*of Declare*/
    declares = null;

    Collection /*of ConcreteTypeMunger*/
    typeMungers = null;

    Collection /*of ResolvedMember*/
    privilegedAccesses = null;

    // see below for the relevant bits
    private int bitflag = 0;

    // guard for lazy initialization of pointcuts
    private static final int DISCOVERED_POINTCUTS = 0x0001;

    // guard for lazy initialization of declares
    private static final int DISCOVERED_DECLARES = 0x0002;

    // guard for lazy initialization of type mungers
    private static final int DISCOVERED_TYPEMUNGERS = 0x0004;

    // guard for lazy initialization of privileged access list
    private static final int DISCOVERED_PRIVILEGEDACCESSES = 0x0008;

    // Sourcecontext is actually held in supertype
    private static final int DISCOVERED_SOURCECONTEXT = 0x0010;

    private static final int DISCOVERED_WEAVERSTATEINFO = 0x0020;

    private static final int SIGNATURE_UNPACKED = 0x0040;

    private static final int ANNOTATION_TYPES_CORRECT = 0x0080;

    private static final int ANNOTATIONX_CORRECT = 0x0100;

    private static final int SUPERSET = 0x0200;

    private static final int FIELDSFIXEDUP = 0x0400;

    private static final int METHODSFIXEDUP = 0x0800;
    ...
    public  AsmDelegate(ReferenceType rt, InputStream inputStream) {
        ...
        try {
            ...
            inputStream.close();
            // remove SYNTHETIC
            if ((classModifiers & 4096) > 0)
                classModifiers -= 4096;
            // remove DEPRECATED
            if ((classModifiers & 131072) > 0)
                classModifiers -= 131072;
    ...
    private void ensureAnnotationXsUnpacked() {
        if ((bitflag & ANNOTATION_TYPES_CORRECT) != 0 && (bitflag & ANNOTATIONX_CORRECT) != 0)
            return;
        ensureAnnotationsUnpacked();
    ...
    private void ensureAnnotationsUnpacked() {
        if ((bitflag & ANNOTATION_TYPES_CORRECT) != 0)
            return;
        if (annotations.size() == 0) {
    ...
    private void ensureSignatureUnpacked() {
        if ((bitflag & SIGNATURE_UNPACKED) != 0)
            return;
        ...
        if (declaredSignature != null) {
            ...
            for (int i = 0; i < typeVariables.length; i++) {
                ...
                try {
                    typeVariables[i] = BcelGenericSignatureToTypeXConverter.formalTypeParameter2TypeVariable(ftp, cSig.formalTypeParameters, getResolvedTypeX().getWorld());
            ...
            if (cSig != null) {
                ...
                try {
                    this.superclassType = BcelGenericSignatureToTypeXConverter.classTypeSignature2TypeX(superSig, formalsForResolution, getResolvedTypeX().getWorld());
                    bitflag |= SUPERSET;
                } catch (GenericSignatureFormatException e) {
                    throw new IllegalStateException("While determing the generic superclass of " + getResolvedTypeX() + " with generic signature " + declaredSignature + " the following error was detected: " + e.getMessage());
                }
                ...
                for (int i = 0; i < cSig.superInterfaceSignatures.length; i++) {
                    try {
                        this.interfaceTypes[i] = BcelGenericSignatureToTypeXConverter.classTypeSignature2TypeX(cSig.superInterfaceSignatures[i], formalsForResolution, getResolvedTypeX().getWorld());
                    } catch (GenericSignatureFormatException e) {
                        throw new IllegalStateException("While determing the generic superinterfaces of " + getResolvedTypeX() + " with generic signature " + declaredSignature + " the following error was detected: " + e.getMessage());
                    }
                }
                if (isGeneric()) {
