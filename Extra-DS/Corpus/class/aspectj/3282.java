    ...
    public void setJavac(Javac javac) {
        ...
        if (null == task) {
            ...
            if (name.equals(Ajc10.class.getName())) {
                err = err + "second class loader - put ajc taskdef library " + "only on system classpath via ant/lib";
            } else {
                err = err + name;
            }
        }
        ...
        // XXX unsupported
        tempAjc.setDeprecation(javac.getDeprecation());
        tempAjc.setEncoding(javac.getEncoding());
        // XXX unsupported
        tempAjc.setDebug(javac.getDebug());
        // XXX unsupported
        tempAjc.setOptimize(javac.getOptimize());
        // XXX unsupported
        tempAjc.setDepend(javac.getDepend() ? "on" : "off");
        ...
        // XXX unsupported
        tempAjc.setIncludeantruntime(javac.getIncludeantruntime());
