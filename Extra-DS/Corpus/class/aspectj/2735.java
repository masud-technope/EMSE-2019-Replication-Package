...
public class AJDBTest implements StopListener, VMListener {
    ...
    protected String[] stringCommands() {
        return new String[] { "workingdir " + Tester.workingDir(), "use " + "./new", "stop on AJDBClass.java:24", "run -classpath " + classpath + " AJDBClass" };
    }
    ...
    protected void step(StepEvent e) {
        ...
        try {
            ...
            if (loc.sourceName().equals("Thread.java") && loc.method().name().equals("exit")) {
                isRunning = false;
            }
        } catch (Throwable t) {
        }
    ...
    static final String FILENAME = "script.txt";
