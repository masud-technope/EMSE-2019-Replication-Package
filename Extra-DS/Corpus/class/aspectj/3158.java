...
public class Runner {
     ...
     */
    public boolean run(IRun run, IRunStatus status, IRunListener listener) {
        return run(run, status, listener, (Class) null);
    }

    public boolean run(IRun run, IRunStatus status, IRunListener listener, Class exceptionClass) {
     ...
     */
    public boolean runChild(IRun child, IRunStatus parentStatus, IRunStatus childStatus, IRunListener listener) {
     ...
     */
    public boolean runChild(IRun child, IRunStatus parentStatus, IRunStatus childStatus, IRunListener listener, Class exceptionClass) {
        ...
        if (null != listeners) {
            try {
                ...
                parentStatus.handleMessage(MessageUtil.abort(m, t));
            }
        }
     ...
     * @throws IllegalArgumentException if status is null
     */
    public boolean runIterator(IRunIterator iterator, IRunStatus status, IRunListener listener) {
    ...
    private boolean abortOnFailure(IRunIterator iterator, IRun run) {
        // XxX not complete
        return ((null == iterator) || iterator.abortOnFailure());
    }
     ...
     */
    public void registerFastFailIterator(IRunIterator iterator) {
        // XXX unimplemented
     ...
     * @see IRunIterator#abortOnFailure()
     */
    public void registerFastFailIteratorTypes(Class iteratorType) {
        // XXX unimplemented
     ...
     */
    public void registerListener(Class type, IRunListener listener) {
        // XXX unregister
        if (null == type) {
    ...
    private boolean runPrivate(IRun run, IRunStatus status, RunListeners listeners, Class exceptionClass) {
            ...
            try {
                ...
                while (iterator.hasNextRun()) {
                    ...
                    if (newMessages > numMessages) {
                        ...
                        String m = "run " + i + " from " + iterator + " not invoked, due to fail(+) message(s) ";
                        MessageUtil.debug(status, m);
                        continue;
                    }
                    // let runChild create
                    RunStatus childStatus = null;
                    if (!runner.runChild(run, status, childStatus, listener)) {
                        if (result) {
                            result = false;
                        }
                        if (iterator.abortOnFailure() || runner.abortOnFailure(iterator, run)) {
                            break;
                        }
                    }
                    i++;
    ...
    static class ClassListeners extends RunListeners {
        ...
        static RunListeners getListeners(Class c) {
