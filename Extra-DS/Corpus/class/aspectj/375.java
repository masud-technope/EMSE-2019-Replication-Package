...
public class LinkCheck {
  ...
  */
    ...
    public static void main(String[] args) {
        final String syntax = "java " + LinkCheck.class.getName() + " file {-log <file> | -printInfo}..";
        ...
        Link.Check exists = Link.getProtocolChecker(new String[] { "file", "http" });
        Link.Check contents = Link.getProtocolChecker(new String[] { "file" });
        LinkCheck me = new LinkCheck(mh, exists, contents);
        me.addLinkToCheck(COMMAND_LINE, startingURL);
        try {
            ...
            System.exit(mh.numMessages(IMessage.ERROR, true));
        }
    }
    ...
    public synchronized void addLinkToCheck(URL doc, String link) {
        ...
        if ((null != link) && !checkedUrls.contains(linkString)) {
            if (!checkExists.check(linkURL)) {
                ...
                if (!linksToCheck.contains(toAdd)) {
    ...
    private void checkLink(final Link link) {
        ...
        try {
            ...
            if (null == type) {
                ...
                parser.parse(new InputStreamReader(input), new LinkListener(url, addingLinks), true);
            }
        } catch (IOException e) {
    ...
    private boolean handleAsRef(Link link) {
        ...
        if (!LangUtil.isEmpty(ref)) {
            String refString = link.url.toString();
    ...
    private class Messages {
        ...
        private void exceptionReading(Link link, IOException e) {
            ...
            if ((m != null) && (-1 != m.indexOf("protocol")) && (-1 != m.indexOf("https")) && "http".equals(link.url.getProtocol())) {
    ...
    private class LinkListener extends HTMLEditorKit.ParserCallback {
        ...
        public void handleSimpleTag(HTML.Tag tag, MutableAttributeSet attributes, int position) {
            ...
            Object key = HTML.Tag.FRAME == tag ? HTML.Attribute.SRC : HTML.Attribute.HREF;
        ...
        private boolean registerIfNamedAnchor(HTML.Tag tag, MutableAttributeSet attributes) {
    ...
    private static class Link {
        ...
        private static Check getProtocolChecker(String[] protocols) {
            final String[] input = (String[]) LangUtil.safeCopy(protocols, protocols);
            if (0 == input.length) {
