...
public class AsmHierarchyBuilder extends ASTVisitor {
    ...
    public boolean visit(TypeDeclaration typeDeclaration, CompilationUnitScope scope) {
        ...
        else if (typeDeclaration.kind() == IGenericType.INTERFACE_DECL)
            kind = IProgramElement.Kind.INTERFACE;
        else if (typeDeclaration.kind() == IGenericType.ENUM_DECL)
            kind = IProgramElement.Kind.ENUM;
        else if (typeDeclaration.kind() == IGenericType.ANNOTATION_TYPE_DECL)
    ...
    public boolean visit(TypeDeclaration memberTypeDeclaration, ClassScope scope) {
        ...
        if (memberTypeDeclaration instanceof AspectDeclaration)
            kind = IProgramElement.Kind.ASPECT;
        else if (memberTypeDeclaration.kind() == IGenericType.INTERFACE_DECL)
            kind = IProgramElement.Kind.INTERFACE;
        else if (memberTypeDeclaration.kind() == IGenericType.ENUM_DECL)
            kind = IProgramElement.Kind.ENUM;
        else if (memberTypeDeclaration.kind() == IGenericType.ANNOTATION_TYPE_DECL)
            kind = IProgramElement.Kind.ANNOTATION;
        //@AJ support
    ...
    public boolean visit(TypeDeclaration memberTypeDeclaration, BlockScope scope) {
        ...
        IProgramElement.Kind kind = IProgramElement.Kind.CLASS;
        if (memberTypeDeclaration.kind() == IGenericType.INTERFACE_DECL)
            kind = IProgramElement.Kind.INTERFACE;
        else if (memberTypeDeclaration.kind() == IGenericType.ENUM_DECL)
            kind = IProgramElement.Kind.ENUM;
        else if (memberTypeDeclaration.kind() == IGenericType.ANNOTATION_TYPE_DECL)
        ...
        IProgramElement peNode = new ProgramElement(fullName, kind, makeLocation(memberTypeDeclaration), memberTypeDeclaration.modifiers, null, null);
        peNode.setSourceSignature(genSourceSignature(memberTypeDeclaration));
        peNode.setFormalComment(generateJavadocComment(memberTypeDeclaration));
    ...
    public boolean visit(MethodDeclaration methodDeclaration, ClassScope scope) {
        ...
        if (methodDeclaration instanceof InterTypeDeclaration) {
            ...
            peNode = new ProgramElement(null, IProgramElement.Kind.ERROR, makeLocation(methodDeclaration), (sig != null ? sig.getModifiers() : 0), null, null);
        } else {
            peNode = new ProgramElement(null, IProgramElement.Kind.ERROR, makeLocation(methodDeclaration), methodDeclaration.modifiers, null, null);
        }
        // will set the name
        ...
        if (peNode.getKind().equals(IProgramElement.Kind.METHOD)) {
            if ((peNode.getName().charAt(0) == 'm') && (peNode.toLabelString().equals("main(String[])") || peNode.toLabelString().equals("main(java.lang.String[])")) && peNode.getModifiers().contains(IProgramElement.Modifiers.STATIC) && peNode.getAccessibility().equals(IProgramElement.Accessibility.PUBLIC)) {
    ...
    protected void genBytecodeInfo(MethodDeclaration methodDeclaration, IProgramElement peNode) {
        if (methodDeclaration.binding != null) {
            ...
            } catch (BCException // bad type name 
            bce) {
                bce.printStackTrace();
            } catch (NullPointerException npe) {
                npe.printStackTrace();
    ...
    public boolean visit(ImportReference importRef, CompilationUnitScope scope) {
        ...
        if (!((ProgramElement) stack.peek()).getPackageName().equals(currPackageImport)) {
            IProgramElement peNode = new ProgramElement(new String(importRef.toString()), IProgramElement.Kind.IMPORT_REFERENCE, makeLocation(importRef), 0, null, null);
    ...
    public boolean visit(FieldDeclaration fieldDeclaration, MethodScope scope) {
        IProgramElement peNode = null;
        if (// The field represents an enum value
        fieldDeclaration.type == null) {
            peNode = new ProgramElement(new String(fieldDeclaration.name), IProgramElement.Kind.ENUM_VALUE, makeLocation(fieldDeclaration), fieldDeclaration.modifiers, null, null);
            peNode.setCorrespondingType(fieldDeclaration.binding.type.debugName());
        } else {
            peNode = new ProgramElement(new String(fieldDeclaration.name), IProgramElement.Kind.FIELD, makeLocation(fieldDeclaration), fieldDeclaration.modifiers, null, null);
    ...
    protected String generateJavadocComment(ASTNode astNode) {
        ...
        currCompilationResult.compilationUnit.getContents()[startIndex] == '/' && currCompilationResult.compilationUnit.getContents()[startIndex + 1] == '*' && currCompilationResult.compilationUnit.getContents()[startIndex + 2] == '*') {
    ...
    protected String genSourceSignature(FieldDeclaration fieldDeclaration) {
        StringBuffer output = new StringBuffer();
        if (// This is an enum value
        fieldDeclaration.type == null) {
            // the "," or ";" has to be put on by whatever uses the sourceSignature
    ...
    public boolean visit(ConstructorDeclaration constructorDeclaration, ClassScope scope) {
        ...
        argumentsSignature.append(")");
        IProgramElement peNode = new ProgramElement(new String(constructorDeclaration.selector) + argumentsSignature, IProgramElement.Kind.CONSTRUCTOR, makeLocation(constructorDeclaration), constructorDeclaration.modifiers, null, null);
    ...
    public boolean visit(Initializer initializer, MethodScope scope) {
        ...
        IProgramElement peNode = new ProgramElement("...", IProgramElement.Kind.INITIALIZER, makeLocation(initializer), initializer.modifiers, null, null);
        //			"",
        //			new ArrayList());	
        ((IProgramElement) stack.peek()).addChild(peNode);
        stack.push(peNode);
    ...
    protected int getStartLine(ASTNode n) {
        ...
        return ProblemHandler.searchLineNumber(lineseps, //			currCompilationResult.lineSeparatorPositions,
        n.sourceStart);
    }
