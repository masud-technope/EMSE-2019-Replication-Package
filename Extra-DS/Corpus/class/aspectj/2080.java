...
public class PointcutDeclaration extends AjMethodDeclaration {
        ...
        if (Modifier.isAbstract(this.declaredModifiers)) {
            if (!(typeDec instanceof AspectDeclaration)) {
                ...
                if (isAtAspectJ(typeDec)) {
                    ...
                    typeDec.scope.problemReporter().signalError(sourceStart, sourceEnd, "The abstract pointcut " + new String(declaredName) + " can only be defined in an aspect");
                    ignoreFurtherInvestigation = true;
                    return;
                }
            } else if (!Modifier.isAbstract(typeDec.modifiers)) {
                typeDec.scope.problemReporter().signalError(sourceStart, sourceEnd, "The abstract pointcut " + new String(declaredName) + " can only be defined in an abstract aspect");
                ignoreFurtherInvestigation = true;
                return;
            }
        }
    ...
    public void addAtAspectJAnnotations() {
        ...
        Annotation pcutAnnotation = AtAspectJAnnotationFactory.createPointcutAnnotation(getPointcutText(), argNames, declarationSourceStart);
        ;
        if (annotations == null) {
    ...
    public ResolvedPointcutDefinition makeResolvedPointcutDefinition(EclipseFactory inWorld) {
        ...
        resolvedPointcutDeclaration = new ResolvedPointcutDefinition(inWorld.fromBinding(this.binding.declaringClass), declaredModifiers, declaredName, inWorld.fromBindings(this.binding.parameters), //??? might want to use null 
        getPointcut());
