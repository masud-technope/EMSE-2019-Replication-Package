...
public class Option implements Comparable {
    ...
    public static final Prefix FORCE_ON = new Prefix("!", "force-on", true, true);

    public static final Prefix FORCE_OFF = new Prefix("^", "force-off", false, true);

    public static final Prefixes LITERAL_PREFIXES = new Prefixes(new Prefix[] { NONE });

    public static final Prefixes STANDARD_PREFIXES = new Prefixes(new Prefix[] { ON });

    public static final Prefixes FORCE_PREFIXES = new Prefixes(new Prefix[] { ON, FORCE_ON, FORCE_OFF });
     ...
     */
    public  Option(String name, Family family, Prefixes permittedPrefixes, boolean acceptSuffixedInput, String[][] permittedArguments) {
    ...
    String validArgument(String input, int position) {
        ...
        return input + " not permitted, expecting one of " + Arrays.asList(permitted);
    }
    ...
    private Prefix hasPrefix(String value) {
        for (Iterator iter = permittedPrefixes.iterator(); iter.hasNext(); ) {
    ...
    public static class Family implements Comparable {
        ...
        protected  Family(String familyName, boolean permitMultipleFamilyValues) {
        ...
        public boolean sameFamily(Family family) {
            return ((null != family) && familyName.equals(family.familyName));
        }
        ...
        public final boolean collision(Option lhs, Option rhs) {
            ...
            if (this != lhsFamily) {
                String s = "expected family " + this + " got family " + lhsFamily;
    ...
    public static class Factory {
         ...
         */
        public boolean setupFamily(String name, boolean permitMultipleValues) {
            LangUtil.throwIaxIfNull(name, "name");
            Family family;
            synchronized (familyNameToFamily) {
                ...
                } else if (permitMultipleValues != family.permitMultipleFamilyValues) {
                    return false;
                }
            }
            return true;
        ...
        public String registerFamily(Family family) {
            ...
            synchronized (familyNameToFamily) {
                Family knownFamily = (Family) familyNameToFamily.get(family.familyName);
                if (null == knownFamily) {
                    familyNameToFamily.put(family.familyName, family);
                } else if (!knownFamily.equals(family)) {
                    return "different family registered, have " + knownFamily + " registering " + family;
                }
            }
            return null;
        }
        ...
        public Option create(String name, String family, Prefixes permittedPrefixes, boolean acceptSuffixedInput) {
            return create(name, family, permittedPrefixes, acceptSuffixedInput, (String[][]) null);
        }

        public Option create(String name, String family, Prefixes permittedPrefixes, boolean acceptSuffixedInput, String[][] permittedArguments) {
            LangUtil.throwIaxIfNull(name, "name");
            LangUtil.throwIaxIfNull(family, "family");
            LangUtil.throwIaxIfNull(permittedPrefixes, "permittedPrefixes");
            ...
            Option result = new Option(name, resolvedFamily, permittedPrefixes, acceptSuffixedInput, permittedArguments);
            synchronized (names) {
    ...
    public static class Value {
        ...
        private static String[] unflatten(Value value) {
            ...
            StringTokenizer st = new StringTokenizer(value.value, FLATTEN_DELIM);
        ...
        private  Value(String value, Prefix prefix, Option option) {
            this(value, prefix, option, NOTARGUMENT);
        }

        private  Value(String value, Prefix prefix, Option option, int argIndex) {
            this.value = value;
            this.prefix = prefix;
            this.option = option;
        ...
        public Value convert(Prefix prefix) {
            ...
            return new Value(this.value, prefix, this.option, this.argIndex);
        }
        ...
        public boolean sameValueIdentifier(Value other) {
            return ((null != other) && sameValueIdentifier(option, other.value));
        }

        public boolean sameValueIdentifier(Option option, String value) {
            return ((null != option) && this.option.sameOptionIdentifier(option) && this.value.equals(value));
        }

        public boolean conflictsWith(Value other) {
            return ((null != other) && option.equals(other.option) && (prefix.force == other.prefix.force) && ((prefix.set != other.prefix.set) || !value.equals(other.value)));
        }
    ...
    public static class Prefixes {
        ...
        private  Prefixes(Prefix[] prefixes) {
            if (LangUtil.isEmpty(prefixes)) {
                ...
                list = Collections.unmodifiableList(Arrays.asList(LangUtil.safeCopy(prefixes, new Prefix[0])));
            }
        }
    ...
    public static class Prefix {
        ...
        private  Prefix(String prefix, String name, boolean set, boolean force) {
            this.prefix = prefix;
            this.name = name;
            this.set = set;
            this.force = force;
    ...
    public static class InvalidInputException extends Exception {
        ...
        public String getFullMessage() {
            return "illegal input \"" + input + "\" for option " + option + ": " + err;
        }
    }
}
