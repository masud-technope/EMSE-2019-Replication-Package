...
public class FlatSuiteReader implements SFileReader.Maker {
    ...
    public Object make(final LineReader reader) throws AbortException, IOException {
        ...
        if ((null == words) || (0 == words.length)) {
            throw new AbortException("expecting dir {option..} at " + reader);
        }
        ...
        while (null != (line = r.read("source.."))) {
            words = LangUtil.split(line);
            if (0 == FileUtil.sourceSuffixLength(// XXX
            words[// XXX
            0])) {
                break;
            } else {
        ...
        while (null != (line = r.read(" errors, warnings, eclipse.warnings, eclipse.error", false))) {
            words = LangUtil.split(line);
            first = words[0];
    ...
    private void cleanup(AjcTest.Spec result, LineReader lineReader) {
        ...
        if (-1 != loc) {
            ...
            try {
                ...
                throw new Error("unable to convert " + pr + " for " + result + " at " + lineReader);
            }
        }
	 ...
	 * @return List
	 */
    private // XXX weak - also support expected exceptions, etc.
    List makeMessages(Kind kind, String[] words, int start, File lastFile) {
        ArrayList result = new ArrayList();
        for (int i = start; i < words.length; i++) {
            ISourceLocation sl = BridgeUtil.makeSourceLocation(words[i], lastFile);
            if (// XXX signalling during make
            null == sl) {
            // System.err.println(...);
            //MessageUtil.debug(handler, "not a source location: " + words[i]);
            } else {
                String text = (("" + sl.getLine()).equals(words[i]) ? "" : words[i]);
    ...
    public AjcTest.Suite.Spec readSuite(File suiteFile) {
        ...
        try {
            READER.readNodes(suiteFile, collector, abortOnError, System.err);
        } catch (IOException e) {
