...
public class Diffs {
    ...
    public static Diffs makeDiffs(String label, List expected, List actual, Comparator comparator) {
        return makeDiffs(label, expected, actual, comparator, ACCEPT_ALL, ACCEPT_ALL);
    }

    public static Diffs makeDiffs(String label, IMessage[] expected, IMessage[] actual) {
    ...
    public static Diffs makeDiffs(String label, IMessage[] expected, IMessage[] actual, IMessage.Kind[] ignoreExpectedKinds, IMessage.Kind[] ignoreActualKinds) {
            ...
            for (ListIterator iter = act.listIterator(); iter.hasNext(); ) {
                ...
                if (actualLine != lastLine) {
                    ...
                    while (expectedIterator.hasNext()) {
                        IMessage curExpected = (IMessage) expectedIterator.next();
                ...
                for (Iterator iterator = expectedForLine.iterator(); !found && iterator.hasNext(); ) {
    ...
    public static Diffs makeDiffs(String label, List expected, List actual, Comparator comparator, Filter missingFilter, Filter unexpectedFilter) {
        label = label.trim();
        if (null == label) {
            label = ": ";
    ...
    static boolean expectingMessage(IMessage expected, IMessage actual) {
        ...
        if (!expectingSourceLocation(expected.getSourceLocation(), actual.getSourceLocation())) {
            return false;
        }
        ...
        ISourceLocation[] esl = (ISourceLocation[]) expected.getExtraSourceLocations().toArray(new ISourceLocation[0]);
        ISourceLocation[] asl = (ISourceLocation[]) actual.getExtraSourceLocations().toArray(new ISourceLocation[0]);
	 ...
	 */
    static boolean expectingSourceLocations(ISourceLocation[] expected, ISourceLocation[] actual) {
        if (LangUtil.isEmpty(expected)) {
            return true;
        } else if (LangUtil.isEmpty(actual)) {
            return false;
	 ...
	 */
    static boolean expectingSourceLocation(ISourceLocation expected, ISourceLocation actual) {
    ...
    private static ArrayList getExcept(IMessage[] source, IMessage.Kind[] skip) {
    ...
    private static List harden(List list) {
        return (LangUtil.isEmpty(list) ? Collections.EMPTY_LIST : Collections.unmodifiableList(list));
    }
    ...
    public  Diffs(String label, List missing, List unexpected) {
        ...
        different = ((0 != this.missing.size()) || (0 != this.unexpected.size()));
    }
    ...
    public String toString() {
        return label + "(unexpected=" + unexpected.size() + ", missing=" + missing.size() + ")";
    }
