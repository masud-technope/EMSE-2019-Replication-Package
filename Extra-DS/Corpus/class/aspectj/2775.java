...
public class DirChanges {
    ...
    public boolean start(IMessageHandler handler, File baseDir) {
        ...
        boolean result = exists("at start, did not expect added file to exist", !EXISTS, spec.added, doCompare);
    ...
    public boolean end(IMessageHandler handler, File srcBaseDir) {
        ...
        try {
            ...
            boolean result = exists("at end, expected file was not added", EXISTS, spec.added, doCompare);
            if (result || !fastFail) {
     ...
     */
    protected boolean exists(String label, boolean exists, List pathList, boolean doCompare) {
        //        boolean expectStartEarlier = true;
        return exists(label, exists, pathList, doCompare, true);
    }

    protected boolean exists(String label, boolean exists, List pathList, boolean doCompare, boolean expectStartEarlier) {
        boolean result = true;
        if (!LangUtil.isEmpty(pathList)) {
            ...
            for (Iterator iter = pathList.iterator(); iter.hasNext(); ) {
                ...
                if (exists != (actualFile.canRead() && actualFile.isFile() && (expectStartEarlier ? startTime <= actualFile.lastModified() : startTime > actualFile.lastModified()))) {
     ...
     */
    protected void failMessage(IMessageHandler handler, boolean exists, String label, String path, File file) {
    ...
    public static class Spec implements IXmlWritable {
        ...
        private boolean hasFileList() {
            return (!LangUtil.isEmpty(added) || !LangUtil.isEmpty(removed) || !LangUtil.isEmpty(updated) || !LangUtil.isEmpty(unchanged));
        }
