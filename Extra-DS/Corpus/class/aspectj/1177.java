...
public final class LazyClassGen {
    ...
    private SortedMap /* <String, InlinedSourceFileInfo> */
    inlinedFiles = new TreeMap();
    ...
    private String getSourceDebugExtensionString() {
        ...
        for (Iterator iter = inlinedFiles.keySet().iterator(); iter.hasNext(); ) {
            ...
            if (ii == -1) {
                ...
                out.append(i++);
                out.append(' ');
                out.append(element.substring(ii + 1));
                out.append('\n');
                out.append(element);
                out.append('\n');
            }
        ...
        for (Iterator iter = inlinedFiles.values().iterator(); iter.hasNext(); ) {
            ...
            out.append(i++);
            out.append(',');
            out.append(element.highestLineNumber);
            out.append(":");
            out.append(element.offset + 1);
            out.append(",1\n");
        }
        // end section
        out.append("*E\n");
    ...
    public  LazyClassGen(BcelObjectType myType) {
        ...
        if (implementsSerializable(getType())) {
            ...
            if (!hasSerialVersionUIDField && world.isAddSerialVerUID()) {
                ...
                Field fg = new FieldGen(Constants.ACC_PRIVATE | Constants.ACC_FINAL | Constants.ACC_STATIC, BasicType.LONG, "serialVersionUID", getConstantPoolGen()).getField();
                addField(fg);
                hasSerialVersionUIDField = true;
    ...
    public String getInternalClassName() {
        return getConstantPoolGen().getConstantPool().getConstantString(myGen.getClassNameIndex(), Constants.CONSTANT_Class);
    }
    ...
    public void errorOnAddedField(Field field, ISourceLocation sourceLocation) {
        if (isSerializable && !hasSerialVersionUIDField) {
            getWorld().getLint().serialVersionUIDBroken.signal(new String[] { myType.getResolvedTypeX().getName().toString(), field.getName() }, sourceLocation, null);
        }
    }
    ...
    private void writeBack(BcelWorld world) {
        ...
        for (int i = 0; i < attrs.length && !hasVersionAttribute; i++) {
            ...
            if (attribute.getName().equals("org.aspectj.weaver.WeaverVersion"))
    ...
    private void fixupGenericSignatureAttribute() {
        ...
        if (// if null, this is a type built from scratch, it won't already have a sig attribute
        myType != null) {
    ...
    private static boolean hasSourceDebugExtensionAttribute(ClassGen gen) {
        ...
        for (int i = 0; i < attrs.length; i++) {
            if ("SourceDebugExtension".equals(((ConstantUtf8) pool.getConstant(attrs[i].getNameIndex())).getBytes())) {
                return true;
            }
        }
        return false;
    ...
    public String toShortString() {
        String s = org.aspectj.apache.bcel.classfile.Utility.accessToString(myGen.getAccessFlags(), true);
        if (s != "")
            s += " ";
    ...
    public LazyMethodGen getStaticInitializer() {
        ...
        LazyMethodGen clinit = new LazyMethodGen(Modifier.STATIC, Type.VOID, "<clinit>", new Type[0], CollectionUtil.NO_STRINGS, this);
    ...
    public LazyMethodGen getAjcPreClinit() {
        ...
        LazyMethodGen ajcClinit = new LazyMethodGen(Modifier.STATIC, Type.VOID, NameMangler.AJC_PRE_CLINIT_NAME, new Type[0], CollectionUtil.NO_STRINGS, this);
    ...
    Map /*BcelShadow, Field*/
    tjpFields = new HashMap();
    ...
    public Field getTjpField(BcelShadow shadow, final boolean isEnclosingJp) {
        ...
        if (// TAG:SUPPORTING12: We didn't have different staticjp types in 1.2
    ...
    private void initializeTjp(InstructionFactory fact, InstructionList list, Field field, BcelShadow shadow) {
        ...
        if (world.isTargettingAspectJRuntime12()) {
            ...
            list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING }, Constants.INVOKEVIRTUAL));
            ...
            list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING, Type.STRING, Type.STRING, Type.STRING, Type.STRING, Type.STRING, Type.STRING }, Constants.INVOKEVIRTUAL));
        } else if (sig.getKind().equals(Member.HANDLER)) {
            BcelWorld w = shadow.getWorld();
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getDeclaringType())));
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getParameterTypes())));
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getParameterNames(w))));
            list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING, Type.STRING, Type.STRING }, Constants.INVOKEVIRTUAL));
            ...
            if (w.isJoinpointArrayConstructionEnabled() && sig.getDeclaringType().isArray()) {
                ...
                list.append(new PUSH(getConstantPoolGen(), makeString(sig.getDeclaringType())));
                list.append(new PUSH(getConstantPoolGen(), makeString(sig.getParameterTypes())));
                //makeString("")));//sig.getParameterNames(w))));
                list.append(new PUSH(getConstantPoolGen(), ""));
                //makeString("")));//sig.getExceptions(w))));
                list.append(new PUSH(getConstantPoolGen(), ""));
                list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING, Type.STRING, Type.STRING, Type.STRING, Type.STRING }, Constants.INVOKEVIRTUAL));
            } else {
                list.append(new PUSH(getConstantPoolGen(), makeString(sig.getModifiers(w))));
                list.append(new PUSH(getConstantPoolGen(), makeString(sig.getDeclaringType())));
                list.append(new PUSH(getConstantPoolGen(), makeString(sig.getParameterTypes())));
            ...
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getDeclaringType())));
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getReturnType())));
            list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING, Type.STRING, Type.STRING, Type.STRING }, Constants.INVOKEVIRTUAL));
            ...
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getModifiers(w))));
            list.append(new PUSH(getConstantPoolGen(), makeString(sig.getDeclaringType())));
            list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING, Type.STRING }, Constants.INVOKEVIRTUAL));
        } else {
            list.append(new PUSH(getConstantPoolGen(), sig.getSignatureString(shadow.getWorld())));
            list.append(fact.createInvoke(factoryType.getClassName(), sig.getSignatureMakerName(), new ObjectType(sig.getSignatureType()), new Type[] { Type.STRING }, Constants.INVOKEVIRTUAL));
        }
        ...
        for (int i = 0; i < interfaces.length; i++) {
            if (interfaces[i].getSignature().equals(UnresolvedType.SERIALIZABLE.getSignature())) {
                ...
                if (superInterface != null && !superInterface.isMissing()) {
                    if (implementsSerializable(superInterface))
                        return true;
                }
            }
        }
