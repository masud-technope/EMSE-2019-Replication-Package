...
public class PackageTreeWriter extends com.sun.tools.doclets.standard.PackageTreeWriter {

    protected class Del extends AbstractTreeWriter {

        public  Del(String s, ClassTree c) throws IOException, DocletAbortException {
            super(s, c);
        }
    ...
    public static void generate(PackageDoc pkg, PackageDoc prev, PackageDoc next, boolean noDeprecated) throws DocletAbortException {
        ...
        try {
            (pw = new PackageTreeWriter(path, filename, pkg, prev, next, noDeprecated)).generatePackageTreeFile();
        } catch (IOException e) {
            Standard.configuration().standardmessage.error("doclet.exception_encountered", e + "", filename);
