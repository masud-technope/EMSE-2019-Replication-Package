...
public class JavadocCompareClassMode {
     ...
     */
    public static int completeProcess(final Process process, int secsToWait, PrintStream outSink, PrintStream errSink) {
        if (null == process)
            throw new IllegalArgumentException("null process");
        if ((Integer.MAX_VALUE != secsToWait) && ((MIN_SECS > secsToWait) || ((MAX_SECS < secsToWait)))) {
        ...
        try {
            ...
            if (null != task)
                task.cancel();
            if (errSink != null)
                writeStream(process.getErrorStream(), errSink);
            // misnamed API: the "input" stream is our input from the process output
            if (outSink != null)
                writeStream(process.getInputStream(), outSink);
        }
        return status;
    }
    ...
    public static int runCommand(String command) {
        ...
        try {
            ...
            throw new RuntimeException("could not execute: " + command + ", " + e.getMessage());
        }
        return result;
    }
}
