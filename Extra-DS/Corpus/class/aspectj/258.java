...
public class AspectDeclaration extends TypeDeclaration {
    ...
    public void checkSpec(ClassScope scope) {
        ...
        if (dominatesPattern != null) {
            scope.problemReporter().signalError(dominatesPattern.getStart(), dominatesPattern.getEnd(), "dominates has changed for 1.1, use 'declare precedence: " + new String(this.name) + ", " + dominatesPattern.toString() + ";' " + "in the body of the aspect instead");
        }
        if (!isAbstract()) {
        ...
        if (superType.isAspect()) {
            ...
            if (superType.isRawType()) {
                scope.problemReporter().signalError(sourceStart, sourceEnd, "a generic super-aspect must be fully parameterized in an extends clause");
                ignoreFurtherInvestigation = true;
                return;
            }
        }
    ...
    public void generateCode(ClassFile enclosingClassFile) {
        ...
        if (!isAbstract()) {
            ...
            // we've already produced an error for this
                ...
                binding.addField(factory.makeFieldBinding(AjcMemberMaker.perSingletonField(typeX)));
                ...
                binding.addField(factory.makeFieldBinding(AjcMemberMaker.perCflowField(typeX)));
	 ...
	 */
    private void generateMethod(ClassFile classFile, MethodBinding methodBinding, List additionalAttributes, /*ResolvedMember realMember*/
    BodyGenerator gen) {
        //		EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(this.scope);
        classFile.generateMethodInfoHeader(methodBinding);
        int methodAttributeOffset = classFile.contentsOffset;
        int attributeNumber;
        if (// mini optimization
    ...
    private void generatePerCflowPushMethod(ClassFile classFile) {
        final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(this.scope);
        generateMethod(classFile, world.makeMethodBinding(AjcMemberMaker.perCflowPush(factory.fromBinding(binding))), new BodyGenerator() {
            ...
            public void generate(CodeStream codeStream) {
                ...
                codeStream.dup();
                codeStream.invokespecial(new MethodBinding(0, "<init>".toCharArray(), BaseTypes.VoidBinding, new TypeBinding[0], new ReferenceBinding[0], binding));
    ...
    private void generatePerCflowAjcClinitMethod(ClassFile classFile) {
        final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(this.scope);
        generateMethod(classFile, world.makeMethodBinding(AjcMemberMaker.ajcPreClinitMethod(world.fromBinding(binding))), new BodyGenerator() {
    ...
    private void generatePerTypeWithinGetInstanceMethod(ClassFile classFile) {
        ...
        generateMethod(classFile, AjcMemberMaker.perTypeWithinGetInstance(world.fromBinding(binding)), new BodyGenerator() {
            ...
            public void generate(CodeStream codeStream) {
                ...
                codeStream.aconst_null();
                codeStream.invokevirtual(new MethodBinding(0, "getDeclaredMethod".toCharArray(), world.makeTypeBinding(// return type
                UnresolvedType.forSignature(// return type
                "Ljava/lang/reflect/Method;")), new TypeBinding[] { world.makeTypeBinding(UnresolvedType.forSignature("Ljava/lang/String;")), world.makeTypeBinding(UnresolvedType.forSignature("[Ljava/lang/Class;")) }, new ReferenceBinding[0], (ReferenceBinding) world.makeTypeBinding(UnresolvedType.JAVA_LANG_CLASS)));
                codeStream.astore_1();
                codeStream.aload_1();
                codeStream.aconst_null();
                codeStream.aconst_null();
                codeStream.invokevirtual(new MethodBinding(0, "invoke".toCharArray(), world.makeTypeBinding(UnresolvedType.OBJECT), new TypeBinding[] { world.makeTypeBinding(UnresolvedType.OBJECT), world.makeTypeBinding(UnresolvedType.forSignature("[Ljava/lang/Object;")) }, new ReferenceBinding[0], (ReferenceBinding) world.makeTypeBinding(UnresolvedType.JAVA_LANG_REFLECT_METHOD)));
    ...
    private void generatePerTypeWithinCreateAspectInstanceMethod(ClassFile classFile) {
        final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(this.scope);
        generateMethod(classFile, AjcMemberMaker.perTypeWithinCreateAspectInstance(world.fromBinding(binding)), new BodyGenerator() {
    ...
    private void generatePerSingletonAspectOfMethod(ClassFile classFile) {
        ...
        generateMethod(classFile, aspectOfMethod, new BodyGenerator() {
            ...
            public void generate(CodeStream codeStream) {
                ...
                // GETSTATIC
                codeStream.getstatic(initFailureField);
                codeStream.invokespecial(world.makeMethodBindingForCall(// INVOKESPECIAL
                AjcMemberMaker.noAspectBoundExceptionInitWithCause()));
                // ATHROW
                codeStream.athrow();
                isNonNull.place();
    ...
    private void generatePerSingletonAjcClinitMethod(ClassFile classFile) {
        final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(this.scope);
        generateMethod(classFile, world.makeMethodBinding(AjcMemberMaker.ajcPostClinitMethod(world.fromBinding(binding))), new BodyGenerator() {
    ...
    private void buildPerClause(ClassScope scope) {
        ...
        //XXX might be too soon for some error checking
        resolvePerClause();
    }
    ...
    public StringBuffer printHeader(int indent, StringBuffer output) {
        ...
        if (superInterfaces != null && superInterfaces.length > 0) {
            //$NON-NLS-2$ //$NON-NLS-1$
            output.append((kind() == IGenericType.INTERFACE_DECL) ? " extends " : " implements ");
