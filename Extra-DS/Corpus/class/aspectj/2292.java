...
public class AtAjAttributes {
        ...
        for (int i = 0; i < javaClass.getFields().length; i++) {
            ...
            for (int j = 0; j < fattributes.length; j++) {
                ...
                if (acceptAttribute(fattribute)) {
                    ...
                    if (handleDeclareErrorOrWarningAnnotation(frvs, fstruct) || handleDeclareParentsAnnotation(frvs, fstruct)) {
    ...
    public static List readAj5MethodAttributes(Method method, BcelMethod bMethod, ResolvedType type, ResolvedPointcutDefinition preResolvedPointcut, ISourceContext context, IMessageHandler msgHandler) {
        ...
        for (int i = 0; i < attributes.length; i++) {
            ...
            try {
                ...
                msgHandler.handleMessage(new Message(WeaverMessages.format(WeaverMessages.RETURNING_FORMAL_NOT_DECLARED_IN_ADVICE, e.getFormalName()), IMessage.ERROR, null, bMethod.getSourceLocation()));
            } catch (ThrownFormalNotDeclaredInAdviceSignatureException e) {
                msgHandler.handleMessage(new Message(WeaverMessages.format(WeaverMessages.THROWN_FORMAL_NOT_DECLARED_IN_ADVICE, e.getFormalName()), IMessage.ERROR, null, bMethod.getSourceLocation()));
            }
        }
        ...
        if (hasAtAspectJAnnotation && !struct.method.isPublic()) {
            msgHandler.handleMessage(new Message("Found @AspectJ annotation on a non public advice '" + methodToString(struct.method) + "'", IMessage.ERROR, null, type.getSourceLocation()));
            // go ahead
            ;
        }
        ...
        if (hasAtAspectJAnnotationMustReturnVoid && !Type.VOID.equals(struct.method.getReturnType())) {
            msgHandler.handleMessage(new Message("Found @AspectJ annotation on a non around advice not returning void '" + methodToString(struct.method) + "'", IMessage.ERROR, null, type.getSourceLocation()));
            // go ahead
            ;
        }
        return struct.ajAttributes;
    ...
    private static boolean handleAspectAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeStruct struct) {
        ...
        if (aspect != null) {
            ...
            if (perClause == null) {
                ...
                //FIXME AVASM
                perClause.setLocation(struct.context, struct.context.getOffset(), struct.context.getOffset() + 1);
        ...
        if (decp != null) {
            ...
            if (decpPattern != null) {
                ...
                if (fieldType.isInterface()) {
                    ...
                    //TODO kick ISourceLocation sl = struct.bField.getSourceLocation();    ??
                    // not ideal...
                    dp.setLocation(struct.context, 0, 0);
                    ...
                    if (hasAtLeastOneMethod) {
                        struct.ajAttributes.add(new AjAttribute.TypeMunger(new MethodDelegateTypeMunger.FieldHostTypeMunger(AjcMemberMaker.itdAtDeclareParentsField(//prototyped
                        null, fieldType, struct.enclosingType), struct.enclosingType, typePattern)));
                    }
                    return true;
                } else {
    ...
    private static boolean handleBeforeAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeMethodStruct struct, ResolvedPointcutDefinition preResolvedPointcut) {
        ...
        if (before != null) {
            ...
            if (beforeAdvice != null) {
                ...
                struct.ajAttributes.add(new AjAttribute.AdviceAttribute(AdviceKind.Before, pc, extraArgument, sl.getOffset(), //FIXME AVASM
                sl.getOffset() + 1, struct.context));
                return true;
            }
        }
        return false;
    ...
    private static boolean handleAfterAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeMethodStruct struct, ResolvedPointcutDefinition preResolvedPointcut) {
        ...
        if (after != null) {
            ...
            if (afterAdvice != null) {
                ...
                setIgnoreUnboundBindingNames(pc, bindings);
                ISourceLocation sl = struct.context.makeSourceLocation(struct.bMethod.getDeclarationLineNumber(), struct.bMethod.getDeclarationOffset());
                struct.ajAttributes.add(new AjAttribute.AdviceAttribute(AdviceKind.After, pc, extraArgument, sl.getOffset(), //FIXME AVASM
                sl.getOffset() + 1, struct.context));
                return true;
            }
        }
    ...
    private static boolean handleAfterReturningAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeMethodStruct struct, ResolvedPointcutDefinition preResolvedPointcut, BcelMethod owningMethod) throws ReturningFormalNotDeclaredInAdviceSignatureException {
        ...
        if (after != null) {
            ...
            setIgnoreUnboundBindingNames(pc, bindings);
            ISourceLocation sl = struct.context.makeSourceLocation(struct.bMethod.getDeclarationLineNumber(), struct.bMethod.getDeclarationOffset());
            struct.ajAttributes.add(new AjAttribute.AdviceAttribute(AdviceKind.AfterReturning, pc, extraArgument, sl.getOffset(), //FIXME AVASM
            sl.getOffset() + 1, struct.context));
            return true;
        }
        return false;
    ...
    private static boolean handleAfterThrowingAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeMethodStruct struct, ResolvedPointcutDefinition preResolvedPointcut, BcelMethod owningMethod) throws ThrownFormalNotDeclaredInAdviceSignatureException {
        ...
        if (after != null) {
            ...
            setIgnoreUnboundBindingNames(pc, bindings);
            ISourceLocation sl = struct.context.makeSourceLocation(struct.bMethod.getDeclarationLineNumber(), struct.bMethod.getDeclarationOffset());
            struct.ajAttributes.add(new AjAttribute.AdviceAttribute(AdviceKind.AfterThrowing, pc, extraArgument, sl.getOffset(), //FIXME AVASM
            sl.getOffset() + 1, struct.context));
            return true;
        }
        return false;
    ...
    private static boolean handleAroundAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeMethodStruct struct, ResolvedPointcutDefinition preResolvedPointcut) {
        ...
        if (around != null) {
            ...
            if (aroundAdvice != null) {
                ...
                setIgnoreUnboundBindingNames(pc, bindings);
                ISourceLocation sl = struct.context.makeSourceLocation(struct.bMethod.getDeclarationLineNumber(), struct.bMethod.getDeclarationOffset());
                struct.ajAttributes.add(new AjAttribute.AdviceAttribute(AdviceKind.Around, pc, extraArgument, sl.getOffset(), //FIXME AVASM
                sl.getOffset() + 1, struct.context));
                return true;
            }
        }
    ...
    private static void handlePointcutAnnotation(RuntimeAnnotations runtimeAnnotations, AjAttributeMethodStruct struct) {
        ...
        if (pointcut != null) {
            ...
            struct.ajAttributes.add(new AjAttribute.PointcutDeclarationAttribute(new LazyResolvedPointcutDefinition(struct.enclosingType, struct.method.getModifiers(), struct.method.getName(), argumentTypes, UnresolvedType.forSignature(struct.method.getReturnType().getSignature()), //can be null for abstract pointcut
            pc, binding)));
        }
    }
