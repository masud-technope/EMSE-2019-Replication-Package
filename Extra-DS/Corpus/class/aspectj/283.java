...
public class CompilerAdapter {
    ...
    private static boolean configureBuildOptions(AjBuildConfig config, BuildOptionsAdapter options, IMessageHandler handler) {
        ...
        if (!LangUtil.isEmpty(debugOptions)) {
            ...
            if (sourceLine)
                optionsToSet.put(CompilerOptions.OPTION_SourceFileAttribute, CompilerOptions.GENERATE);
            if (varAttr)
                optionsToSet.put(CompilerOptions.OPTION_LocalVariableAttribute, CompilerOptions.GENERATE);
            if (lineNo)
                optionsToSet.put(CompilerOptions.OPTION_LineNumberAttribute, CompilerOptions.GENERATE);
        }
    ...
    private static boolean configureNonStandardOptions(AjBuildConfig config, String nonStdOptions, IMessageHandler messageHandler) {
        ...
        if (// dont tokenize within double quotes
        (ind > -1) && (ind2 > -1)) {
    ...
    private void init() {
        if (// XXX plug into AJDE initialization
        !initialized) {
    ...
    class MessageHandlerAdapter extends MessageHandler {
        ...
        public boolean handleMessage(IMessage message) throws AbortException {
            ...
            if (isIgnoring(kind) || (!showInfoMessages && IMessage.INFO.equals(kind))) {
                return true;
            }
