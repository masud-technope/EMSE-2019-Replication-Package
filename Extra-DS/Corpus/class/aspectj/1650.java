...
public class InterTypeFieldBinding extends FieldBinding {
    ...
    public  InterTypeFieldBinding(EclipseFactory world, ResolvedTypeMunger munger, UnresolvedType withinType, AbstractMethodDeclaration sourceMethod) {
        ...
        reader = new SimpleSyntheticAccessMethodBinding(world.makeMethodBinding(AjcMemberMaker.interFieldGetDispatcher(munger.getSignature(), withinType)));
        writer = new SimpleSyntheticAccessMethodBinding(world.makeMethodBinding(AjcMemberMaker.interFieldSetDispatcher(munger.getSignature(), withinType)));
    }
    ...
    public boolean canBeSeenBy(TypeBinding receiverType, InvocationSite invocationSite, Scope scope) {
        ...
        if (isPrivate()) {
            ...
            while (!receiverTypeIsSameOrInsideDeclaringType && typeToCheckNext != null) {
                if (typeToCheckNext == declaringType)
                    receiverTypeIsSameOrInsideDeclaringType = true;
            }
            if (!receiverTypeIsSameOrInsideDeclaringType)
                return false;
            if (invocationType != declaringType) {
