...
public class DeprecatedAPIListBuilder extends com.sun.tools.doclets.standard.DeprecatedAPIListBuilder {
                ...
                for (int j = 0; j < intros.length; j++) {
                    if (intros[j] instanceof IntroducedDoc) {
                        ...
                        if (md == null)
                            continue;
                        if (md.isField()) {
                            _composeDeprecatedList(deprecatedfieldintroductions, intros[j]);
                        } else if (md.isMethod()) {
                            _composeDeprecatedList(deprecatedmethodintroductions, intros[j]);
                        } else {
                            _composeDeprecatedList(deprecatedconstructorintroductions, intros[j]);
                        }
                    } else {
                        _composeDeprecatedList(deprecatedsuperintroductions, intros[j]);
                    }
                }
            }
        }
    ...
    protected void _composeDeprecatedList(List list, MemberDoc[] members) {
        Util.invoke(com.sun.tools.doclets.standard.DeprecatedAPIListBuilder.class, this, "composeDeprecatedList", new Class[] { java.util.List.class, com.sun.javadoc.MemberDoc[].class }, new Object[] { list, members });
    }
