...
public class Node {
     ...
     */
    public static final boolean visit(Node node, ObjectChecker itemChecker, ObjectChecker nodeVisitor) {
        if (null == node) {
        ...
        for (int i = 0; result && (i < nodes.length); i++) {
            result = (null == nodeVisitor ? visit(nodes[i], itemChecker, null) : nodeVisitor.isValid(nodes[i]));
        }
        if (result) {
