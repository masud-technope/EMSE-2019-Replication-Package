...
public class BcelShadow extends Shadow {
    ...
    private void deleteNewAndDup() {
        ...
        while (true) {
            ...
            if (inst instanceof INVOKESPECIAL && ((INVOKESPECIAL) inst).getName(cpg).equals("<init>")) {
                depth++;
    ...
    protected void prepareForMungers() {
        ...
        if (getKind() == ConstructorCall) {
            if (!world.isJoinpointArrayConstructionEnabled() || !this.getSignature().getDeclaringType().isArray())
                // no new/dup for new array construction
                deleteNewAndDup();
            initializeArgVars();
        } else if (// pr74952
        ...
        //world.isXlazyTjp(); // lazy is default now
    ...
    public static BcelShadow makeConstructorExecution(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle justBeforeStart) {
        ...
        BcelShadow s = new BcelShadow(world, ConstructorExecution, world.makeJoinPointSignature(enclosingMethod), enclosingMethod, null);
    ...
    public static BcelShadow makeStaticInitialization(BcelWorld world, LazyMethodGen enclosingMethod) {
        ...
        //        	}
        //        }
        BcelShadow s = new BcelShadow(world, StaticInitialization, world.makeJoinPointSignature(enclosingMethod), enclosingMethod, null);
        ShadowRange r = new ShadowRange(body);
        r.associateWithShadow(s);
    ...
    public static BcelShadow makeUnfinishedInitialization(BcelWorld world, LazyMethodGen constructor) {
        BcelShadow ret = new BcelShadow(world, Initialization, world.makeJoinPointSignature(constructor), constructor, null);
    ...
    public static BcelShadow makeUnfinishedPreinitialization(BcelWorld world, LazyMethodGen constructor) {
        BcelShadow ret = new BcelShadow(world, PreInitialization, world.makeJoinPointSignature(constructor), constructor, null);
    ...
    public static BcelShadow makeMethodExecution(BcelWorld world, LazyMethodGen enclosingMethod) {
        return makeShadowForMethod(world, enclosingMethod, MethodExecution, //world.makeMethodSignature(enclosingMethod));
    ...
    public static BcelShadow makeFieldGet(BcelWorld world, ResolvedMember field, LazyMethodGen enclosingMethod, InstructionHandle getHandle, BcelShadow enclosingShadow) {
        final InstructionList body = enclosingMethod.getBody();
        BcelShadow s = new BcelShadow(world, FieldGet, field, //                    (FieldInstruction) getHandle.getInstruction()),
    ...
    public static BcelShadow makeFieldSet(BcelWorld world, LazyMethodGen enclosingMethod, InstructionHandle setHandle, BcelShadow enclosingShadow) {
        final InstructionList body = enclosingMethod.getBody();
        BcelShadow s = new BcelShadow(world, FieldSet, BcelWorld.makeFieldJoinPointSignature(enclosingMethod.getEnclosingClass(), (FieldInstruction) setHandle.getInstruction()), enclosingMethod, enclosingShadow);
        ShadowRange r = new ShadowRange(body);
        r.associateWithShadow(s);
    ...
    private Map /*<UnresolvedType,BcelVar>*/
    kindedAnnotationVars = null;

    private Map /*<UnresolvedType,BcelVar>*/
    thisAnnotationVars = null;

    private Map /*<UnresolvedType,BcelVar>*/
    targetAnnotationVars = null;

    private Map[] argAnnotationVars = null;

    private Map /*<UnresolvedType,BcelVar>*/
    withinAnnotationVars = null;

    private Map /*<UnresolvedType,BcelVar>*/
    withincodeAnnotationVars = null;
        ...
        for (Iterator i = mungers.iterator(); i.hasNext(); ) {
            ...
            if (munger instanceof Advice) {
                if (((Advice) munger).getKind() == AdviceKind.Around) {
                    if (// do we know enough to bother reporting?
    ...
    public BcelVar getThisJoinPointStaticPartBcelVar(final boolean isEnclosingJp) {
        if (thisJoinPointStaticPartVar == null) {
            ...
            if (world.isTargettingAspectJRuntime12()) {
                ...
                sjpType = isEnclosingJp ? world.getCoreType(UnresolvedType.forName("org.aspectj.lang.JoinPoint$EnclosingStaticPart")) : world.getCoreType(UnresolvedType.forName("org.aspectj.lang.JoinPoint$StaticPart"));
            }
    ...
    public UnresolvedType ensureTargetTypeIsCorrect(UnresolvedType tx) {
        ...
        if (msig.getArity() == 0 && getKind() == MethodCall && msig.getName().charAt(0) == 'c' && tx.equals(ResolvedType.OBJECT) && msig.getReturnType().equals(ResolvedType.OBJECT) && msig.getName().equals("clone")) {
            ...
            while (Range.isRangeHandle(searchPtr) || searchPtr.getInstruction() instanceof StoreInstruction) {
    ...
    public void initializeWithinCodeAnnotationVars() {
        ...
        for (int i = 0; i < annotations.length; i++) {
            ...
            withincodeAnnotationVars.put(ann, new KindedAnnotationAccessVar(k, ann, getEnclosingType(), getEnclosingCodeSignature()));
        }
    }
 ...
 44:  invokespecial   #23; //Method java/lang/Object."<init>":()V
        ...
        if (ret != null) {
            ...
            for (Iterator i = returns.iterator(); i.hasNext(); ) {
                ...
                Utility.replaceInstruction(ih, InstructionFactory.createBranchInstruction(Constants.GOTO, gotoTarget), enclosingMethod);
            }
            range.append(advice);
    ...
    public void weaveAfterThrowing(BcelAdvice munger, UnresolvedType catchType) {
        ...
        if (this.getEnclosingMethod().getName().equals("<clinit>")) {
            ...
            BranchInstruction bi = InstructionFactory.createBranchInstruction(Constants.IFEQ, ih.getStart());
            handler.append(bi);
            handler.append(exceptionVar.createLoad(fact));
        ...
        enclosingMethod.addExceptionHandler(range.getStart().getNext(), protectedEnd.getPrev(), //???Type.THROWABLE, 
        handlerStart, //???Type.THROWABLE, 
        (ObjectType) BcelWorld.makeBcelType(catchType), // high priority if our args are on the stack
    ...
    public void weaveSoftener(BcelAdvice munger, UnresolvedType catchType) {
        ...
        handler.append(exceptionVar.createLoad(fact));
        handler.append(fact.createInvoke(NameMangler.SOFT_EXCEPTION_TYPE, "<init>", Type.VOID, new Type[] { Type.THROWABLE }, //??? special
        ...
        enclosingMethod.addExceptionHandler(range.getStart().getNext(), protectedEnd.getPrev(), handlerStart, (ObjectType) BcelWorld.makeBcelType(catchType), // high priority if our args are on the stack
        getKind().hasHighPriorityExceptions());
    }
        ...
        weaveAfter(new BcelAdvice(null, null, null, 0, 0, 0, null, null) {
            ...
            public InstructionList getAdviceInstructions(BcelShadow s, BcelVar extraArgVar, InstructionHandle ifNoAdvice) {
                ...
                if (munger.getKind() != AdviceKind.PerCflowEntry && munger.getKind() != AdviceKind.PerCflowBelowEntry && munger.getExposedStateAsBcelVars(false).length == 0) {
                    exitInstructions.append(fact.createInvoke(NameMangler.CFLOW_COUNTER_TYPE, "dec", Type.VOID, new Type[] {}, Constants.INVOKEVIRTUAL));
                } else {
                    exitInstructions.append(fact.createInvoke(NameMangler.CFLOW_STACK_TYPE, "pop", Type.VOID, new Type[] {}, Constants.INVOKEVIRTUAL));
                }
    ...
    public void weaveAroundInline(BcelAdvice munger, boolean hasDynamicTest) {
        ...
        if (declaringType.isMissing()) {
            world.getLint().cantFindType.signal(new String[] { WeaverMessages.format(WeaverMessages.CANT_FIND_TYPE_DURING_AROUND_WEAVE, declaringType.getClassName()) }, getSourceLocation(), new ISourceLocation[] { munger.getSourceLocation() });
        ...
        LazyMethodGen extractedMethod = extractMethod(NameMangler.aroundCallbackMethodName(getSignature(), getEnclosingClass()), Modifier.PRIVATE, munger);
        ...
        LazyMethodGen localAdviceMethod = new LazyMethodGen(Modifier.PRIVATE | Modifier.FINAL | Modifier.STATIC, BcelWorld.makeBcelType(mungerSig.getReturnType()), adviceMethodName, parameterTypes, new String[0], getEnclosingClass());
        ...
        // InstructionHandle adviceMethodInvocation;
        {
            ...
            advice.append(munger.getAdviceArgSetup(this, null, (munger.getConcreteAspect().isAnnotationStyleAspect() && munger.getDeclaringAspect() != null && munger.getDeclaringAspect().resolve(world).isAnnotationStyleAspect()) ? this.loadThisJoinPoint() : new InstructionList(InstructionConstants.ACONST_NULL)));
            // adviceMethodInvocation =
            advice.append(//(fact, getWorld(), munger.getSignature()));
            Utility.createInvoke(fact, localAdviceMethod));
            advice.append(Utility.createConversion(getFactory(), BcelWorld.makeBcelType(mungerSig.getReturnType()), extractedMethod.getReturnType(), world.isInJava5Mode()));
            if (!isFallsThrough()) {
        ...
        // and that's it.
            ...
            while (curr != end) {
                ...
                if ((inst instanceof INVOKEINTERFACE) && "proceed".equals(((INVOKEINTERFACE) inst).getMethodName(cpg))) {
                    ...
                    InstructionList insteadProceedIl = getRedoneProceedCallForAnnotationStyle(fact, extractedMethod, munger, localAdviceMethod, proceedVarList, isProceedWithArgs);
    ...
    public void weaveAroundClosure(BcelAdvice munger, boolean hasDynamicTest) {
        ...
        LazyMethodGen callbackMethod = extractMethod(NameMangler.aroundCallbackMethodName(getSignature(), getEnclosingClass()), 0, munger);
        BcelVar[] adviceVars = munger.getExposedStateAsBcelVars(true);
        ...
        if (getKind() == PreInitialization) {
            ...
            returnConversionCode = Utility.createConversion(getFactory(), BcelWorld.makeBcelType(munger.getSignature().getReturnType()), callbackMethod.getReturnType(), world.isInJava5Mode());
            if (!isFallsThrough()) {
        ...
        if (munger.getConcreteAspect() != null && munger.getConcreteAspect().isAnnotationStyleAspect() && munger.getDeclaringAspect() != null && munger.getDeclaringAspect().resolve(world).isAnnotationStyleAspect()) {
            closureInstantiation.append(Utility.createInvoke(getFactory(), getWorld(), new MemberImpl(Member.METHOD, UnresolvedType.forName("org.aspectj.runtime.internal.AroundClosure"), Modifier.PUBLIC, "linkClosureAndJoinPoint", "()Lorg/aspectj/lang/ProceedingJoinPoint;")));
        }
        InstructionList advice = new InstructionList();
    ...
    private Type[] getSuperConstructorParameterTypes() {
        ...
        return superCallInstruction.getArgumentTypes(getEnclosingClass().getConstantPoolGen());
    }
    ...
    private LazyMethodGen createMethodGen(String newMethodName, int visibilityModifier) {
        ...
        if (targetVar != null && targetVar != thisVar) {
            ...
            if (getKind() == FieldGet && getActualTargetType() != null && !getActualTargetType().equals(targetType.getName())) {
                targetType = UnresolvedType.forName(getActualTargetType()).resolve(world);
            }
            ResolvedMember resolvedMember = getSignature().resolve(world);
            if (resolvedMember != null && Modifier.isProtected(resolvedMember.getModifiers()) && !samePackage(targetType.getPackageName(), getEnclosingType().getPackageName()) && !resolvedMember.getName().equals("clone")) {
        ...
        return new LazyMethodGen(modifiers, BcelWorld.makeBcelType(returnType), newMethodName, parameterTypes, new String[0], //                UnresolvedType.getNames(getSignature().getExceptions(world)),
        getEnclosingClass());
    }
