...
public class LangUtil {
     ...
     */
    public static final void throwIaxIfNotAllAssignable(final Collection collection, final Class c, final String name) {
    ...
    static String clipBuffer(StringBuffer buffer, int maxLines) {
        ...
        for (int i = 0, srcBegin = 0; i < MAX; srcBegin += j) {
            ...
            for (j = 0; j < srcEnd - srcBegin; /*chars.length*/
            j++) {
                char c = chars[j];
                if (c == '\n') {
                    i++;
     ...
     */
    public static // XXX dumb implementation
    String makeClasspath(String bootclasspath, String classpath, String classesDir, String outputJar) {
     ...
     */
    public static // not needed?
    ProcessController makeProcess(ProcessController controller, String[] command, String label) {
        if (null == controller) {
            controller = new ProcessController();
        }
