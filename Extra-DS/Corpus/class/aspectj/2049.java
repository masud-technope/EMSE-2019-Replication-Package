...
public class InpathTestcase extends AjdeTestCase {
    ...
    public void testInpathToOutjar2() {
        ...
        try {
            ...
            assertTrue("Weaving of Aspect should have occurred but the input and output size for HelloWorld.class are the same", (inputsize != outputsize));
        } catch (Exception e) {
            e.printStackTrace();
            fail();
        }
    ...
    public void compareJars(File dirFile, String sourceDir, File outjarFile, Set expectedOutputJarContents) {
        try {
            assertTrue("outjar older than injar", (outjarFile.lastModified() > dirFile.lastModified()));
            ...
            outjar.close();
            assertTrue("Didnt make it into the output jar: " + expectedOutputJarContents.toString(), expectedOutputJarContents.isEmpty());
