...
public abstract class Builder {
    ...
    private void verifyBuildSpec(BuildSpec buildSpec) {
        if (null == buildSpec.productDir) {
            ...
            if (null == buildSpec.moduleDir) {
                ...
                handler.log("Builder using derived baseDir: " + buildSpec.baseDir);
            }
    ...
    protected Result specifyResultFor(BuildSpec buildSpec) {
        ...
        Kind kind = Result.kind(buildSpec.trimTesting, buildSpec.assembleAll);
    ...
    protected final boolean buildOnly(Result result, List errors) {
        ...
        try {
            return (compile(result, classesDir, useEclipseCompiles, errors)) && assemble(result, classesDir, errors);
        } finally {
    ...
    protected ProductModule[] discoverModules(File productDir, Modules modules) {
        ...
        FileFilter filter = new // empty jar files
        FileFilter() {

            public boolean accept(File file) {
                if ((null != file) && file.canRead() && // XXXFileLiteral
                file.getPath().endsWith(".jar") && (0l == file.length())) {
                    found.add(file);
                }
                return true;
            }
     ...
     */
    protected abstract Result[] getAntecedantResults(Result toBuild);
     ...
     */
    protected abstract boolean compile(Result result, File classesDir, boolean useExistingClasses, List errors);
     ...
     */
    protected abstract boolean assemble(Result result, File classesDir, List errors);
     ...
     * @see #removeLibraryFilesToSkip(Module, File)
     */
    protected abstract boolean assembleAll(Result result, Messager handler);

    /**
     * Generate the installer for this product to targDirPath
     */
    protected abstract boolean buildInstaller(BuildSpec buildSpec, String targDirPath);

    /**
     * Copy fromFile to toFile, optionally filtering contents
     */
    protected abstract boolean copyFile(File fromFile, File toFile, boolean filter);
     ...
     *            token/value pairs
     */
    protected abstract boolean copyFiles(File fromDir, File toDir, String included, String excluded, boolean filter);
}
