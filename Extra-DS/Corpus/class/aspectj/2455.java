...
public class BuildModuleTests extends TestCase {
    ...
    // XXX never to enable again...
    private static final boolean replacing = false;
    ...
    static final List SOURCE_NAMES = Collections.unmodifiableList(Arrays.asList(new String[] { "src", "testsrc", "java5-src", "aspectj-src" }));
    ...
    public  BuildModuleTests(String name) {
        super(name);
    }
    ...
    void checkSourceDirectory(File srcDir, String module) {
        ...
        if (!"testsrc".equals(srcDir.getName())) {
            ...
            if (!unknownFiles.isEmpty()) {
                String s = "unknown files (see readme-build-module.html to " + "update Builder.properties resource patterns): ";
                fail(s + unknownFiles);
            }
        }
    }
    ...
    static class UnknownFileCheck implements FileFilter {
        ...
        static {
            ...
            suffixes.add(".html");
            // others from Builder
            final String input = Builder.BINARY_SOURCE_PATTERN + "," + Builder.RESOURCE_PATTERN;
