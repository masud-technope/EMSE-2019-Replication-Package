...
public class MainWrapper {
    ...
    /** quit on first exception */
    // todo public class controls - yuck 
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (!Modifier.isStatic(main.getModifiers())) {
                ...
                if (null != outStream)
                    outStream.println("main is not static");
                result = -1;
            }
     ...
     */
    protected static void loadAllClasses(File classesDir) {
        if (null != classesDir) {
            String[] names = LangUtil.classesIn(classesDir);
            StringBuffer err = new StringBuffer();
            LangUtil.loadClasses(names, null, err);
            if (0 < err.length()) {
                throw new Error("MainWrapper Errors loading classes: " + err.toString());
            }
        }
    }
    ...
    static void reportException(String context, Throwable t) {
        PrintStream outStream = OUT_STREAM;
        if (null != outStream) {
            while ((null != t) && (InvocationTargetException.class.isAssignableFrom(t.getClass()))) {
