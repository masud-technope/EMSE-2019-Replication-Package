...
public class Ajctest extends Task implements PropertyChangeListener {
    ...
    //XXX
    private File workingdir = new File("ajworkingdir");
    ...
    private void parse(Argument arg, String name) {
        ...
        String values = ileft == -1 ? rest : name.substring(ileft + 1, iright > ileft ? iright : rest.length() - 1);
        String value = null;
        if (itilde != -1) {
    ...
    public void setClasses(String str) {
        for (StringTokenizer t = new StringTokenizer(str, ", ", false); t.hasMoreTokens(); ) {
    ...
    public static class Argument {
        ...
        public void setName(String str) {
            this.name = str.startsWith("-") ? str : ("-" + (str.startsWith("J") ? str.substring(1) : str));
        }
    ...
    public void setDepends(String depends) {
        for (StringTokenizer t = new StringTokenizer(depends, ", ", false); t.hasMoreTokens(); ) {
    ...
    public class Testset extends FileSet {
        ...
        public void resolve() throws BuildException {
            ...
            for (Iterator iter = argfileNames.iterator(); iter.hasNext(); ) {
                ...
                if (check(argfile, name, location))
                    argfiles.add(argfile);
            }
            if (havecludes || argfiles.size() <= 0) {
                String[] filenames = getDirectoryScanner(project).getIncludedFiles();
                for (int j = 0; j < filenames.length; j++) {
                    String name = filenames[j];
                    if (name.endsWith(".java")) {
                        File file = new File(src, name);
                        if (check(file, name, location))
                            files.add(file);
                    }
                }
            }
            for (Iterator i = Ajctest.this.testclasses.iterator(); i.hasNext(); ) {
        ...
        private boolean check(File file, String name, Location loc) throws BuildException {
            loc = loc != null ? loc : location;
            if (file == null) {
                throw new BuildException("file " + name + " is null!", loc);
            }
            if (!file.exists()) {
                throw new BuildException("file " + file + " with name " + name + " doesn't exist!", loc);
            }
            return true;
        }
    ...
    private void execute(Testset testset, List args) throws BuildException {
        if (testset.files.size() > 0) {
            log("\tfiles:");
            for (Iterator i = testset.files.iterator(); i.hasNext(); ) {
                log("\t  " + i.next());
            }
        }
        if (testset.argfiles.size() > 0) {
            log("\targfiles:");
            for (Iterator i = testset.argfiles.iterator(); i.hasNext(); ) {
                log("\t  " + i.next());
            }
        }
        if (args.size() > 0) {
        ...
        if (testset.testclasses.size() > 0) {
            log("\tclasses:");
            for (Iterator i = testset.testclasses.iterator(); i.hasNext(); ) {
                log("\t  " + i.next());
            }
        }
        ...
        if (!isSet("nocompile") && !nocompile) {
            log("\tcompile" + (testset.noclean ? "(boostrapped)" : "") + "...");
        ...
        if (!goodCompile) {
            post(testset, new Vector(), "couldn't run classes " + testset.testclasses + "due to failed compile", -1, "run");
        } else if (!isSet("norun")) {
            for (Iterator i = testset.testclasses.iterator(); i.hasNext(); ) {
    ...
    public void execute() throws BuildException {
        ...
        prepare();
        log(testsets.size() + " testset" + s(testsets), Project.MSG_VERBOSE);
    ...
    private boolean isSet(String name) {
        String uprop = project.getUserProperty(name);
        if (uprop == null || "no".equals(uprop) || "false".equals(uprop))
            return false;
        String prop = project.getProperty(name);
        if (prop == null || "no".equals(prop) || "false".equals(prop))
            return false;
        return true;
    }
    ...
    public class RunWrapper extends Java {
        ...
        /** tracked in MainWrapper.PROP_NAME */
    ...
    public class Run extends RunWrapper {
        ...
        public Path createBootbootclasspath() {
            if (bootclasspath == null)
                bootclasspath = new Path(this.project);
        ...
        public Path createBootbootclasspatha() {
            if (bootclasspatha == null)
                bootclasspatha = new Path(this.project);
        ...
        public Path createBootbootclasspathp() {
            if (bootclasspathp == null)
                bootclasspathp = new Path(this.project);
        ...
        public int executeJava() {
            Path cp = Ajctest.this.classpath != null ? Ajctest.this.classpath : new Path(this.project, destdir.getAbsolutePath());
    ...
    private void post(Testset testset, List args, String msgs, int exit, String type) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (null == current) {
                    ...
                    int allFails = errs + current.ajdocStats.fails + current.ajcStats.fails + current.runStats.fails;
                    if (1 > allFails) {
                ...
                // setup multiLine
                {
                    ...
                    Object[] os = new Object[] { "ajdoc", current.ajdocStats.goods + "", current.ajdocStats.fails + "", "ajc", current.ajcStats.goods + "", current.ajcStats.fails + "", "run", current.runStats.goods + "", current.runStats.fails + "" };
                ...
                err.println(oneLine);
                if (dumpresults && (allErrors.size() + current.ajdocStats.fails + current.ajcStats.fails + current.runStats.fails) > 0) {
                    String date = date(System.currentTimeMillis());
                    String filename = "ajc-errors";
                    for (StringTokenizer t = new StringTokenizer(date, ",: "); t.hasMoreTokens(); ) {
    ...
    static class Failure {
        ...
        public  Failure(Testset testset, List args, String msgs, int exit, String type, String testId) {
    ...
    private List argcombo(List arguments) {
        ...
        for (Iterator iter = arguments.iterator(); iter.hasNext(); ) {
            ...
            if (arg.values.size() == 0)
                arg.values.add("");
            if (!arg.always && !arg.values.contains(null))
                arg.values.add(null);
            if (arg.values.size() > 0) {
                combos.add(arg);
            } else if (arg.always) {
    ...
    private abstract class ExecWrapper {
        ...
        protected final int run(Commandline cmd) {
            ...
            try {
                ...
                OutputStream los = new LogOutputStream(Ajctest.this, Project.MSG_INFO);
                outPumper = new Thread(new StreamPumper(process.getInputStream(), los));
                errPumper = new Thread(new StreamPumper(process.getErrorStream(), os));
    ...
    private class AjcWrapper extends JavaCommandWrapper {
        ...
        public  AjcWrapper(Testset testset, List args) {
            ...
            if (testset.noclean) {
                setExtraclasspath(new Path(project, destdir.getAbsolutePath()));
            }
        }
    ...
    static List ajdocArgs(List args) {
        ...
        for (Iterator i = args.iterator(); i.hasNext(); ) {
            ...
            if (arg.startsWith("-X")) {
                newargs.add(arg);
            } else if (arg.equals("-public") || arg.equals("-package") || arg.equals("-protected") || arg.equals("-private")) {
                newargs.add(arg);
            } else if (arg.equals("-d") || arg.equals("-classpath") || arg.equals("-cp") || arg.equals("-sourcepath") || arg.equals("-bootclasspath") || arg.equals("-argfile")) {
                newargs.add(arg);
                newargs.add(i.next() + "");
    ...
    private abstract class JavaCommandWrapper extends ExecWrapper {
        ...
        public  JavaCommandWrapper(Testset testset, List args, boolean needsClasspath) {
            this.testset = testset;
            this.args = args;
        ...
        public String toString() {
            return LangUtil.unqualifiedClassName(getClass()) + "(" + getMainClassName() + ")";
        }
        ...
        protected Commandline createCommandline() {
            ...
            if (extraClasspath == null) {
                Path aspectjBuildDir = new Path(project, project.getProperty("ajctest.pathelement"));
                // todo: dependency on ant script variable name ajctest.pathelement
                if (cp == null)
                    cp = aspectjBuildDir;
                else
                    cp.append(aspectjBuildDir);
            }
            if (cp == null) {
            ...
            if (!alreadySetClasspath && testset.classpath != null) {
                ...
                Path _cp = Ajctest.this.classpath != null ? Ajctest.this.classpath : new Path(project, destdir.getAbsolutePath());
        ...
        for (Iterator iter = arglist.iterator(); iter.hasNext(); ) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (Iterator valiter = arg.values.iterator(); valiter.hasNext(); ) {
                    ...
                    if (val != null)
                        newlist.add(new Arg(arg.name, val + "", arg.isj));
                    result.add(newlist);
                }
            }
        }
    ...
    private static class Gui extends JPanel implements PropertyChangeListener {
        ...
        private static class StatusPanel extends TitledPanel {
            ...
            private static class StatusInnerPanel extends TitledPanel {
                ...
                public  StatusInnerPanel(String str) {
                    ...
                    Object[] os = new Object[] { "Passed", goods, "Failed", fails, "Totals", total };
        ...
        private class TablePanel extends TitledPanel {
            ...
            public  TablePanel() {
                ...
                Object[] names = new String[] { "Task", "Type", "Number", "Time" };
            ...
            public void add(Failure f, String taskname, String type, int num, long time) {
                model.addRow(new Object[] { taskname, type, new Integer(num), date(time) });
                failures.add(f);
            }
        }
        ...
        private static class FailurePanel extends TitledPanel {
            ...
            public  FailurePanel() {
                ...
                msgs.setFont(StyleContext.getDefaultStyleContext().getFont("SansSerif", Font.PLAIN, 10));
            ...
            private static class InfoPanel extends JPanel {
                ...
                public  InfoPanel() {
                    ...
                    Object[] os = new Object[] { "Type", "Args", "Exit", "Time", "Files", "Classnames" };
