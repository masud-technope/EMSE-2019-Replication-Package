...
public class Options {
     ...
     */
    public static String missedMatchError(String[] input, Values values) {
        int[] missed = values.indexMissedMatches();
        LangUtil.throwIaxIfNull(input, "input");
        LangUtil.throwIaxIfNull(values, "values");
        LangUtil.throwIaxIfFalse(input.length == values.length(), "input is not matched by values");
        if (0 == missed.length) {
            return null;
        }
     ...
     */
    public Values acceptInput(String[] input) throws Option.InvalidInputException {
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            final int index = i;
            if (null != result) {
                for (int len = result.option.numArguments(); len > 0; len--) {
                    i++;
                    if (i >= input.length) {
                        throw new Option.InvalidInputException("not enough arguments", null, result.option);
                    }
