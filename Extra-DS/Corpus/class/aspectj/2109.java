 ...
 *
 * <p>$Id: CalloutEmitter.java,v 1.1 2002/05/15 17:22:27 isberg Exp $</p>
 *
 ...
 *
 * @version $Id: CalloutEmitter.java,v 1.1 2002/05/15 17:22:27 isberg Exp $
 *
 */
public class CalloutEmitter extends CopyEmitter {
   ...
   */
    public  CalloutEmitter(NamePool namePool, int defaultColumn, boolean foStylesheet, FormatCallout fCallout) {
        super(namePool);
        ...
        if (tokenCount == 2) {
            if (units != null && units.equalsIgnoreCase("linerange")) {
                for (int count = c1; count <= c2; count++) {
                    callout[calloutCount++] = new Callout(coNum, area, count, defaultColumn);
                }
            } else {
                // assume linecolumn
   ...
   */
    protected boolean skipThisElement(int nameCode) {
        if (firstElement) {
            ...
            if ((foStylesheet && thisFingerprint == foBlockFingerprint) || (!foStylesheet && (thisFingerprint == htmlPreFingerprint || thisFingerprint == htmlDivFingerprint))) {
   ...
   */
    private class StartElementInfo {
        ...
        public  StartElementInfo(int nameCode, org.xml.sax.Attributes attributes, int[] namespaces, int nscount) {
            _nameCode = nameCode;
