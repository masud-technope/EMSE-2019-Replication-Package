...
public class RunSpecIterator implements IRunIterator {
     ...
     */
    /** spec for this test */
    // XXX reconsider public after debugging
     ...
     */
    public  RunSpecIterator(AbstractRunSpec spec, Sandbox sandbox, Validator validator, boolean haltOnFailure) {
        this(spec, sandbox, validator, haltOnFailure, (haltOnFailure ? IMessage.ABORT : IMessage.ERROR));
    }
     ...
     */
    public  RunSpecIterator(AbstractRunSpec spec, Sandbox sandbox, Validator validator, boolean haltOnFailure, IMessage.Kind failureKind) {
    ...
    public boolean hasNextRun() {
        return ((specIndex < childSpecs.size()) || ((null != childIterator) && childIterator.hasNextRun()));
    }
                        ...
                        if (null == sandbox) {
                            ...
                            if (null == iter) {
                                ...
                                if ((iter instanceof WrappedRunIterator)) {
                                    if (!iter.hasNextRun()) {
                                        error = "child \"" + childSpec + "\".hasNextRun()" + " is not true - should be exactly one run";
                                    } else {
                                        result = iter.nextRun(handler, runner);
                                        if (null == result) {
                                            error = "child \"" + childSpec + "\".nextRun()" + " returned null - should be exactly one run";
                                        } else {
                                            childIterator = null;
                                            return result;
                                        }
