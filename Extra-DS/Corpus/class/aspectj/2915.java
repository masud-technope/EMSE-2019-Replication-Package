...
public abstract class AbstractStandard extends com.sun.tools.doclets.standard.Standard {
        ...
        class MyIndexBuilder extends IndexBuilder {
            ...
            protected void putMembersInIndexMap(ClassDoc classdoc) {
                ...
                if (classdoc instanceof org.aspectj.ajdoc.ClassDoc) {
                    org.aspectj.ajdoc.ClassDoc cd = (org.aspectj.ajdoc.ClassDoc) classdoc;
    ...
    protected void startGeneration(RootDoc root) throws DocletAbortException {
        if (!generateCheckPasses(getConfiguration(), root))
            return;
        performCopy(getConfiguration().destdirname, getConfiguration().helpfile);
        performCopy(getConfiguration().destdirname, getConfiguration().stylesheetfile);
        classtree = new ClassTree(root, getConfiguration().nodeprecated);
        generatePrePasses(getConfiguration(), root);
        generatePackageCycle(getConfiguration().packages, getConfiguration().createtree, getConfiguration().nodeprecated);
        ...
        public final void generate(AbstractStandard std, ConfigurationStandard cs, RootDoc root) throws DocletAbortException {
            ...
            if (cond()) {
                ...
                if (cs.log && title != null) {
                    ...
                    cs.standardmessage.notice("doclet.done_msg", title, (stop - start) + "");
                }
            }
        }
    ...
    private final void generatePasses(ConfigurationStandard cs, RootDoc root, Class[] classes) throws DocletAbortException {
        if (classes == null)
            return;
        nextClass: for (int i = 0; i < classes.length; i++) {
            try {
                Constructor[] ctrs = classes[i].getConstructors();
                nextCtr: for (int j = 0; j < ctrs.length; j++) {
                    Pass pass = null;
                ...
                e.printStackTrace();
                Standard.configuration().standardmessage.error("doclet.exception", e + "");
                throw new DocletAbortException();
            }
        }
    }
    ...
    protected void generatePackageCycle(PackageDoc[] pkgs, boolean createtree, boolean nodeprecated) throws DocletAbortException {
        ...
        for (int i = 0; i < pkgs.length; i++) {
            ...
            generatePackages(prev, curr, next, createtree, nodeprecated);
        }
    }
     ...
     */
    protected void generateClassCycle(ClassDoc[] cs, ClassTree classtree, boolean nopackage) throws DocletAbortException {
        Arrays.sort(cs);
        for (int i = 0; i < cs.length; i++) {
            if (configuration().nodeprecated && cs[i].tags("deprecated").length > 0) {
                continue;
            }
            ClassDoc prev = i == 0 ? null : cs[i - 1];
            ClassDoc curr = cs[i];
            ClassDoc next = i == cs.length - 1 ? null : cs[i + 1];
            generateClasses(prev, curr, next, classtree, nopackage);
        }
    }
     ...
     */
    protected void generateClasses(ClassDoc prev, ClassDoc curr, ClassDoc next, ClassTree classtree, boolean nopackage) throws DocletAbortException {
