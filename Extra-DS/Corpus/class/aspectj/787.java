...
public class ProgramElement implements IProgramElement {
    ...
    // XXX is this right?
    private static int AccPrivileged = 0x0006;
	 ...
	 */
    public  ProgramElement(String name, Kind kind, int modifiers, //Accessibility accessibility,
    String declaringType, String packageName, String comment, ISourceLocation sourceLocation, List relations, List children, boolean member) {
        this(name, kind, children);
        this.sourceLocation = sourceLocation;
        this.kind = kind;
        this.modifiers = modifiers;
    ...
    public Accessibility getAccessibility() {
        // accessibility
        return genAccessibility(modifiers);
    }
    ...
    public void setParameterTypes(List list) {
        if (kvpairs == Collections.EMPTY_MAP)
            kvpairs = new HashMap();
        if (list == null || list.size() == 0)
            kvpairs.put("parameterTypes", Collections.EMPTY_LIST);
        else
            kvpairs.put("parameterTypes", list);
    //		parameterTypes = list; 
    }
