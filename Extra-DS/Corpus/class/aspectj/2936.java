...
public class AjcMessageHandler extends MessageHandler {
    ...
    public CompilerDiffs getCompilerDiffs() {
        if (null == diffs) {
            ...
            if (!ignoreWarnings) {
                ...
                expected = Arrays.asList(expectedMessages.getMessages(IMessage.ERROR, true));
            ...
            boolean usingNew = true;
            if (usingNew) {
                final IMessage.Kind[] NOSKIPS = new IMessage.Kind[0];
                IMessage.Kind[] skipActual = new IMessage.Kind[] { IMessage.INFO };
                int expectedInfo = MessageUtil.numMessages(expected, IMessage.INFO, false);
                if (0 < expectedInfo) {
                    // fyi, when expecting any info messages, have to expect all
                    skipActual = NOSKIPS;
                }
                messages = Diffs.makeDiffs("message", (IMessage[]) expected.toArray(new IMessage[0]), (IMessage[]) actual.toArray(new IMessage[0]), NOSKIPS, skipActual);
            } else {
                messages = Diffs.makeDiffs("message", expected, actual, COMP_IMessage, Diffs.ACCEPT_ALL, CompilerDiffs.SKIP_UNEXPECTED_INFO);
            }
            Diffs recompiled = Diffs.makeDiffs("recompiled", expectedRecompiled, actualRecompiled, COMP_File);
    ...
    private IMessage[] getMessagesWithoutExpectedFails() {
        ...
        for (int i = 0; i < result.length; i++) {
            if ((0 == leftToFilter) || !IMessage.FAIL.isSameOrLessThan(result[i].getKind())) {
                list.add(result[i]);
            } else {
    ...
    private // LangUtil instead?
    void render(StringBuffer result, String prefix, String suffix, List items) {
        if ((null != items)) {
