...
public class BuildCancellingTest extends AjdeTestCase {
    ...
    public void testCancelFirstCompile() {
        ...
        programmableBPM.cancelOn("compiled:", 1);
        compilerAdapter.compile((String) openFile("LoadsaCode.lst").getAbsolutePath(), programmableBPM, false);
        assertTrue("Should have cancelled after first compile?:" + programmableBPM.numCompiledMessages, programmableBPM.numCompiledMessages == 1);
    ...
    public void testCancelThirdCompile() {
        ...
        programmableBPM.cancelOn("compiled:", 3);
        compilerAdapter.compile((String) openFile("LoadsaCode.lst").getAbsolutePath(), programmableBPM, false);
        assertTrue("Should have cancelled after third compile?:" + programmableBPM.numCompiledMessages, programmableBPM.numCompiledMessages == 3);
        //		Comment out to check the disk contents 		  
        //		assertTrue("As weaving was cancelled, no files should have been written out, but I found:"+wovenClassesFound(),
        //		  wovenClassesFound()==0);
        boolean expectedCancelMessageFound = checkFor("Compilation cancelled as requested");
    ...
    public void testCancelFirstAspectWeave() {
        ...
        programmableBPM.cancelOn("woven aspect ", 1);
        compilerAdapter.compile((String) openFile("LoadsaCode.lst").getAbsolutePath(), programmableBPM, false);
        assertTrue("Should have cancelled after first aspect weave?:" + programmableBPM.numWovenAspectMessages, programmableBPM.numWovenAspectMessages == 1);
        // 		Comment out to check the disk contents
    ...
    public void testCancelThirdAspectWeave() {
        ...
        compilerAdapter.compile((String) openFile("LoadsaCode.lst").getAbsolutePath(), programmableBPM, false);
        assertTrue("Should have cancelled after third weave?:" + programmableBPM.numWovenAspectMessages, programmableBPM.numWovenAspectMessages == 3);
    ...
    public void testCancelFirstClassWeave() {
        ...
        compilerAdapter.compile((String) openFile("EvenMoreCode.lst").getAbsolutePath(), programmableBPM, false);
        ...
        assertTrue("Should have cancelled after first class weave?:" + programmableBPM.numWovenClassMessages, programmableBPM.numWovenClassMessages == 1);
        boolean expectedCancelMessageFound = checkFor("Weaving cancelled as requested");
        // Useful for debugging
        if (!expectedCancelMessageFound)
            dumpTaskData();
    ...
    public void testCancelSecondClassWeave() {
        ...
        assertTrue("Should have cancelled after first class weave?:" + programmableBPM.numWovenClassMessages, programmableBPM.numWovenClassMessages == 2);
        boolean expectedCancelMessageFound = checkFor("Weaving cancelled as requested");
        // Useful for debugging
        if (!expectedCancelMessageFound)
            dumpTaskData();
    ...
    private class BuildProgMon implements BuildProgressMonitor {
        ...
        public void setProgressText(String text) {
            ...
            if (text.startsWith("compiled:"))
                numCompiledMessages++;
            if (programmableString != null && text.indexOf(programmableString) != -1) {
                count--;
                if (count == 0) {
                    if (debugTests)
        ...
        public boolean containsMessage(String prefix, String distinguishingMarks) {
            for (Iterator iter = messagesReceived.iterator(); iter.hasNext(); ) {
                ...
                if (element.startsWith(prefix) && element.indexOf(distinguishingMarks) != -1)
                    return true;
            }
            return false;
        }
