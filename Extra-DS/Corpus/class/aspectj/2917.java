...
public class WeaveTests {
    ...
    private static void parseArgs(String[] args) {
        ...
        while (i < args.length) {
            String arg = args[i++];
            try {
                if (// Number of classes
                arg.equals("-c")) {
                    if (i < args.length)
                        NUMCLASSES = Integer.parseInt(args[i++]);
                    else
                        error = true;
                } else if (// Number of methods
                arg.equals("-m")) {
                    if (i < args.length)
                        NUMMETHODS = Integer.parseInt(args[i++]);
                    else
                        error = true;
                } else if (// Number of method lines
                arg.equals("-l")) {
                    if (i < args.length)
                        METHODLINES = Integer.parseInt(args[i++]);
                    else
                        error = true;
                } else if (// Number of iterations
                arg.equals("-i")) {
                    if (i < args.length)
                    ...
                    String advice = args[i++];
                    if (advice.equals("fast"))
                        TEST_ONE = true;
                    else if (advice.equals("meduim"))
                        TEST_ONE_SEARCH_ALL = true;
                    else if (advice.equals("slow"))
                        TEST_ALL = true;
                    else
                        error = true;
    ...
    public static void createClasses() throws IOException {
        ...
        if (INCLUDE_TEST_CLASSES) {
            ...
            try {
                ...
                testFiles.append("\tpublic static void main(String args[]) {" + NL);
        ...
        for (int i = 0; i < NUMCLASSES; i++) buildList.append("C" + i + ".java" + NL);
        if (INCLUDE_TEST_CLASSES) {
    ...
    public static void compileClasses() throws IOException {
        ...
        if (ECHO)
            System.out.println((average / ITERATIONS) + " millis");
    }

    private static long performCompile() throws IOException {
        String ajcargs = "-noExit -outjar " + OUTPUT_PACKAGE + File.separatorChar + "classes.jar " + "-argfile " + OUTPUT_PACKAGE + File.separatorChar + "build.lst";
    ...
    public static void weaveAspects(String adviceType) throws IOException {
        ...
        long average = 0;
        if (ECHO)
            System.out.println((char) (adviceType.charAt(0) - 32) + adviceType.substring(1));
        if (ALL_POINTCUT_TESTS || TEST_ONE) {
            if (ECHO)
    ...
    private static long weaveAllAspects(String adviceType, int pointcut) throws IOException {
        ...
        String ajcargs = "-noExit -injars " + OUTPUT_PACKAGE + File.separatorChar + "classes.jar " + "-outjar " + OUTPUT_PACKAGE + File.separatorChar + adviceName + ".jar " + OUTPUT_PACKAGE + File.separatorChar + adviceName + ".aj";
        String[] parsedArgs = RunWeaveTests.split(ajcargs);
        long start = System.currentTimeMillis();
        org.aspectj.tools.ajc.Main.main(parsedArgs);
        long stop = System.currentTimeMillis();
