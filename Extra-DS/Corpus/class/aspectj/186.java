...
public class IncCompilerRun implements IAjcRun {
    ...
    public boolean setupAjcRun(Sandbox sandbox, Validator validator) {
        ...
        if (!validator.nullcheck(sandbox, "sandbox") || !validator.nullcheck(spec, "spec") || !validator.nullcheck(spec.tag, "fileSuffix")) {
            return false;
        }
        File srcDir = sandbox.getTestBaseSrcDir(this);
        File destDir = sandbox.stagingDir;
        if (!validator.canReadDir(srcDir, "testBaseSrcDir") || !validator.canReadDir(destDir, "stagingDir")) {
            return false;
        }
    ...
    boolean doStaging(final Validator validator) {
        ...
        try {
            ...
            doStaging(validator, ".properties", holder, copied);
            // arbitrary resource extension
            doStaging(validator, ".xml", holder, copied);
    ...
    private void doStaging(final Validator validator, final String toSuffix, final intHolder holder, final List copied) throws IOException {
        final String fromSuffix = "." + spec.tag + toSuffix;
    ...
    public boolean run(IRunStatus status) {
        ...
        try {
            handler.init();
            if (spec.fresh) {
                if (compiler instanceof CompileCommand) {
                    // urk
            ...
            if (!handlerResult) {
                ...
                if (!result) {
                    String m = commandResult ? "incremental compile command did not return false as expected" : "incremental compile command returned false unexpectedly";
            ...
            if (!result || spec.runtime.isVerbose()) {
    ...
    public static class Spec extends AbstractRunSpec {
        ...
        public void addDirChanges(DirChanges.Spec spec) {
            // XXX copy/paste of CompilerRun.Spec...
            if (null == spec) {
                return;
            }
