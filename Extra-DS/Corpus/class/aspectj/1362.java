...
abstract class SignatureImpl implements Signature {
    ...
    String toString(StringMaker sm) {
        ...
        if (toStringCacheRef == null || toStringCacheRef.get() == null) {
            ...
            if (useCache)
                toStringCacheRef = new SoftReference(toStringCache);
        } else {
    ...
    String fullTypeName(Class type) {
        if (type == null)
            return "ANONYMOUS";
        if (type.isArray())
            return fullTypeName(type.getComponentType()) + "[]";
    ...
    private ClassLoader getLookupClassLoader() {
        if (lookupClassLoader == null)
            lookupClassLoader = this.getClass().getClassLoader();
