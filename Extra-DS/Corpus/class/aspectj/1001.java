...
public class AjTypeImpl<T> implements AjType<T> {
    ...
    public Field[] getDeclaredFields() {
        ...
        for (Field field : fields) if (!field.getName().startsWith(ajcMagic) && !field.isAnnotationPresent(DeclareWarning.class) && !field.isAnnotationPresent(DeclareError.class)) {
    ...
    public Pointcut getDeclaredPointcut(String name) throws NoSuchPointcutException {
        ...
        for (Pointcut pc : pcs) if (pc.getName().equals(name))
            return pc;
                ...
                if (f.isAnnotationPresent(org.aspectj.lang.annotation.DeclareParents.class)) {
                    ...
                    for (Method itdM : f.getType().getDeclaredMethods()) {
                        if (!Modifier.isPublic(itdM.getModifiers()) && publicOnly)
                            continue;
                        InterTypeMethodDeclaration itdm = new InterTypeMethodDeclarationImpl(this, AjTypeSystem.getAjType(f.getType()), itdM, Modifier.PUBLIC);
                        toList.add(itdm);
                    }
                }
            }
    ...
    public DeclarePrecedence[] getDeclarePrecedence() {
        ...
        if (clazz.isAnnotationPresent(org.aspectj.lang.annotation.DeclarePrecedence.class)) {
            org.aspectj.lang.annotation.DeclarePrecedence ann = clazz.getAnnotation(org.aspectj.lang.annotation.DeclarePrecedence.class);
