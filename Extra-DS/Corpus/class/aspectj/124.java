...
public class ClassGen extends AccessFlags implements Cloneable {
   ...
   */
    public  ClassGen(JavaClass clazz) {
        ...
        for (int i = 0; i < annotations.length; i++) addAnnotation(annotations[i]);
   ...
   */
    private AnnotationGen[] unpackAnnotations(Attribute[] attrs) {
        List /*AnnotationGen*/
        annotationGenObjs = new ArrayList();
   ...
   */
    public void update() {
        if (observers != null)
            for (Iterator e = observers.iterator(); e.hasNext(); ) ((ClassObserver) e.next()).notify(this);
    }
