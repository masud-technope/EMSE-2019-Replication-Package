...
public class TreeCompare {
    ...
    public static void main(String[] args) {
        ...
        if ((args.length > 0) || (!expected.exists() || (!actual.exists()))) {
    ...
    public void compareSnapshots(File expected, File actual) {
        try {
            ...
            //                }
            //            } else {
            System.err.println("Unrecognized objects - expected: " + expectedObject + " actual: " + actualObject);
        //            }
        } catch (Throwable t) {
    ...
    public void signalDifferentTypes(Object lhs, Object rhs) {
        ...
        String err = "Different Types? lhs: " + lhc + "=" + lhs + " rhs: " + rhc + "=" + rhs;
        throw new Error(err);
    }
    ...
    public void compareSnapshots(TreeNode expected, TreeNode actual) {
        try {
            GenericTreeNodeFactoryI factory = JTreeNodeGenericTreeNodeFactory.SINGLETON;
            // this is the custom part: adapter generating generic model
            GenericTreeNode expectRoot = factory.createGenericTreeNode(expected, null);
            GenericTreeNode actualRoot = factory.createGenericTreeNode(actual, null);
            if (null == actualRoot)
                System.err.println("null actualRoot");
            if (null == expectRoot)
                System.err.println("null expectRoot");
            compareSnapshots(expectRoot, actualRoot);
        } catch (Throwable t) {
            System.err.println("TEST FAILED: " + t.getMessage());
            t.printStackTrace(System.err);
