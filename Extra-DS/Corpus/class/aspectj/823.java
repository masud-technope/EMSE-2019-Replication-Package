...
public class AjcTest extends RunSpecIterator {
    ...
    public static class Spec extends AbstractRunSpec {
         ...
         */
        private static final XMLNames NAMES = new XMLNames(XMLNames.DEFAULT, "title", null, null, null, "", null, "", "", true, false, false);
        ...
        private static final List VALID_SUFFIXES = Collections.unmodifiableList(Arrays.asList(new String[] { TITLE_LIST, TITLE_FAIL_LIST, TITLE_CONTAINS, REQUIRE_KEYWORDS, SKIP_KEYWORDS, PICK_PR }));
        ...
        private static List makeTitlesList(String titlesKey, boolean fail) {
            ...
            return file.canRead() ? readTitlesFile(file, fail) : parseTitlesList(titlesKey);
        }
        ...
        private static List parseTitlesList(String titlesList) {
            ...
            if (null != last) {
                // XXX messages
                String m = "unterminated entry \"" + last;
        ...
        private static List readTitlesFile(File titlesFile, boolean fail) {
            ...
            try {
                ...
                while (null != (line = lines.readLine())) {
                    if ((line.startsWith("FAIL ") || (!fail && line.startsWith("PASS "))) && (!line.substring(5).startsWith("Suite.Spec("))) {
                ...
                System.err.println("ignoring titles in " + titlesFile);
        ...
        public IRunIterator makeRunIterator(Sandbox sandbox, Validator validator) {
            ...
            if (null == suiteDir) {
                ...
                if (!validator.nullcheck(loc, "suite file location") || !validator.nullcheck(loc.getSourceFile(), "suite file")) {
                    return null;
                }
        ...
        protected boolean doAdoptParentValues(RT parentRuntime, IMessageHandler handler) {
            ...
            for (int i = 0; i < globalOptions.length; i++) {
                ...
                if (option.startsWith(REQUIRE_KEYWORDS)) {
                    ...
                    throw new Error("unrecognized suffix: " + globalOptions[i] + " (expecting: " + OPTION_PREFIX + VALID_SUFFIXES + "...)");
                }
                if (null != permittedTitleStrings) {
                    boolean gotHit = false;
                    for (Iterator iter = permittedTitleStrings.iterator(); !gotHit && iter.hasNext(); ) {
                    ...
                    if (!gotHit) {
                        String reason = "title " + this.description + " does not contain any of " + option;
                        skipMessage(handler, reason);
                        return false;
                    }
                } else if (null != permittedTitles) {
                    if (!permittedTitles.contains(this.description)) {
                        String reason = "titlesList " + option + " did not contain " + this.description;
                        skipMessage(handler, reason);
                        return false;
                    }
                } else {
                    ...
                    for (Iterator iter = specs.iterator(); iter.hasNext(); ) {
                        ...
                        if (null != havePr) {
                            if (havePr.equals(spec)) {
                                // String.equals()
                                havePr = null;
                            }
                        } else if (keywordMustExist != keywords.contains(spec)) {
                            String reason = "keyword " + spec + " was " + (keywordMustExist ? "not found" : "found");
                            skipMessage(handler, reason);
                            return false;
                        }
                    }
