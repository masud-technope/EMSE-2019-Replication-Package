        ...
        panes = new WizardPane[] { new IntroPane(), new LocationPane() {
            ...
            public String getDefaultLocation() {
                if (context.onWindows()) {
                    ...
                    String[] paths = { "c:\\JBuilder6\\lib\\ext", "c:\\apps\\JBuilder6\\lib\\ext", "c:\\Program Files\\JBuilder6\\lib\\ext" };
                    int pathIndex = 0;
            ...
            public void verify() {
                ...
                if (!jbuilder.exists() && hasGui()) {
                    int ret = JOptionPane.showConfirmDialog(frame, "The location you specified does not seem to be a " + "valid JBuilder install directory." + " Continue?", "Confirm Install", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                ...
                if (oldFile.exists() && hasGui()) {
                    int ret = JOptionPane.showConfirmDialog(frame, "This old version of AJDE for JBuilder (\"ajbuilder.jar\") exists" + " and must be removed from the install directory." + " OK to delete?", "Confirm Delete", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                    if (ret != JOptionPane.YES_OPTION) {
                        Main.exit(-1);
                    } else {
                        oldFile.delete();
        ...
        panes = new WizardPane[] { new IntroPane(), new LocationPane() {
            ...
            public void verify() {
                ...
                if (!forte.exists() && hasGui()) {
                    int ret = JOptionPane.showConfirmDialog(frame, "The location you specified does not seem to be a " + "valid Forte install directory." + " Continue?", "Confirm Install", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                    if (ret != JOptionPane.YES_OPTION) {
                        Main.exit(-1);
                    } else {
                    // do nothing
            ...
            public void finalActions() {
                // todo verify dir ../lib/ext exists?
...
abstract class Installer {
    ...
    public ActionListener makeCancelAction(WizardPane pane) {
        return new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                int ret = JOptionPane.showConfirmDialog(frame, EXIT_MESSAGE, EXIT_TITLE, JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                if (ret == JOptionPane.YES_OPTION) {
                    Main.exit(-1);
                }
            }
...
class InstallContext {
    ...
    static final String[] OVERWRITE_OPTIONS = { //, "No to all"
    "Yes", //, "No to all"
    "No", //, "No to all"
    "Yes to all" };
    ...
    boolean shouldOverwrite(final File file) {
        ...
        try {
            SwingUtilities.invokeAndWait(new Runnable() {
                ...
                public void run() {
                    int ret = JOptionPane.showOptionDialog(installer.frame, OVERWRITE_MESSAGE + file.getPath(), OVERWRITE_TITLE, JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, OVERWRITE_OPTIONS, OVERWRITE_OPTIONS[OVERWRITE_YES]);
                    overwriteState = ret;
                }
            });
...
class LaunchScriptMaker {
    ...
    private void writeAJWindowsLaunchLine(PrintStream ps) {
        ps.println("\"%JAVA_HOME%\\bin\\java\" -classpath " + "\"%ASPECTJ_HOME%\\lib\\aspectjweaver.jar\"" + " \"-Djava.system.class.loader=org.aspectj.weaver.loadtime.WeavingURLClassLoader\"" + " \"-Daj.class.path=%ASPECTPATH%;%CLASSPATH%\"" + " \"-Daj.aspect.path=%ASPECTPATH%\"" + " " + makeScriptArgs(false));
    }
    ...
    private void writeAJ5WindowsLaunchLine(PrintStream ps) {
        ps.println("\"%JAVA_HOME%\\bin\\java\" -classpath " + "\"%ASPECTJ_HOME%\\lib\\aspectjweaver.jar;%CLASSPATH%\"" + " \"-javaagent:%ASPECTJ_HOME%\\lib\\aspectjweaver.jar\"" + " " + makeScriptArgs(false));
    }
    ...
    private void writeAJUnixLaunchLine(PrintStream ps) {
        ps.println("\"$JAVA_HOME/bin/java\" -classpath" + " \"$ASPECTJ_HOME/lib/aspectjweaver.jar\"" + " \"-Djava.system.class.loader=org.aspectj.weaver.loadtime.WeavingURLClassLoader\"" + " \"-Daj.class.path=$ASPECTPATH:$CLASSPATH\"" + " \"-Daj.aspect.path=$ASPECTPATH\"" + " " + makeScriptArgs(true));
    }
    ...
    private void writeAJ5UnixLaunchLine(PrintStream ps) {
        ps.println("\"$JAVA_HOME/bin/java\" -classpath" + " \"$ASPECTJ_HOME/lib/aspectjweaver.jar:$CLASSPATH\"" + " \"-javaagent:$ASPECTJ_HOME/lib/aspectjweaver.jar\"" + " " + makeScriptArgs(true));
    }
    ...
    private void writeWindowsLaunchLine(String className, PrintStream ps) {
        ps.println("\"%JAVA_HOME%\\bin\\java\" -classpath " + //                   "\"%ASPECTJ_HOME%\\lib\\aspectjtools.jar;%CLASSPATH%\""+
    ...
    private void writeUnixLaunchLine(String className, PrintStream ps) {
        ...
        ps.println("\"$JAVA_HOME/bin/java\" -classpath " + "\"$ASPECTJ_HOME/lib/aspectjtools.jar" + sep + "$JAVA_HOME/lib/tools.jar" + sep + "$CLASSPATH\"" + " -Xmx64M " + className + " " + makeScriptArgs(true));
    }
    ...
    public String makeClassPath(boolean unixStyle) throws IOException {
        return context.toolsJarPath.getAbsolutePath() + File.pathSeparator + new File(new File(context.getOutputDir(), "lib"), "aspectjtools.jar").getAbsolutePath() + File.pathSeparator + makeClassPathVar(unixStyle);
    }
