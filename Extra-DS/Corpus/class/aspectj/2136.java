...
public class InteractiveBrowserTest extends TestCase {
    ...
    public void testAddProjectTask() {
        ...
        Ajde.getDefault().getTaskListManager().addProjectTask("project-level task", IMessage.ERROR);
    ...
    public void testAddSourceLineTasks() {
        ...
        Ajde.getDefault().getTaskListManager().addSourcelineTask("error task", dummyLocation, IMessage.ERROR);
        Ajde.getDefault().getTaskListManager().addSourcelineTask("warning task", dummyLocation, IMessage.WARNING);
        Ajde.getDefault().getTaskListManager().addSourcelineTask("info task", dummyLocation, IMessage.INFO);
        BrowserManager.getDefault().showMessages();
    ...
    private boolean verifySuccess(String message) {
        int result = JOptionPane.showConfirmDialog(BrowserManager.getDefault().getRootFrame(), "Verify Results", message, JOptionPane.YES_NO_OPTION);
