...
public class RunUtils {
    ...
    public static void enableVerbose(AbstractRunSpec spec) {
        // instanceof hack
    ...
    public static void print(PrintStream out, String prefix, IRunStatus status, IMessageRenderer renderer, IMessageHandler selector) {
        String label = status.getIdentifier() + (status.runResult() ? "PASS" : "FAIL");
    ...
    public static String debugString(IRunStatus status) {
        ...
        final String[] LABELS = new String[] { "runResult", "id", "result", "numChildren", "completed", //"parent",
        "abort", "started", "thrown", "messages" };
        ...
        Object[] values = new Object[] { runResult, status.getIdentifier(), status.getResult(), numChildren, new Boolean(status.isCompleted()), //status.getParent(),               // costly if parent printing us
        status.getAbortRequest(), new Boolean(status.started()), thrownString, numMessages };
    ...
    public static final IRunStatusPrinter VERBOSE_PRINTER = new IRunStatusPrinter() {
        ...
        private void printRunStatus(PrintStream out, IRunStatus status, String prefix) {
            ...
            String label = (status.runResult() ? " PASS: " : " FAIL: ") + status.getIdentifier();
            out.println(prefix + "------------ " + label);
    ...
    public static final IRunStatusPrinter AJCSUITE_PRINTER = new IRunStatusPrinter() {
        ...
        public void printRunStatus(PrintStream out, IRunStatus status) {
            ...
            label = "ran " + children.length + " tests" + (numFails == 0 ? "" : "(" + numFails + " fails)");
            out.println("");
        }
    };
        ...
        public void printRunStatus(PrintStream out, IRunStatus status) {
            ...
            if (0 < numFails) {
                ...
                for (int i = 0; i < fails.length; i++) {
                    out.println("[fail " + LangUtil.toSizedString(i, 3) + "]: " + MessageUtil.MESSAGE_ALL.renderToString(fails[i]));
                }
            }
        }
    };
