...
public class AjcSpecXmlReader {
    ...
    public static final String DOCTYPE = "<!DOCTYPE " + AjcTest.Suite.Spec.XMLNAME + " SYSTEM \"" + DTD_PATH + "\">";
    ...
    public static void writeDTD(File dtdFile) throws IOException {
        ...
        try {
            out.println("<!-- document type for ajc test suite - see " + AjcSpecXmlReader.class.getName() + " -->");
    ...
    public AjcTest.Suite.Spec readAjcSuite(File file) throws IOException, AbortException {
        ...
        // trace debug XXX
        System.setProperty("org.apache.commons.logging.simplelog.defaultlog", LOG[logLevel]);
    ...
    private void setupDigester(Digester digester) {
        ...
        // ok to have suite messages and global suite options, etc.
        digester.addSetProperties(suiteX);
        digester.addSetProperties(ajctestX, new String[] { "title", "dir", "pr" }, new String[] { "description", "testDirOffset", "bugId" });
        digester.addSetProperties(compileX, new String[] { "files", "argfiles" }, new String[] { "paths", "argfiles" });
        digester.addSetProperties(compileX + "/file");
        digester.addSetProperties(inccompileX, "classes", "paths");
        digester.addSetProperties(runX, new String[] { "class", "vm", "skipTester", "fork", "vmargs", "aspectpath" }, new String[] { "className", "javaVersion", "skipTester", "fork", "vmArgs", "aspectpath" });
    ...
    static BProps[] expectedProperties() {
        return new BProps[] { new BProps(AjcTest.Suite.Spec.class, // verbose removed
        new String[] { "suiteDir" }), new BProps(AjcTest.Spec.class, new String[] { "description", "testDirOffset", "bugId" }), // mapped from { "title", "dir", "pr"}
        new BProps(CompilerRun.Spec.class, new String[] { "files", "options", "staging", "badInput", "reuseCompiler", "includeClassesDir", "argfiles", "aspectpath", "classpath", "extdirs", "sourceroots", "xlintfile", "outjar" }), new BProps(IncCompilerRun.Spec.class, new String[] { "tag" }), new BProps(JavaRun.Spec.class, new String[] { "className", "skipTester", "options", "javaVersion", "errStreamIsError", "outStreamIsError", "fork", "vmArgs", "aspectpath" }), // mapped from { "class", ...}
        new BProps(DirChanges.Spec.class, new String[] { "added", "removed", "updated", "unchanged", "dirToken", "defaultSuffix" }), //                    new String[] { "path"}),
        new BProps(SoftMessage.class, new String[] { "kindAsString", "lineAsString", "text", "details", "file" }) };
    }
    ...
    static class SuiteResolver implements EntityResolver {
        ...
        private String getPath(String id) {
            ...
            if (isDir(suiteFileDir)) {
                String path = suiteFileDir.getPath() + "/" + id;
            ...
            final File[] baseDirs = new File[] { suiteFileDir, new File(".") };
            final String[] locations = new String[] { ".", "..", "../tests", "../../tests", "../../../tests", "tests", "modules/tests" };
            File baseDir;
        ...
        public InputSource resolveEntity(String publicId, String systemId) throws SAXException {
            ...
            if ((null != systemId) && systemId.endsWith(NAME)) {
