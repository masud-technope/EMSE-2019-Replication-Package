...
public class AbstractSubWriter extends com.sun.tools.doclets.standard.AbstractSubWriter implements AbstractSubWriterAJ {
    ...
    public  AbstractSubWriter(com.sun.tools.doclets.standard.SubWriterHolderWriter writer, ClassDoc classdoc) {
        super(writer, classdoc);
    }

    public  AbstractSubWriter(com.sun.tools.doclets.standard.SubWriterHolderWriter writer) {
        super(writer);
    }
    ...
    protected final void setDelegator() {
        ...
        try {
            ...
            if (classdoc != null)
                list.add(classdoc);
            setDelegator(mw = findDelegate(list.toArray(new Object[list.size()])));
        } finally {
            del = mw;
        }
    }
    ...
    private final void setDelegator(Object o) {
        ...
        try {
            ...
            Standard.configuration().standardmessage.error("doclet.exception_encountered", e + "");
        }
    }

    private final com.sun.tools.doclets.standard.AbstractSubWriter findDelegate(Object[] params) {
        ...
        try {
            ...
            for (int i = 0; i < cs.length; i++) {
                if (cs[i].getParameterTypes().length == params.length) {
                    return (com.sun.tools.doclets.standard.AbstractSubWriter) cs[i].newInstance(params);
                }
            }
        } catch (Exception e) {
            Standard.configuration().standardmessage.error("doclet.exception_encountered", e + "");
    ...
    protected final void summaryLabel(String type, ClassDoc cd) {
        writer.bold();
        writer.printText("doclet." + propertyName() + "s_" + type, Statics.type(cd), writer.getPreQualifiedClassLink(cd));
        writer.boldEnd();
    }
    ...
    protected final void navLink(ClassDoc cd, String keyName, boolean link, String kind) {
        if (link) {
            writer.printHyperLink("", cd == null ? keyName.toLowerCase() + "_" + kind : keyName.toLowerCase() + "s_inherited_from_class_" + cd.qualifiedName(), writer.getText(navKey(keyName)));
        } else {
    ...
    protected final void header(String keyName) {
        ...
        writer.printTableHeadingBackground(writer.getText("doclet." + keyName + "_Detail"));
    }
    ...
    protected List nointros(List members) {
        ...
        for (Iterator i = members.iterator(); i.hasNext(); ) {
            ...
            if (o instanceof org.aspectj.ajdoc.MemberDoc) {
                IntroducedDoc intro = ((org.aspectj.ajdoc.MemberDoc) o).introduced();
    ...
    protected final void printSummaryLinkComment(ProgramElementDoc member) {
        ...
        boolean newline = member.tags("deprecated").length + member.firstSentenceTags().length > 0;
        if (classdoc != null || true) {
            // ?? always
            if (hasCrosscuts(classdoc, member)) {
                if (newline) {
                    writer.br();
    ...
    public void printIntroducedSummaryMember(ClassDoc cd, ProgramElementDoc member) {
        printIntroducedSummaryLink(cd, member);
    }

    public void printIntroducedSummaryLink(ClassDoc cd, ProgramElementDoc member) {
    }
