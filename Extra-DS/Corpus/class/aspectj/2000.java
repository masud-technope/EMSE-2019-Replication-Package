...
public class IntroducedDocImpl extends IntroductionDocImpl implements IntroducedDoc {
    ...
    public  IntroducedDocImpl(com.sun.javadoc.ClassDoc containingClass, IntroducedDec introducedDec) {
    ...
    protected void createTargets() {
        ...
        if (affects.size() < 1)
            return;
        nextType: for (Iterator it = affects.iterator(); it.hasNext(); ) {
            Object o = it.next();
            if (o instanceof TypeDec) {
                ...
                for (int i = 0; i < fs.length; i++) {
                    if (member.weakEquals(fs[i])) {
                        // XXX weakEquals is unimplemented
                        ((FieldDocImpl) fs[i]).setIntroduced(this);
                        addTarget(cd);
                        ((FieldDocImpl) member).setType(((FieldDocImpl) fs[i]).fieldDec().getType());
                ...
                for (int i = 0; i < ms.length; i++) {
                    if (member.weakEquals(ms[i])) {
                        ...
                        addTarget(cd);
                        ((MethodDocImpl) member).setType(((MethodDocImpl) ms[i]).codeDec().getResultTypeD().getType());
                        ((ExecutableMemberDocImpl) member).makeParameters(((MethodDocImpl) ms[i]).codeDec().getFormals());
                        continue nextType;
                    }
                }
                ...
                for (int i = 0; i < cs.length; i++) {
                    if (member.weakEquals(cs[i])) {
                        ...
                        addTarget(cd);
                        ((ExecutableMemberDocImpl) member).makeParameters(((ConstructorDocImpl) cs[i]).codeDec().getFormals());
                        continue nextType;
                    }
                }
            }
    ...
    private MemberDocImpl findMember() {
        ...
        // PR790, 712
        dec.setSourceLocation(introducedDec.getSourceLocation());
