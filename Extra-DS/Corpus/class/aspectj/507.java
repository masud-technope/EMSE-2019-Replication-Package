...
public class AspectClinit extends Clinit {
    ...
    protected void generateSyntheticCode(ClassScope classScope, CodeStream codeStream) {
        ...
        if (hasPre) {
            ...
            codeStream.invokestatic(world.makeMethodBindingForCall(AjcMemberMaker.ajcPreClinitMethod(world.fromBinding(classScope.referenceContext.binding))));
        }
    ...
    protected void generatePostSyntheticCode(ClassScope classScope, CodeStream codeStream) {
        ...
        if (hasPost) {
            final EclipseFactory world = EclipseFactory.fromScopeLookupEnvironment(classScope);
            codeStream.invokestatic(world.makeMethodBindingForCall(AjcMemberMaker.ajcPostClinitMethod(world.fromBinding(classScope.referenceContext.binding))));
        }
        if (initFailureField != null) {
