...
public class SerialFieldTagImpl extends TagImpl implements SerialFieldTag, Comparable {
     ...
     */
    public  SerialFieldTagImpl(Doc doc, String name, String text, Locale loc, ErrPrinter err) {
    ...
    private void resolveNames(String str) {
        ...
        if (i < N && !start(str.charAt(i++))) {
            err().error("serialField_tag_invalid_field_name_start", "" + str.charAt(i));
            return;
        }
        // Find the rest of the field-name
        while (i < N && !space(str.charAt(i))) {
            if (!ident(str.charAt(i))) {
                err().error("serialField_tag_invalid_field_name_part", "" + str.charAt(i));
                return;
            }
            i++;
        }
        ...
        start = i;
        if (i < N && !start(str.charAt(i++))) {
            err().error("serialField_tag_invalid_type_name_start", "" + str.charAt(i));
            return;
        }
        // Find the rest of the field-name
        while (i < N && !space(str.charAt(i))) {
            if (!(str.charAt(i) == '[' || str.charAt(i) == ']' || ident(str.charAt(i)))) {
                err().error("serialField_tag_invalid_type_name_part", "" + str.charAt(i));
                return;
            }
        }
