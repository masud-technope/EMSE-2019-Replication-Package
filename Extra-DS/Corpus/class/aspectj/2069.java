...
public class MessageTest extends TestCase {
    ...
    private static final String ME = // XXX
    "org.aspectj.bridge.MessageTest";
    ...
    void checkListOrder(List list, Comparator c) {
        // XXX util
        assertNotNull(list);
        assertNotNull(c);
    ...
    public void testKindOrder() {
        ...
        checkListOrder(Arrays.asList(new String[] { "a", "b", "C" }), String.CASE_INSENSITIVE_ORDER);
    ...
    public void checkEmptyMessageHolder(IMessageHolder h, final boolean handleMessageResult) {
    ...
    public void testMessage() {
        ...
        // XXX unchecked
        File f = new File("some/file.java");
    ...
    protected void roundTripCheck(IMessage message, String input, IMessage.Kind kind, Throwable thrown, ISourceLocation sourceLocation) {
        ...
        assertTrue("" + sourceLocation, equals(sourceLocation, m.getSourceLocation()));
    ...
    String checkExpectedKind(IMessage m) {
        ...
        for (int i = 0; i < KINDS.length; i++) {
            if (expected.get(i) != checkKindMethods(m, i)) {
                String s = "expected " + expected.get(i) + " for is{Method} for " + KINDS[i];
                result.append(s + "\n");
            }
        }
