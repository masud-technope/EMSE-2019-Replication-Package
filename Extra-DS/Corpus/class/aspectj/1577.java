...
public class ConcreteCflowPointcut extends Pointcut {
    ...
    protected Test findResidueInternal(Shadow shadow, ExposedState state) {
        ...
        if (usesCounter) {
            ...
            if (// null for cflows managed by counters
            slots != null) {
                for (Iterator i = slots.iterator(); i.hasNext(); ) {
                    Slot slot = (Slot) i.next();
                    //System.out.println("slot: " + slot.formalIndex);
                    state.set(slot.formalIndex, new BcelCflowAccessVar(slot.formalType, cflowField, slot.arrayIndex));
                }
            }
    ...
    private static final Member cflowStackIsValidMethod = MemberImpl.method(UnresolvedType.forName(NameMangler.CFLOW_STACK_TYPE), 0, "isValid", "()Z");

    private static final Member cflowCounterIsValidMethod = MemberImpl.method(UnresolvedType.forName(NameMangler.CFLOW_COUNTER_TYPE), 0, "isValid", "()Z");
