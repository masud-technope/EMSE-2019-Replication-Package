...
public class OptionChecker {
    ...
    public void checkOptions(String[] input, String[] expected, boolean resolve) {
        ...
        // Value.render(values);
        String[] actual = values.render();
        checkEqual(expected, actual);
    }

    public void checkOptionsNegative(String[] input, String expectedMissedMatchErr, String expectedResolveErr) {
        checkOptionsNegative(input, null, expectedMissedMatchErr, expectedResolveErr);
    }

    public void checkOptionsNegative(String[] input, String expectedInValuesException, String expectedMissedMatchErr, String expectedResolveErr) {
        Values values = getValuesNegative(input, expectedInValuesException);
    ...
    private Values getValuesNegative(String[] input, String expectedInExceptionMessage, Options options) {
        try {
            return options.acceptInput(input);
        } catch (Option.InvalidInputException e) {
            String m = e.getFullMessage();
            boolean ok = (null != expectedInExceptionMessage) && (-1 != m.indexOf(expectedInExceptionMessage));
            if (!ok) {
                e.printStackTrace(System.err);
                if (null != expectedInExceptionMessage) {
                    m = "expected \"" + expectedInExceptionMessage + "\" in " + m;
                }
                assertionFailed(m);
            }
            return null;
    ...
    private Values getValuesNegative(String[] input, String expectedInExceptionMessage) {
        return getValuesNegative(input, expectedInExceptionMessage, options);
    }
    ...
    private void checkContains(String expected, String expectedIn) {
        if (null == expected) {
            ...
            if ((null == expectedIn) || (-1 == expectedIn.indexOf(expected))) {
                assertionFailed("expected \"" + expected + "\" in \"" + expectedIn + "\"");
            }
        }
    }
    ...
    private void checkEqual(String[] expected, String[] actual) {
        if (!isEqual(expected, actual)) {
            assertionFailed("expected \"" + safeString(expected) + "\" got \"" + safeString(actual) + "\"");
        }
    }
