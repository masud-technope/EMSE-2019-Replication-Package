 ...
 */
    static final int // copy source directly, else applies logic operations
    BLIT_SRC = 1, // enable alpha blending
    BLIT_ALPHA = 2, // enable dithering in low color modes
    BLIT_DITHER = 4;

    /**
 * Alpha mode, values 0 - 255 specify global alpha level
 */
    static final int // Fully opaque (ignores any alpha data)
    ALPHA_OPAQUE = 255, // Fully transparent (ignores any alpha data)
    ALPHA_TRANSPARENT = 0, // Use alpha channel from separate alphaData
    ALPHA_CHANNEL_SEPARATE = -1, // Use alpha channel embedded in sourceData
    ALPHA_CHANNEL_SOURCE = -2, // Use transparency mask formed by bytes in alphaData (non-zero is opaque)
    ALPHA_MASK_UNPACKED = -3, // Use transparency mask formed by packed bits in alphaData
    ALPHA_MASK_PACKED = -4, // Consider source palette indices transparent if in alphaData array
    ALPHA_MASK_INDEX = -5, // Consider source RGBs transparent if in RGB888 format alphaData array
    ALPHA_MASK_RGB = -6;
 ...
 * Data types (internal)
 */
    private static final int // direct / true color formats with arbitrary masks & shifts
    TYPE_GENERIC_8 = 0, TYPE_GENERIC_16_MSB = 1, TYPE_GENERIC_16_LSB = 2, TYPE_GENERIC_24 = 3, TYPE_GENERIC_32_MSB = 4, TYPE_GENERIC_32_LSB = 5, // palette indexed color formats
    TYPE_INDEX_8 = 6, TYPE_INDEX_4 = 7, TYPE_INDEX_2 = 8, TYPE_INDEX_1_MSB = 9, TYPE_INDEX_1_LSB = 10;
            ...
            for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
                ...
                switch(alphaMode) {
                    ...
                    case ALPHA_MASK_PACKED:
                        ...
                        ap += (sfx >> 16);
                        break;
                    case // could speed up using binary search if we sorted the indices
                    ALPHA_MASK_INDEX:
                        {
                            int i = 0;
                            while (i < alphaData.length) {
