...
public class GenericTreeNode implements Comparable {
     ...
     */
    public static boolean traverse(GenericTreeNode lhsParent, GenericTreeNode rhsParent, GenericTreeNodeListOrdererFactoryI childrenPrepFactory, GenericTreeNodesVisitorI visitor) {
        if (null == visitor) {
        ...
        if (visitor.visit(lhsParent, rhsParent)) {
            ...
            if (null != childrenPrepFactory) {
                GenericTreeNodeListOrdererI factory = childrenPrepFactory.produce(lhsParent, rhsParent, visitor);
                if (null != factory) {
            ...
            while (true) {
                ...
                if (!traverse((GenericTreeNode) lhs, (GenericTreeNode) rhs, childrenPrepFactory, visitor)) {
                    return false;
                }
            }
        }
    ...
    public void init(GenericTreeNode parent, Comparator nodeComparator, Object node, List children, Class childClass) {
        // weak sync
        if (ready)
            ready = false;
        if (null == node) {
    ...
    public boolean equals(Object input) {
        if (input == this) {
            ...
            if (node.equals(in.getNode())) {
                // assumes nodes are unique, not flyweights?
                return true;
            } else {
                Comparator inComp = in.getNodeComparator();
                if ((this == nodeComparator) || (this == inComp) || (nodeComparator == inComp) || (nodeComparator.equals(inComp))) {
        ...
        public int compare(Object lhs, Object rhs) {
            ...
            if (Integer.MAX_VALUE == result) {
                ...
                if (Integer.MAX_VALUE == result) {
                    ...
                    if ((Integer.MAX_VALUE == result) || ((0 == result) && (null == lhComp))) {
                        ...
                        String err = "Program error - result: " + result + " lhComp: " + lhComp + " rhComp: " + rhComp;
                        throw new Error(err);
                    }
                }
            }
