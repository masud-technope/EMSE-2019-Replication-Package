...
public class ModulesTest extends TestCase {
    ...
    static {
        String[] names = { "ajbrowser", "ajde", "ajdoc", "asm", "aspectj5rt", "bridge", "loadtime", "loadtime5", "org.aspectj.ajdt.core", "runtime", "taskdefs", "testing-client", "testing-util", "tests", "util", "weaver" };
    ...
    private static boolean delete(File file) {
        ...
        if (file.isFile()) {
            ...
            for (int i = 0; i < files.length; i++) {
                if (!ModulesTest.delete(files[i]) && result) {
                    result = false;
                }
            }
    ...
    public void skip_testBuildingProduct() {
        ...
        File distDir = new File(buildDir, "dist");
        File jarDir = new File(buildDir, "jars");
        File productDir = new File(modulesDir, Util.path(new String[] { "build", "products", productName }));
        jarDir.mkdirs();
        distDir.mkdirs();
        Project project = new Project();
        project.setBaseDir(modulesDir);
