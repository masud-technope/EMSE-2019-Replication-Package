...
public class DeclareParentsDeclaration extends DeclareDeclaration {
    ...
    public static final ChildPropertyDescriptor CHILD_TYPE_PATTERN_PROPERTY = //$NON-NLS-1$
    new ChildPropertyDescriptor(DeclareParentsDeclaration.class, "childTypePattern", TypePattern.class, MANDATORY, NO_CYCLE_RISK);

    public static final SimplePropertyDescriptor IS_EXTENDS_PROPERTY = //$NON-NLS-1$
    new SimplePropertyDescriptor(DeclareParentsDeclaration.class, "isExtends", boolean.class, MANDATORY);

    public static final ChildListPropertyDescriptor PARENTS_TYPE_PATTERNS_LIST_PROPERTY = //$NON-NLS-1$
    new ChildListPropertyDescriptor(DeclareParentsDeclaration.class, "typePatternsList", TypePattern.class, NO_CYCLE_RISK);
    ...
    ASTNode clone0(AST target) {
        DeclareParentsDeclaration result = new DeclareParentsDeclaration(/*,declareDecl*/
        target);
        result.setSourceRange(this.getStartPosition(), this.getLength());
        result.setJavadoc((Javadoc) ASTNode.copySubtree(target, getJavadoc()));
        result.setChildTypePattern((TypePattern) ASTNode.copySubtree(target, getChildTypePattern()));
        result.setExtends(isExtends());
        result.parentTypePatterns().addAll(ASTNode.copySubtrees(target, parentTypePatterns()));
        return result;
    }
