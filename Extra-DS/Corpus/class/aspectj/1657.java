...
class ErrHandler {
    ...
    public void handle(String message, Throwable t) {
        String eMessage = (null == t ? "" : t.getClass().getName() + ": " + t.getMessage());
                ...
                if (arg.startsWith("-")) {
                    switch(arg.charAt(1)) {
                        ...
                        case 'I':
                            ...
                            break;
                        case 'b':
                            result.collapseWhitespace = true;
                            break;
                        case 'B':
                            result.collapseWhitespace = false;
                            break;
                        case 't':
                            result.trimWhitespace = true;
                            break;
                        case 'T':
                            result.trimWhitespace = false;
                            break;
                        case 's':
                            ++i;
                            if (i < args.length) {
            ...
            if (null == path) {
                ...
                if (!file.exists() && file.canRead()) {
                    ...
                    try {
                        ...
                        String message = "Error processing file " + path + " at line " + lineNumber + ": \"" + line + "\"" + ": " + e.getClass().getName() + ": " + e.getMessage();
...
class ReplaceOperation {
     ...
     */
    protected static boolean isValidSearch(String search) {
     ...
     */
    protected static boolean isValidReplace(String replace) {
        // todo: too weak to be useful now
    ...
    * <li> emit unmatched suffix, if any</li>
    ...
    public String replace(String input) {
        ...
        if (matches(input)) {
            ...
            if (specialChar) {
                handler.handle(label + "\\ without register: " + replace, ErrHandler.ABORT);
            }
