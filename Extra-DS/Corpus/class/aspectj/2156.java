...
public class AspectJElementHierarchy implements IHierarchy {
    ...
    public IProgramElement getElement(String handle) {
        ...
        // st.nextToken();
        String file = AsmManager.getDefault().getHandleProvider().getFileForHandle(handle);
        // st.nextToken();
        int line = AsmManager.getDefault().getHandleProvider().getLineNumberForHandle(handle);
    ...
    public IProgramElement findElementForSourceLine(ISourceLocation location) {
        try {
            return findElementForSourceLine(AsmManager.getDefault().getCanonicalFilePath(location.getSourceFile()), location.getLine());
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private IProgramElement findNodeForSourceLineHelper(IProgramElement node, String sourceFilePath, int lineNumber, int offSet) {
        ...
        if (node != null && node.getChildren() != null) {
            for (Iterator it = node.getChildren().iterator(); it.hasNext(); ) {
                IProgramElement foundNode = findNodeForSourceLineHelper((IProgramElement) it.next(), sourceFilePath, lineNumber, offSet);
                if (foundNode != null)
                    return foundNode;
            }
        }
    ...
    public void updateHandleMap(Set deletedFiles) {
        ...
        for (Iterator iter = k.iterator(); iter.hasNext(); ) {
            ...
            if (deletedFiles.contains(getFilename(handle)))
