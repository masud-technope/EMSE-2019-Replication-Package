...
public class IncrementalCase {
	 ...
	 */
    public boolean run(File srcBase, IMessageHandler handler) throws IOException {
        final String cname = ReflectionFactory.ECLIPSE;
        File targetBase = makeDir(getSandboxDir(), "IncrementalCaseSandbox", handler);
        if (null == targetBase) {
            return false;
        }
        ...
        final ICommand compiler = ReflectionFactory.makeCommand(cname, handler);
        ...
        for (int i = 1; result && (i < 10); i++) {
            ...
            files.clear();
            FileUtil.copyDir(srcBase, targetSrc, fromSuffix, toSuffix, collector);
            if (// XXX detect incomplete?
            0 == files.size()) {
                break;
            }
            ...
            if (1 == i) {
                ...
                // sigh
                argList.addAll(Arrays.asList(FileUtil.getAbsolutePaths(fra)));
                ...
                result = compiler.repeatCommand(compilerMessages);
            }
            result = verifyCompile(i, result, srcBase, targetSrc, targetClasses, defs[i - 1], compilerMessages, commandLine, handler);
        }
        return result;
    }
    ...
    protected boolean verifyCompile(int iteration, boolean result, File srcDir, File sandboxSrcDir, File sandboxClassesDir, Definition def, IMessageHolder compilerMessages, StringBuffer commandLine, IMessageHandler handler) {
        ...
        if (def.expectFail == result) {
            failures.append("iteration " + iteration + " expected to " + (def.expectFail ? "fail\n" : "pass"));
        }
        ...
        String[] expected = (null != def.errors ? def.errors : def.eclipseErrors);
        if (haveAll("errors", expected, messages, handler)) {
            if (!ignoreWarnings) {
                messages = compilerMessages.getMessages(IMessage.WARNING, IMessageHolder.EQUAL);
                expected = (null != def.warnings ? def.warnings : def.eclipseWarnings);
	 ...
	 */
    protected // XXX util
    File getSandboxDir() throws // XXX util
    IOException {
    ...
    protected List getBaseArgs(File srcDir, File classesDir) {
        ...
        String[] input = new String[] { "-verbose", //				System.getProperty("sun.boot.class.path"),
        "-d", classesDir.getAbsolutePath() };
        return Collections.unmodifiableList(new ArrayList(Arrays.asList(input)));
    }

    protected File makeDir(File parent, String name, // XXX util
    ...
    // -------------------------------------- test case verification
    List normalizeFilenames(String[] ra) {
        // XXX util
    ...
    String normalizeFilename(String s) {
        // XXX error-prone
    ...
    boolean haveAll(String label, String[] expected, IMessage[] messages, IMessageHandler handler) {
        if (null == expected) {
        ...
        sb.append("]");
        final String context = label + "\n in context haveAll expected=" + Arrays.asList(expected) + " exp=" + sb + " actual=" + Arrays.asList(messages);
        ...
        for (int i = 0; i < exp.length; i++) {
            ...
            for (int j = 0; !found && (j < messages.length); j++) {
                ...
                if (found) {
                    ...
                    if (foundSet.get(j)) {
                        info(handler, "duplicate " + label + " expected: " + exp[i]);
                    }
                    foundSet.set(j);
                }
            }
            if (!found) {
                String s = "expected " + label + " not found: " + exp[i] + context;
                fail(handler, s);
                if (!result) {
                    result = false;
                }
    ...
    static class Definition {
        ...
        String[] eclipseWarnings;
         ...
         Definition(String prefix, Properties props) {
            ...
            expectFail = (((null != errors) && (0 < errors.length)) || ((null != eclipseErrors) && (0 < eclipseErrors.length)));
        }
        ...
        public String toString() {
            return "Definition " + " expectFail=" + expectFail + " prefix=" + prefix + " files=" + safe(files) + " recompiled=" + safe(recompiled) + " errors=" + safe(errors) + " warnings=" + safe(warnings) + " eclipseErrors=" + safe(eclipseErrors) + " eclipseWarnings=" + safe(eclipseWarnings);
        }
