...
public class Sandbox {
     ...
     */
    // XXX required after completing tests? 
    ...
    public  Sandbox(File testBaseDir, Validator validator) {
        ...
        // XXX not really iax
        Sandbox.iaxWrite(sandboxDir, "sandboxDir");
    ...
    private String getToStringLeader() {
        if (null == toStringLeader) {
            toStringLeader = "Sandbox(" + sandboxDir.getName() + ", " + testBaseSrcDir.getName();
        }
    ...
    public String toLongString() {
        return getToStringLeader() + ", " + classesDir.getPath() + (null == command ? ", (null command)" : ", " + command) + ")";
    }
    ...
    void clearCommand(AjcTest caller) {
        ...
        if (null != command) {
            ...
            if (command instanceof AjcTaskCompileCommand) {
                // XXX urk!
    ...
    void reportClassDiffs(final IMessageHandler handler, IncCompilerRun caller, long classesDirStartTime, String[] expectedSources) {
        LangUtil.throwIaxIfFalse(0 < classesDirStartTime, "0 >= " + classesDirStartTime);
        boolean acceptPrefixes = true;
        Diffs diffs = org.aspectj.testing.util.FileUtil.dirDiffs("classes", classesDir, classesDirStartTime, ".class", expectedSources, acceptPrefixes);
    ...
    void compilerRunInit(CompilerRun caller, File testBaseSrcDir, File[] aspectPath, boolean aspectpathReadable, File[] classPath, boolean classpathReadable, String bootclassPath) {
        if (null != testBaseSrcDir) {
