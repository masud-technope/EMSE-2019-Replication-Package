...
class AssertTest {
    ...
    static void testContexts() {
        ...
        boolean pass = false;
        try {
            String s = InnerStaticInitFailure.class.getName();
        } catch (AssertionError e) {
            pass = true;
            ...
            Tester.check(pass, "no assertion in static initializer");
        }
        AssertTest enclosing = new AssertTest();
        // field initialization of inner class
        pass = false;
        try {
            enclosing.new InnerFieldInitFailure();
        } catch (AssertionError e) {
            pass = true;
        } finally {
            Tester.check(pass, "no assertion in field initializer");
        }
        // instance initialization of inner class
        pass = false;
        try {
            enclosing.new InnerInitFailure();
        } catch (AssertionError e) {
            pass = true;
        } finally {
            Tester.check(pass, "no assertion in constructor");
        }
        ...
        enclosing.x = 0;
        if (false) {
            // disabled now as current javac/j2se bug XXX
    ...
    static void requireAssertions() {
        ...
        try {
            ...
            Tester.check(pass, "assertions not enabled");
        }
    }
    ...
    static void testExpressions() {
        ...
        try {
            assert x > 0 : new StringWrapper("String");
        } catch (AssertionError e) {
            check(e, "String");
        }
        try {
        ...
        try {
            assert MyError.throwError("throwit");
        } catch (MyError e) {
            check(e, "throwit");
        } catch (AssertionError e) {
            Tester.check(false, "expected throwit-1");
        }
        try {
            assert x > 0 : MyError.throwError("throwit");
        } catch (MyError e) {
            check(e, "throwit");
        } catch (AssertionError e) {
            Tester.check(false, "expected throwit-2");
        }
        try {
            assert MyInnerError.throwError("throwit");
        } catch (MyInnerError e) {
            check(e, "throwit");
        } catch (AssertionError e) {
            Tester.check(false, "expected throwit-1");
        }
        try {
            assert x > 0 : MyInnerError.throwError("throwit");
        } catch (MyInnerError e) {
            check(e, "throwit");
        } catch (AssertionError e) {
            Tester.check(false, "expected throwit-2");
    ...
    static void check(Error e, String prefix) {
        ...
        Tester.check(m.startsWith(prefix), "expected " + m + " to start with " + prefix);
    }
    ...
    private class InnerInstanceTester {
        ...
        void testExpressions() {
            // ---------- tests of not throwing assertions
            // first is any boolean expression
            // variable from enclosing instance
            x = 0;
            assert x < 2;
            assert // parser questions
            x < 2;
